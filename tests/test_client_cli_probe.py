...
an agent harness:
    ...
    ok -> 0 · can't reach it -> 3 · not set up -> 4
...
def test_status_unreachable_exits_three(monkeypatch, capsys):
    """A configured-but-down endpoint is the can't-reach-it case: the value a
    down daemon reports is deliberately unchanged (#3832 both-directions)."""
    ...
    assert json.loads(capsys.readouterr().out)["status"] == "tortoise_unavailable"
...
def test_status_not_configured_exits_four(monkeypatch, capsys):
    ...
    outage. The driver is unchanged (it has no notion of a missing endpoint and
    falls back to the self-host default), so the probe splits the state out.

    On THIS probe 'TORTOISE_MCP_URL is unset' is the definition of
    not-configured, and the rewritten payload must not contradict itself: the
...
def test_status_down_daemon_with_declared_default_stays_unavailable(monkeypatch, capsys):
    """Boundary: declaring the endpoint - even as the bare default - is what
    moves a down daemon OUT of not-configured. The probe never guesses."""
...
def test_status_forwards_driver_not_configured(monkeypatch):
    """Forward-compat: a `not_configured` word from the driver maps to 4 and is
    never second-guessed by the endpoint heuristic."""
    monkeypatch.setenv("TORTOISE_MCP_URL", "http://127.0.0.1:1/mcp")
