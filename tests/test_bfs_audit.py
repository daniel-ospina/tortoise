...
def test_no_propagate_shock_callers():
    ...
    all_refs = _grep("propagate_shock", "tortoise/", "tools/", "validation/", "tests/")
    filtered = [
        l for l in all_refs
    ...
    for line in filtered:
        ...
        elif "tools/fix_" in path_part:
            callers["fixup_scripts (historical)"].append(line)
        elif "/tools/" in path_part:
...
def test_no_confidence_readers():
    ...
    all_refs = _grep("n\\.confidence", "tortoise/", "tools/", "validation/", "tests/")
