...
def _register_types():
    ...
    for t in (
        ...
        "gatePassed", "gateBlocked", "cardCompleted", "cardFailed",
    ):
        try:  # noqa: SIM105
...
class TestAppendAndReplay:
    def test_append_single_event(self, tmp_log):
        assert append_event(tmp_log, "cardCreated", {"card_id": "c1"}, event_id="ev-1")
        events = replay_events(tmp_log)
        assert len(events) == 1
        assert events[0]["type"] == "cardCreated"
        assert events[0]["card_id"] == "c1"
        assert events[0]["event_id"] == "ev-1"

    def test_dedup_by_event_id(self, tmp_log):
        append_event(tmp_log, "cardCreated", {"card_id": "c1"}, event_id="ev-1")
    ...
    def test_dedup_different_event_ids(self, tmp_log):
        append_event(tmp_log, "cardCreated", {"card_id": "c1"}, event_id="ev-1")
        append_event(tmp_log, "cardCreated", {"card_id": "c1"}, event_id="ev-2")
        events = replay_events(tmp_log)
        assert len(events) == 2

    def test_multiple_event_types(self, tmp_log):
        append_event(tmp_log, "cardCreated", {"card_id": "c1"}, event_id="ev-1")
        append_event(tmp_log, "stepStarted", {"card_id": "c1", "step_id": "s1"}, event_id="ev-2")
        append_event(tmp_log, "stepCompleted", {"card_id": "c1", "step_id": "s1"}, event_id="ev-3")
        events = replay_events(tmp_log)
        assert len(events) == 3
        types = [e["type"] for e in events]
        assert types == ["cardCreated", "stepStarted", "stepCompleted"]

    def test_no_event_id_no_dedup(self, tmp_log):
        append_event(tmp_log, "cardCreated", {"card_id": "c1"})
        append_event(tmp_log, "cardCreated", {"card_id": "c1"})
        events = replay_events(tmp_log)
...
class TestIntegrity:
    ...
    def test_verify_hash_changes_with_content(self, tmp_log):
        ...
        append_event(tmp_log, "cardCompleted", {"card_id": "c1"}, event_id="ev-2")
...
class TestCrashRecovery:
    ...
    def test_scan_incomplete_missing(self, tmp_log):
        append_event(tmp_log, "cardCreated", {"card_id": "c1"}, event_id="ev-1")
        append_event(tmp_log, "cardCompleted", {"card_id": "c1"}, event_id="ev-2")
        append_event(tmp_log, "cardCreated", {"card_id": "c2"}, event_id="ev-3")
        # c2 has no completion
        assert scan_incomplete(tmp_log, {"c1", "c2"}) == ["c2"]

    def test_scan_incomplete_failed_card(self, tmp_log):
        append_event(tmp_log, "cardCreated", {"card_id": "c1"}, event_id="ev-1")
        append_event(tmp_log, "cardFailed", {"card_id": "c1"}, event_id="ev-2")
        # cardFailed is also a completion
        assert scan_incomplete(tmp_log, {"c1"}) == []
    ...
    def test_recover_ignores_other_cards(self, tmp_log):
        append_event(tmp_log, "cardCreated", {"card_id": "c1"}, event_id="ev-1")
        append_event(tmp_log, "stepCompleted", {"card_id": "c1", "step_id": "s1"}, event_id="ev-2")
        append_event(tmp_log, "cardCreated", {"card_id": "c2"}, event_id="ev-3")
        append_event(tmp_log, "stepCompleted", {"card_id": "c2", "step_id": "other"}, event_id="ev-4")
