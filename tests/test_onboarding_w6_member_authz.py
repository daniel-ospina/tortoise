...
def _team_state(fake, org_id: str) -> dict:
...
class TestSettingsViewDeleteSessionLane:
    def test_owner_session_view_and_delete_roundtrip(self, env, monkeypatch):
        ...
        _seed_membership(fake, org_id, user_id)
        sid = f"authz-s1-{uuid.uuid4().hex[:6]}"
        cap = _capture_key_lane(client, key, org_id, sid)
    ...
    def test_non_member_session_is_403_before_any_data_op(self, env, monkeypatch):
        ...
        client, _fake = env
        _key, org_id = _provision_anon(client)
    ...
    def test_member_of_other_team_cannot_target_this_team(self, env, monkeypatch):
        """?org_id= targeting a team the session user is NOT a member of →
    ...
    def test_unauthenticated_is_401(self, env):
        client, _fake = env
        assert client.get("/v1/sessions/whatever-3?org_id=t").status_code == 401
        assert client.delete("/v1/sessions/whatever-3?org_id=t").status_code == 401
...
class TestSettingsViewDeleteKeyLane:
    def test_key_lane_roundtrip_unchanged(self, env):
        ...
        cap = _capture_key_lane(client, key, org_id, sid)
        assert cap["first_capture"] is True
        det = client.get(f"/v1/sessions/{sid}?org_id={org_id}",
                         headers={"Authorization": f"Bearer {key}"})
        assert det.status_code == 200, det.text
        d = client.delete(f"/v1/sessions/{sid}?org_id={org_id}",
                          headers={"Authorization": f"Bearer {key}"})
        assert d.status_code == 200, d.text
        assert client.get(f"/v1/sessions/{sid}?org_id={org_id}",
                          headers={"Authorization": f"Bearer {key}"}
                          ).status_code == 404
    ...
    def test_cross_team_key_sees_404_not_the_session(self, env):
        ...
        assert client.get(f"/v1/sessions/{sid}?org_id={team_b}",
                          headers=hb).status_code == 404
        assert client.delete(f"/v1/sessions/{sid}?org_id={team_b}",
                             headers=hb).status_code == 404
        # and the owner's session is untouched by the foreign attempt
        det = client.get(f"/v1/sessions/{sid}?org_id={team_a}",
    ...
    def test_capture_hook_stays_key_only(self, env, monkeypatch):
        ...
        _patch_session_user(monkeypatch, user_id)
        _seed_membership(fake, org_id, user_id)
        r = client.post(f"/v1/sessions?org_id={org_id}",
                        headers={"Authorization": _SESS},
