...
class TestOperatorStructuralQuery:
    def test_operator_uses_point_label_and_is_operator(self):
        ...
        results = run_structural_query(g, None, None, entity_type="operator", limit=10)
    ...
    def test_operator_kind_field_is_op_type(self):
        ...
        run_structural_query(g, "IMPL", None, entity_type="operator", limit=10)
        assert any("n.op_type = $kind" in q for q, _ in g.queries), \
            "operator kind filter must use op_type"

    def test_operator_context_filter(self):
        """Operator context filter included."""
        g = MockGraph([["op-3", 1.0]])
        run_structural_query(g, None, "licensing", entity_type="operator", limit=10)
        assert any("n.context = $context" in q for q, _ in g.queries), \
            "operator should support context filter"
...
class TestSourceStructuralQuery:
    def test_source_uses_source_label(self):
        ...
        results = run_structural_query(g, "T1", None, entity_type="source", limit=10)
    ...
    def test_source_kind_field_is_source_kind(self):
        ...
        g = MockGraph([["https://example.com", 1.0]])
        run_structural_query(g, "T1", None, entity_type="source", limit=10)
    ...
    def test_source_does_not_have_context(self):
        ...
        g = MockGraph([["https://example.com", 1.0]])
        run_structural_query(g, None, "some-context", entity_type="source", limit=10)
