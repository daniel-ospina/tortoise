...
try:
    for i in range({writes}):
        ...
        # Explicit SAVE per write: the writer-death test (#879) killpg's the
        # python writer while the daemonized redis-server survives — the SAVE
        # keeps RDB + AOF in sync so the survivor-reconnect assertion is
        # deterministic on every host (see #915 for the AOF durability
        # contract; graceful close() also persists via shutdown(save=True)).
