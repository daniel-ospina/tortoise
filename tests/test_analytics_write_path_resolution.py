...
def test_write_path_falls_back_to_jsonl_when_the_post_is_rejected(monkeypatch,
                                                                 tmp_path):
    """A 4xx/5xx from PostgREST does NOT raise, so an uninspected response
    silently discarded the event — the #3677 loss class, reachable whenever the
    key is revoked or INSERT-denied (a 401 raises nothing). It must reach the
    local JSONL instead of vanishing.

    Mutation that reds this test: delete the ``200 <= resp.status_code < 300``
    check (return unconditionally after the POST).
    """
    from tortoise import hosted_api as ha
...
def test_write_path_bounds_the_post_timeout(monkeypatch, tmp_path):
    ...
    # The call site must use the module constant (one tunable point, no drift)
    # and the constant itself must be sane.
