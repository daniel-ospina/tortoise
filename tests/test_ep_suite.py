...
def test_ep_camp_detection():
    """EP projects to a single Beta per claim → no multimodality.

    Camp detection: sample from EP's Beta posteriors, compute
    camp_frac (fraction of sample pairs in opposite regimes).
    Independence baseline for two Betas is ~0.25 (each has ~50%
    chance of being above its own median).

    Assert: camp_frac near 0.25 (±0.15) for all NAND pairs.
    SVBP (for comparison) should show camp_frac > 0.35.
    """
    random.seed(42)
    ep = InMemoryEP()
    ep.run(_NAND_GRAPH)
    ep_posts = dict(ep.posteriors)

    # EP: sample from Beta posteriors
    np.random.seed(42)
    n_samples = 2000
    ep_samples = {}
    for cid in [f"c{i}" for i in range(5)]:
        a, b = ep_posts.get(cid, (1.0, 1.0))
        ep_samples[cid] = np.random.beta(a, b, n_samples)

    # Camp check for (c0,c1) and (c2,c3) — the NAND pairs
    ep_camp_pairs = []
    for a, b in [("c0", "c1"), ("c2", "c3")]:
        s_a = ep_samples[a]
        s_b = ep_samples[b]
        med_a, med_b = np.median(s_a), np.median(s_b)
        hl = int(np.sum((s_a > med_a) & (s_b <= med_b)))
        lh = int(np.sum((s_a <= med_a) & (s_b > med_b)))
        cf = min(hl, lh) / n_samples
        ep_camp_pairs.append(cf)

    mean_cf = float(np.mean(ep_camp_pairs))

    # SVBP baseline: should detect camps
    random.seed(42)
    svbp = TortoiseSVBP(n_particles=100, n_svgd_steps=20, svgd_lr=0.01,
                        damping=0.5, max_iter=80, tol=5e-3, seed=42)
    svbp.run(_NAND_GRAPH)
    svbp_camp_pairs = []
    for a, b in [("c0", "c1"), ("c2", "c3")]:
        if svbp._has_particles(a) and svbp._has_particles(b):
            y_a = svbp._particles[a]
            y_b = svbp._particles[b]
            c_a = sigmoid(y_a)
            c_b = sigmoid(y_b)
            med_a, med_b = float(jnp.median(c_a)), float(jnp.median(c_b))
            hl = int(jnp.sum((c_a > med_a) & (c_b <= med_b)))
            lh = int(jnp.sum((c_a <= med_a) & (c_b > med_b)))
            cf = min(hl, lh) / len(c_a)
            svbp_camp_pairs.append(cf)

    svbp_mean_cf = float(np.mean(svbp_camp_pairs)) if svbp_camp_pairs else 0

    # EP camp_frac should be near independence (0.25), not like SVBP's camps
    assert abs(mean_cf - 0.25) < 0.15, \
        f"EP camp_frac={mean_cf:.3f} too far from independence baseline 0.25"
    # SVBP may not always hit 0.30 with small graphs; just verify EP < SVBP
    assert mean_cf < svbp_mean_cf * 1.1, \
        f"EP camp_frac ({mean_cf:.3f}) should not exceed SVBP ({svbp_mean_cf:.3f})"
