...
from tortoise.shared_state.events import register_event_type  # noqa: I001
from tortoise.shared_state.event_log import (
    append_event,
    replay_events,
    verify_hash,
    scan_incomplete,
...
def _register_card_types():
    ...
    for t in (
        "cardCreated", "stepStarted", "stepCompleted",
        "cardCompleted", "cardFailed", "checkpoint",
    ):
        try:  # noqa: SIM105
...
def test_crash_recovery_full_cycle(tmp_log):
    ...
    Flow:
      1. Card c1 completes fully (cardCreated → stepStarted → stepCompleted → cardCompleted)
      2. Checkpoint saved (c1 done)
      3. Card c2 starts but crashes mid-step (cardCreated → stepStarted — no completion)
      4. Card c3 is created but crashes immediately (cardCreated only)
    ...
    append_event(tmp_log, "cardCreated", _card_created("c1"), event_id="ev-c1-create")
    append_event(tmp_log, "stepStarted", _step_event("s1", "c1"), event_id="ev-c1-s1-start")
    append_event(tmp_log, "stepCompleted", _step_event("s1", "c1"), event_id="ev-c1-s1-done")
    append_event(tmp_log, "cardCompleted", _card_created("c1"), event_id="ev-c1-complete")
    ...
    append_event(tmp_log, "cardCreated", _card_created("c2"), event_id="ev-c2-create")
    append_event(tmp_log, "stepStarted", _step_event("s1", "c2"), event_id="ev-c2-s1-start")
    # CRASH HERE — no stepCompleted, no cardCompleted for c2

    # c3 created, crashes immediately
    append_event(tmp_log, "cardCreated", _card_created("c3"), event_id="ev-c3-create")
    ...
    # Recover c2 — it had stepStarted for s1 but no stepCompleted
    r2 = recover(tmp_log, "c2", retry_fn)
    assert r2["card_id"] == "c2"
    assert r2["recovered"] is True
    # c2's last completed step is None (no stepCompleted event for c2)
    assert r2["last_step"] is None

    # Recover c3 — only cardCreated, no step events at all
    ...
    assert types == [
        "cardCreated", "stepStarted", "stepCompleted", "cardCompleted",
        "checkpoint",
        "cardCreated", "stepStarted",
        "cardCreated",
...
def test_crash_recovery_multiple_checkpoints(tmp_log):
    ...
    append_event(tmp_log, "stepStarted", _step_event("s2", "c1"), event_id="ev-c1-s2-start")
    # CRASH — no stepCompleted for s2
    ...
    # c1 had stepCompleted for s1 → last_step should be "s1"
...
def test_crash_recovery_no_crash_all_complete(tmp_log):
    ...
    for cid in ("c1", "c2", "c3"):
        append_event(tmp_log, "cardCreated", _card_created(cid), event_id=f"ev-{cid}-create")
        append_event(tmp_log, "stepStarted", _step_event("s1", cid), event_id=f"ev-{cid}-s1-start")
        append_event(tmp_log, "stepCompleted", _step_event("s1", cid), event_id=f"ev-{cid}-s1-done")
        append_event(tmp_log, "cardCompleted", _card_created(cid), event_id=f"ev-{cid}-complete")
...
def test_crash_recovery_card_failed_is_complete(tmp_log):
    """cardFailed counts as a completion — not reported as incomplete."""
    append_event(tmp_log, "cardCreated", _card_created("c1"), event_id="ev-c1-create")
    append_event(tmp_log, "cardFailed", _card_created("c1"), event_id="ev-c1-fail")

    incomplete = scan_incomplete(tmp_log, {"c1"})
    assert incomplete == [], f"cardFailed is a completion, got: {incomplete}"
...
def test_crash_recovery_hash_integrity_after_crash(tmp_log):
    ...
    events = [
        ("cardCreated", _card_created("c1"), "ev-c1-create"),
        ("stepStarted", _step_event("s1", "c1"), "ev-c1-s1-start"),
        ("stepCompleted", _step_event("s1", "c1"), "ev-c1-s1-done"),
        ("cardCreated", _card_created("c2"), "ev-c2-create"),
    ...
    append_event(tmp_log, "stepCompleted", _step_event("s1", "c2"), event_id="ev-c2-s1-done")
...
def test_crash_recovery_concurrent_cards_independent(tmp_log):
    ...
    append_event(tmp_log, "stepStarted", _step_event("s1", "c2"), event_id="ev-c2-s1-start")
    append_event(tmp_log, "stepCompleted", _step_event("s1", "c2"), event_id="ev-c2-s1-done")
    append_event(tmp_log, "cardCompleted", _card_created("c2"), event_id="ev-c2-complete")
...
def test_crash_recovery_dedup_on_recovery_events(tmp_log):
    ...
    append_event(tmp_log, "stepStarted", _step_event("s1", "c1"), event_id="ev-recovery-s1")
    # Same event_id — should be dedup'd
    assert append_event(tmp_log, "stepStarted", _step_event("s1", "c1"), event_id="ev-recovery-s1")

    events = replay_events(tmp_log)
    step_starts = [e for e in events if e["type"] == "stepStarted"]
    assert len(step_starts) == 1, (
        f"Dedup should prevent duplicate stepStarted, got {len(step_starts)}"
    )


def test_scan_incomplete_card_never_created(tmp_log):
    """scan_incomplete with a card_id that has no cardCreated event at all.

    Real crash scenario: card was created in memory but cardCreated never
    flushed to disk before SIGKILL. scan_incomplete should not crash.
    """
    append_event(tmp_log, "cardCreated", _card_created("c1"), event_id="ev-c1-create")
    append_event(tmp_log, "cardCompleted", _card_created("c1"), event_id="ev-c1-done")
