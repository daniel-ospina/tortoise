...
def test_scan_socket_dirs_scoped_skips_custom_full_scan_finds(tmp_path):
...
def test_scan_socket_dirs_finds_either_marker_name(tmp_path):
    from tortoise.embedded_reaper import _scan_socket_dirs
...
def test_scan_socket_dirs_symlinked_marker_file_still_discovered(tmp_path):
...
def test_scan_socket_dirs_budget_expiry_partial_and_warns(tmp_path, caplog):
    ...
    with caplog.at_level("WARNING"):
        # `budget` is a DURATION (the walk takes its deadline after root
        # discovery, #3752); a negative one is already spent.
        res = _scan_socket_dirs(str(tmp_path), budget=-1.0)
...
def test_lock_is_tempdir_scoped_not_home_scoped():
    ...
    # #1658 / #3752: the lock's scope follows the SWEEP TARGET, whatever that
    # resolves to — a harness that redirects TMPDIR moves the lock with it.
    assert lock_path == os.path.join(
        os.path.realpath(_tf.gettempdir()),
        f".tortoise-reaper-{os.geteuid()}", ".reaper.lock"), \
        f"_LOCK_PATH {lock_path!r} must follow the sweep-domain tempdir"
    # #3752: ACTIVE_SUITES_DIR is HOST-pinned, and must NOT follow the sweep
    # domain — a production/cron sweep (--only-safe) reads it to defer kills
    # while a suite is mid-run, and an empty dir would silently lose that.
    ...
    assert os.path.dirname(lock_path) != coordination_root, \
        "the lock must never share the host coordination dir: the lock " \
        "follows the sweep domain, the markers are host-global (#3752/#4098)"
    if os.path.realpath(_tf.gettempdir()) != er._host_coordination_tmpdir():
        # A harness redirected the temp dir (the suite does, #3752). Pin the
        # redirect so a future refactor that silently stops honouring
        # TORTOISE_HOST_TMPDIR fails here and forces the decision again.
