...
class TestRegistryEquivalence:
    ...
    def test_derived_http_allowed_equals_literal(self):
        ...
        # Exact count: 65 tools - 4 excluded (team_create, backfill_v25,
        # ingest_corpus, index_sessions) = 61 (incl. 6 onboarding tools
        # #498/#499/#500 + 1 human-approval tool #531)
        assert len(HTTP_ALLOWED) == 61, f"Expected 61, got {len(HTTP_ALLOWED)}"
        assert len(derived) == 61
