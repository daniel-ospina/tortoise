...
def aggregate_metrics(session_results: list[dict]) -> dict:
    ...
    """
    kta_missed = sum(r.get("kta", {}).get("missed", 0) for r in session_results)
    kta_should = sum(r.get("kta", {}).get("should", 0) for r in session_results)
    ff_fires = sum(r.get("false_fire", {}).get("fires", 0) for r in session_results)
    ff_silent = sum(r.get("false_fire", {}).get("silent_required", 0) for r in session_results)
    push_prec_num = sum(r.get("push", {}).get("prec_num", 0) for r in session_results)
    push_prec_den = sum(r.get("push", {}).get("prec_den", 0) for r in session_results)
    push_rec_num = sum(r.get("push", {}).get("recall_num", 0) for r in session_results)
    push_rec_den = sum(r.get("push", {}).get("recall_den", 0) for r in session_results)
    wb_survived = sum(r.get("write_back", {}).get("survived", 0) for r in session_results)
    wb_total = sum(r.get("write_back", {}).get("total", 0) for r in session_results)
    cont_surfaced = sum(r.get("continuity", {}).get("surfaced", 0) for r in session_results)
    cont_total = sum(r.get("continuity", {}).get("total", 0) for r in session_results)
    iso_violations = sum(r.get("isolation", {}).get("violations", 0)
