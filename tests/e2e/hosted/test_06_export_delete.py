...
def _provision_owner_tenant(api, user_id: str) -> dict:
    ...
    org_id = f"e2e6d-{uuid.uuid4().hex[:10]}"
    r = api.post("/internal/provision",
                 headers={"Authorization": f"Bearer {INTERNAL_KEY}"},
                 data={"org_id": org_id, "org_name": f"E2E6D {org_id[-6:]}",
...
def test_owner_export_and_delete_lifecycle(api, session_jwt):
    ...
    r = api.post("/v1/session/key", headers=h,
                 data={"org_id": org_id, "purpose": "recovery"})
    ...
    r = api.get(f"/v1/organizations/{org_id}/export", headers=h)
    ...
    assert export.get("team", {}).get("id") == org_id

    # Owner deletion → 202 (soft delete + grace window)
    r = api.delete(f"/v1/organizations/{org_id}", headers=h)
...
def test_export_requires_session_auth_401(api, tenant_factory):
    ...
    r = api.get(f"/v1/organizations/{t['org_id']}/export")
