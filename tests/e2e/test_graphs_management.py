...
def test_graphs_last_backup_column_per_graph(page: Page) -> None:
    ...
    tooltip), the neutral empty copy for a graph with no backup, and never a
    team-wide count. The default row is a first-class key (legacy-flat
    manifests bucket to 'default'); an unidentified manifest (no graph_id)
    leaks onto no row. Copy comes from graphBackupCellState (#2784)."""
    ...
    expect(rows.nth(1)).to_contain_text("None recorded")  # g_dev: no backup
