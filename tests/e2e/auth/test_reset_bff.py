"""
POST /auth/reset — the BFF password-recovery route (#4054).

Runs the REAL Cloudflare Pages runtime (`wrangler pages dev` from the dashboard
Pages project root) against a focused mock GoTrue that emulates `/recover`.

The property under test is NON-ENUMERATION, and it is asserted as an EQUALITY of
responses across addresses that GoTrue treats differently (a normal address,
which it answers 200, and `refused@example.test`, which it answers 422). If a
known address and an unknown one ever produced different bytes, this route would
be an account-existence oracle. The upstream request log is read from the mock so
that "GoTrue was called" is observed rather than inferred.

Status discipline under test (the #3485 class): a provider/config fault is 503
and must never be folded into the 200 (a wrong anon key must not look like a
successful request).
"""
...
APP_PORT = int(os.environ.get("RESET_TEST_APP_PORT", "9042"))
MOCK_PORT = int(os.environ.get("RESET_TEST_MOCK_PORT", "9043"))
...
def test_known_and_unknown_addresses_are_indistinguishable(stack):
    ...
    calls = _calls()
    assert len(calls) == 2, f"expected two /recover calls, got {calls}"
...
def test_a_provider_refusal_is_still_an_indistinguishable_200(stack):
    ...
    s_refused, b_refused, _ = _post(APP, "/auth/reset", REFUSED)
...
def test_malformed_input_is_400_and_gotrue_is_not_called(stack, payload):
    _calls(reset=True)
    status, body, _ = _post(APP, "/auth/reset", payload)
...
def test_provider_outage_is_503(stack):
    ...
    try:
        _calls(reset=True)
        status, body, _ = _post(APP, "/auth/reset", KNOWN)
    finally:
        _fault(recover=False)
...
def test_config_fault_401_is_503(stack):
    ...
    assert status != 200, "a wrong anon key was reported as an accepted recovery request"
    assert json.loads(body)["error"] == "provider_unavailable", body
...
def test_non_post_methods_are_405(stack, method):
    _calls(reset=True)
    opener = urllib.request.build_opener(_NoRedirect())
    req = urllib.request.Request(f"{APP}/auth/reset", method=method)
    try:
...
def test_get_does_not_reach_gotrue(stack):
    """A GET is served the SPA shell by Pages, never a recovery mail request."""
    _calls(reset=True)
    opener = urllib.request.build_opener(_NoRedirect())
    req = urllib.request.Request(f"{APP}/auth/reset", method="GET")
    try:
        with opener.open(req, timeout=30) as r:
            r.read()
    except urllib.error.HTTPError as e:
