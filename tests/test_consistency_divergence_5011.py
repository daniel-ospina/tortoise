...
def test_a_torn_sidecar_is_reported_and_never_re_baselined(proj, tmp_path):
    """The sidecar's tmp path is per-process and `O_EXCL|O_NOFOLLOW`, and a
    sidecar that is not a REGULAR file (a directory, a FIFO, a symlink) is
    refused rather than opened — an unbounded read of a planted FIFO would hang
    the check, and a symlink would read a file the sidecar does not own.
    (1) Fails if `read_projection_state` opens the path blindly (FIFO hangs;
    directory raises OSError out of the check) or if a torn state is recorded
    over. (2) Reachable: the journal directory is writable by the operator.
    """
    log_path = str(tmp_path / "torn.jsonl")
    _seed(proj, log_path, [_pt("t1", "a")])
    state_path = log_path + ".projection-state.json"

    os.mkdir(state_path)  # a directory where the sidecar belongs
    r = check_consistency(log_path, proj)
    assert r["state_error"], r
