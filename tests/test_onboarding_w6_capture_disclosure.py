    ...
    def test_guard_skips_receipt_when_session_gone_before_write(self, monkeypatch):
        ...
        def fake_query(query, params=None, **kw):
            ...
            if "RETURN count(s)" in query and "{id:$sid}" in query:
                # #2335 WI-2b: the session probe also reads
                # capture_ok/capture_extractor; #3681 adds s.harness (4th
                # column). A Session that is "gone" returns 0 rows with null
                # state columns.
                return type("Rows", (), {"result_set": [[0, None, None, None]]})()
