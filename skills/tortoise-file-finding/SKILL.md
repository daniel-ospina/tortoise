...
1. Accept finding content and context from the user or calling agent.
2. Call `tortoise_create_point(kind='statement', content=<finding>, context=<source>)` to create the Point.
...
- **G1 (Static):** Content must be non-empty. Context must be a valid source identifier.
...
- If `tortoise_query` returns empty, report "no existing Points in this context" — this is a success state.
