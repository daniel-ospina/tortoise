...
- Which claims have no counter-arguments? File NAND edges for opposing views.
...
### 6. CONVERGE — The decision becomes clear
...
### 7. PRESERVE — Everything in the graph

Run `tortoise_compute_confidence` to propagate belief scores. Verify chain integrity with `tortoise_check_structure`. The decision and its full reasoning are now preserved — future you or your team can query why we decided this.
