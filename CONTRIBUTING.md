...
baseline (a duplicate name a name comparison cannot see), an exemption that has become reachable, a
transform that reuses an approved name but is defined outside `tortoise/mcp_server.py` (a class
name is not an identity — `__module__` is checked), or an `approval_status` that is neither `pending-owner-approval` nor `approved`. A
