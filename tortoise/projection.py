...
class FalkorProjection:
    ...
    URI:       FalkorProjection.from_uri('docker://:pass@host:16379/graph')
    ...
    def _upsert(self, p: dict) -> None:
        ...
        # Ontology v2.1: about edges from aboutEntities property (per-type)
        if p.get("aboutEntities"):
            self._link_about_entities(p["id"], p["aboutEntities"])
    ...
    def _create_edges(self, p: dict) -> None:
        ...
        op = p["operator"]
        rel_type = {"NAND": "NAND", "IMPL": "IMPL"}.get(op["op_type"])
        if not rel_type:
            return  # skip unknown operator types (was: INPUT)
    ...
    def _link_about_entities(self, point_id: str, entities: list[str]) -> None:
        """Create per-type about edges from aboutEntities property list.

        Entity format: "Subject:name" → aboutSubject edge to Subject node.
        Supports Subject, Object, Action, Event prefixes.
        """
        kind_map = {
            "Subject": ("aboutSubject", "Subject", "subjectKind"),
            "Object": ("aboutObject", "Object", "objectKind"),
            "Action": ("aboutAction", "Action", "actionKind"),
            "Event": ("aboutEvent", "Event", "eventKind"),
        }
        for entity_ref in entities:
            ...
            if prefix not in kind_map:
                continue
            edge_type, label, kind_prop = kind_map[prefix]
            # MERGE stub entity if it doesn't exist
            self.g.query(
                f"MERGE (e:{label} {{name: $name}}) "
                f"ON CREATE SET e.{kind_prop} = 'other', e.createdAt = $now",
                params={"name": name.strip(), "now": _now_iso()},
            )
            # Create about edge
            self.g.query(
                f"MATCH (p:Point {{id: $pid}}) "
                f"MATCH (e:{label} {{name: $name}}) "
                f"MERGE (p)-[:{edge_type}]->(e)",
                params={"pid": point_id, "name": name.strip()},
            )
    ...
    def _upsert_source(self, ev: dict) -> None:
        ...
        self.g.query(
            ...
            "           s.updatedAt = $now",
            params={
                "url": url, "id": sid or url,
                "st": ev.get("sourceType", "document"),
    ...
    def edge_stats(self) -> dict:
        """Return {operators, impl_edges, nand_edges} for diagnostics."""
        ops = self.g.query(
