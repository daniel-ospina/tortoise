        ...
        for i, item in enumerate(bundle.get("points") or []):
            if isinstance(item, dict) and item.get("status") == "live":
                violations.append({
                    ...
                    "message": f"ingest: points[{i}] status:'live' is not "
                               f"allowed under promotion_policy 'gated' — "
                               f"pass promotion_policy='auto' for explicit "
                               f"live, or keep draft and promote via "
