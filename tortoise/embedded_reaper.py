...
# connection) writes `<OWNER_INFLIGHT_PREFIX><pid>-<start>` for the whole
# duration of `RedisMixin.__init__`. It is a LIVENESS-ONLY hold read by
# `embedded_lifecycle.cotenant_holds_server` (the last-client decision), so
# that a peer process's close cannot tear the server down under the
# attaching construction. The prefix is load-bearing: it is deliberately
# unparseable as a `<pid>` record name, so `_owner_records` below IGNORES
# these files entirely — a construction claim is not an owner record and must
# never move the reaper's `total`/`live` arithmetic. Declared here (the
