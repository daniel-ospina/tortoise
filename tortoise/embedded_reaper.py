...
def _discover_from_live(live_pids, jobs, tmpdir, seen_dirs,
    ...
    scan = _scan_socket_dirs(tmpdir, full_scan=full_scan)
...
def _iter_candidate_dirs_over_roots(tmpdir: str, *, predicate,
                                    budget: float | None = None,
                                    ) -> _ScanResult:
    """`_iter_candidate_dirs` over the whole walk-root set (#3752).

    A harness that redirects ``TMPDIR`` to one private per-session root
    (``tests/_tmpdir_isolation.py``) puts every scratch dir one level deeper
    — ``T/tt_<8-char>/<socket dir>/redis.socket`` is depth 3 — so a depth-1
    scan of the tempdir alone cannot see it, and a session's killed-suite
    residue would never be reaped.
    ...
    ``budget`` is a DURATION in seconds (``None`` = the default budget, never
    ...
    return _ScanResult(sorted(set(dirs)), complete)
...
def _scan_socket_dirs(tmpdir: str, *, full_scan: bool = False,
                      budget: float | None = None) -> _ScanResult:
    ...
    rather than reported as a finished one. A ``None`` budget means "use
    the default" — never unbounded (#1642 FIX 2's always-bounded intent is
    preserved for direct callers); it is a DURATION, and the walk's deadline
    starts after root discovery (see _iter_candidate_dirs_over_roots).
    """
    predicate = _always_match if full_scan else _ephemeral_name
    res = _iter_candidate_dirs_over_roots(
        tmpdir, predicate=predicate, budget=budget)
    out: list[str] = []
    for d in res.dirs:
        try:
...
def _socket_walk_roots(tmpdir: str) -> list[str]:
    """The roots every census walk covers: the tempdir, plus one level of
    nesting under known session-scratch roots (#3752).

    Consumed by `_iter_candidate_dirs_over_roots` (the depth-1 `os.scandir`
    census of #4068, which replaced the `find` subprocess this was originally
    written for).
...
_LOCK_PATH = os.path.join(
    ...
    # make the session-end sweep SKIP and leak its own servers.
    #
    # #4098: the lock DIR is uid-scoped (`.tortoise-reaper-<euid>`). A fixed
    # name is pre-creatable under a shared `/tmp` by ANY local uid, and the
    # ownership gate in _ReaperLock.acquire() then fails closed forever — a
    # permanent, zero-privilege denial of the victim's reaper.
...
def _sweep_quarantine_dirs(dry_run: bool = False,
    ...
    """
    tmpdir = _real_gettempdir()
    scan = _iter_candidate_dirs_over_roots(
        tmpdir,
