...
hosted_api.py's inline `get_current_org` FastAPI dependency.
...
TODO: The inline `get_current_org` dependency in hosted_api.py is
missing the `last_used_at` tracking that was present in the old
AuthMiddleware here. Add the following after org resolution in
hosted_api.py's get_current_org:
