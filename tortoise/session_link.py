...
Resolution trigger rule (pinned by the plan):
  ...
  - bare ``#n``                             — ONLY with a false-positive guard
    ...
    never match)
  - first-match PER extracted point, ALL-matches for the Session node
  - no-match ⇒ no link, honest (nothing is fabricated)

Resolution is by the STABLE Object id (``github-issue-{repo}-{n}`` — the
WorkItem Object's external anchor; GitHub indexer mints it once and
supersession/status folds never change it), so ``aboutObject`` never dangles
on supersede — "resolve-to-current by externalId" (P1-2). Misses are
warn-logged; outcomes are tracked on the Session node
...
def _object_id(repo: str, num: int) -> str:
    """The stable WorkItem Object id minted by the GitHub indexer
    (github_map.issue_to_object): ``github-issue-{org}/{repo}-{n}``."""
    return f"github-issue-{repo}-{num}"
...
def _resolve_targets(proj, refs: list[dict[str, str]]) -> list[str]:
    ...
    for ref in refs:
        ...
        elif ref["form"] == "repo_num":
            ids = [_object_id(ref["repo"], int(num))]
        # else bare_num: resolved by name suffix only (below).
        for oid in ids:
            if oid in seen:
                continue
            rows = proj.g.query(
            ...
            # name ends with `#{n}`. ALL matches link (session form).
