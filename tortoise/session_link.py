...
def link_session_entities(proj, session_id: str,
    ...
    # Per-point: FIRST-match only. turn_ids is generated by callers from
    # the same list as turn_texts, so the pairing is 1:1 — enforce it.
    if turn_ids is not None:
        for tid, text in zip(turn_ids, turn_texts, strict=True):
