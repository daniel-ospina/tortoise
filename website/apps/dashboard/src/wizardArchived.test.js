...
// #2779: the org-create validation copy moved to the shared org-naming
// module (main.jsx delegates to displayNameError), so the copy sweep reads it
// from there too.
const orgNamingSrc = readFileSync(join(__dirname, 'orgNaming.js'), 'utf8')
...
test('DE2E-2 copy sweep: org-create dialog + wizard copy say Organization', () => {
  ...
  // #2779: the validation message is generated by the shared org-naming
  // module (main.jsx has no inline copy left).
  assert.ok(orgNamingSrc.includes('Organization name is required'),
    'validation error copy (shared org-naming module)')
  assert.ok(src.includes('Organization name'), 'org-create field label')
