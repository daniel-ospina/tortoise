...
function claimIntentInFlight() {
  ...
  async function loadBranches(repo) {
    ...
    // #3687: the pin MUST be function-scoped, not try-scoped. The `catch` reads
    // it, so a declaration INSIDE the `try` is a ReferenceError on every failed
    // branch load instead of the intended best-effort empty-list path. Mirrors
    // loadRepos above (same one-line fix from the parked lane, ported by hand).
    // #1893 (code-review P1): pin the SELECTED team (see refreshOnboarding).
