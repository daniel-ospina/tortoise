...
GitHub index path has **no quota gate** (`_run_indexing` → `index_issues` → `create_point`; zero `count_org_usage` calls, unlike /v1/sessions). Default max_points=1000; `_MAX_ITEMS_PER_RUN=500` is **per repo** — a 20-repo org writes up to 10,000 Points on onboarding day → default cap blown by auto-index-after-connect → every subsequent /v1/sessions capture fails 402 — the feature the wizard just promised is dead on arrival. Separately: hosted capture leg unproven (P1); extraction 503s without provider key; wizard can't detect either condition.
