...
| **Embedding/similarity candidates over `:Object` (the vector leg)** | ⚠️ **CONDITIONAL — `STORAGE-ARCHITECTURE.md` §12.2c + `#4997` / V1** | **NEEDS A REOPEN of V1 — not yet adoptable** | Entity vectors would index **7,859 strings that are 62.3 % low-information** (the storage doc's own measurement) — a cost on the dominant line. And all 7,859 Object embeddings **already exist but are unindexed** (`#4997`), so the mechanism is *present and paid for*; **V1 decides whether it stays.** Using it for candidate retrieval (a brute-forced 9.26 ms scan) is a **consequence of the V1 decision**, not a separate adoption. |
...
**Adoption gate (one line, per the template):** **adopt** the scoped-candidate + Jev-SELECT + placeholder-quarantine + entity-review-queue findings (no recorded decision reached, convergent, aligned); **reopen V1** for the entity-vector leg (a decided-but-unimplemented cost question, and a different home); **refuse** the scope-first *identity key* (contradicts #3584) and any replacement of the resolution step (owner sequencing).
...
**Adoption gate line:** `Adoption gate: adopt (scoped candidates · Jev SELECT · placeholder quarantine · entity review queue) | reopen V1 (entity-vector leg) | refuse (scope-first identity key — contradicts #3584; replacing S3 — owner sequencing)`.
