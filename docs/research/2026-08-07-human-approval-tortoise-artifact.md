...
- **Supersession:** `supersede_point(old, new)` (`sdk.py:630`) — CORRECTS edge, marks outdated, transfers edges. Available for approval revocation.
...
- **Mechanism:** approval Point -[:IMPL {direction: unidirectional, label: "approvedBy"}]-> each approved claim Point. Unidirectional direction is critical — directional IMPL is the resolved *stance* (back-message information loss is semantically correct), but note `create_operator`'s code default is `direction="bidirectional"` (sdk.py:719), so the flag must be passed **explicitly**; an omitted flag silently enables bidirectional back-message propagation — the exact amplification the stance warns against.
...
3. **IMPL fan-out** — approval Point → (unidirectional IMPL) → each approved claim Point, then **EP with evidence prior** on the approval Point. Confidence lifts propagate to dependents B, C, D (RQ3). Unidirectional is required and must be passed explicitly (`create_operator` defaults to `bidirectional` — sdk.py:719).
...
5. **Reputation guard (required)** — `compute_reputation` (sdk.py:3606) scores Beta(1+impl_count, 1+nand_count) — it counts Event→Point IMPL/NAND edges per subject as success/failure (target Point confidence is read only into the returned outcomes payload, not the score). Two risks: (a) if approval Events ever get Event→Point IMPL edges (the §3.8 producer), each approval would increment impl_count and inflate the approver's reputation from their own approvals; (b) today NO code creates Event→Point IMPL edges — the pattern's Point→Point fan-out means the reputation query wouldn't match approval events at all, so the guard is defensive against the §3.8 gap being filled later. **Design must exclude `eventKind: humanApproval` from reputation outcome aggregation** either way (or track approval *quality* via downstream falsification — deferred, out of scope).
