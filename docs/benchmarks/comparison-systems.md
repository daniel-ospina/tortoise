...
| `trigger_population` ⚠️ **amended 2026-09-12 (#3327)** | the retrieval-capable comparators `{a1, a2, a2b, a3, a4}` — contract text; **no constant yet** in `matcher.py` (which holds `TOP_K`, `F1_TOLERANCE`, `SUBSET_FLOOR`); wiring tracked by #3327 — ⚠️ **corrected 2026-09-13, #3346: the "no constant yet" clause above is stale/false on current `main` and is preserved verbatim as the pre-wiring record. PR #3439 (#3327) landed `TRIGGER_POPULATION` at `battery/recall/matcher.py:49` and made it the default of `match_recall`'s keyword-only `population` parameter (`matcher.py:206`); `battery/recall/prepass.py` / `battery/runner/run.py` wire it into the run path. The contract value is unchanged — `trigger_population` is still `{a1, a2, a2b, a3, a4}`.** | the arms the divergent trigger is defined over. `a0` (no-memory control) is **excluded** — **not a participant**: it is retained as the trigger's **positive control** and as a published profile row annotated *recall-confounded*. Reason: a0's recall is 0.0 by construction, so an a0-inclusive trigger fires on every run and makes every differential run INCONCLUSIVE |
...
| 2026-09-13 | **Corrected the §3.2.1 `trigger_population` row's "no constant yet" clause** (annotated, not rewritten — the original clause stays in the cell with a dated inline correction). | PR #3439 (#3327) landed `TRIGGER_POPULATION` (`battery/recall/matcher.py:49`) and made it the default of `match_recall`'s keyword-only `population` parameter (`matcher.py:206`), so the row's parenthetical — "**no constant yet** in `matcher.py` (which holds `TOP_K`, `F1_TOLERANCE`, `SUBSET_FLOOR`)" — was left false. The same stale claim (added by #3345, 2026-09-12) was corrected in `docs/agent-reasoning-eval-battery.md`. Scope note: **wiring-status correction only — no contract value changed** (`trigger_population` is still `{a1, a2, a2b, a3, a4}`); the population remains caller-overridable via the `population` parameter, so the a0 positive-control reading is unaffected. | Additive correction; **no published number changed**; no row filled; no run executed. |
