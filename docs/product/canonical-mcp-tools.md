...
| **`tortoise_get_entity` must NOT be retired** by the #3883 warning shim. PR #4031's `RETIRED_USE_INSTEAD` currently retires it (`"tortoise_get_entity": 'tortoise_get(id, type="entity")'`), which is the **opposite** direction from this list — and because `list_tools` strips retired names *by string*, a re-declared live `get_entity` would be hidden from agents entirely. The map must instead retire `tortoise_get` → `tortoise_get_entity(id, type=...)`. | **must-fix before cutover** |
