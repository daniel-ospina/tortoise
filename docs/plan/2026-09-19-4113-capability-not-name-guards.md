...
| Set | Derivation |
|---|---|
| `sdk_graph_mutators()` | `TortoiseSDK` methods whose body (transitively through `self.<m>()`, module-level helpers, `proj.apply`/`proj.create_*`/`proj.set_*`/… write helpers) contains a mutating Cypher clause (`CREATE\|MERGE\|SET\|DELETE\|REMOVE\|DROP\|DETACH DELETE`). Docstrings excluded. Visited-set traversal. |
...
6. **Non-vacuity + falsifiability.** Every derived set has an anchor-in/anchor-out sentinel and an over-inclusion sentinel; each predicate has a synthetic-violation test that must fail it. Synthetic probes are injected with `monkeypatch.setitem(mcp_server.__dict__, probe_name, probe_fn)` (fixture-scoped teardown — no leak into `register_all`'s `globals()` map) and each derivation is fed a synthetic source/AST. Classes: **T1** read-labelled tool whose handler reaches `create_point` — value-passed, alias, **and table-dispatch** forms; **T2** HTTP tool whose handler reaches `ingest_corpus` (filesystem leg) **and** `org_create` (control-plane leg), plus a body-guard-less `http_policy=False` probe whose handler reaches `create_point` (must FAIL — the HTTP exemption is conditional on self-guard); **T3** stale `sdk_method`; **T4** `_quota_gated(fn)` dynamic site and a per-site weighted/unweighted duplicate.
...
- **Cross-test pollution from probe injection** → `monkeypatch.setitem` + fixture teardown; derived-set caches reset per test.
...
**Acceptance:** each T1–T4 test constructs its synthetic violation (probe handlers via `monkeypatch.setitem(mcp_server.__dict__, …)`, synthetic sources for the AST derivations) and asserts the predicate reports the failure; sentinels assert each derived set is non-empty and excludes a known read; a rename-simulation asserts guard 4 fails loudly.
