...
3. Every **removal** path requires exactly that predicate: kill-path cleanup (`:1619`), `_remove_stale_socket_dir` Guard 1 (`:1682`), `_sweep_quarantine_dirs` (`:1983`).
...
> **OVERRIDES:** the unconditional full-tempdir socket-dir walk recorded by **#1642 FIX 2** (`embedded_reaper.py:157-165`, "pollution cannot disable the ONLY path that cleans killed-suite residue") — pass-2 **discovery** is now scoped to the ephemeral namespace. Reason: for depth-1 candidates that namespace is *exactly* the set every **REMOVAL** path already requires (`_is_ephemeral_dir`; the KILL verb is gated by classification + the pass-1 lemma that live servers are enumerated name-independently), so scoping loses no removable or killable record while removing the 224k-entry metadata storm. The recorded intent ("never gated on the tempdir's entry count") is preserved — the new gate is a **name** scope, not an entry-count gate — and `--full-scan` restores the pre-#4068 un-scoped enumeration.
