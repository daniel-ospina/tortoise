...
- **A5 — the census is already past its own timeout (latent correctness bug).** `SOCKET_WALK_TIMEOUT = 20.0` (`embedded_reaper.py:165`) is below the measured 16–24.6 s walk, and the `TimeoutExpired` path (`:1138-1144`) returns `[]` with only a warning — under load **pass 2 silently discovers nothing**. Not a scope bug; the scope fix makes it moot for pass 2 but the silent-`[]` pattern must not be re-introduced by the replacement.

### Contract answer (the issue's "Ask")

The global walk is **not redundant as a mechanism**. `discover()` pass 1 (`_pgrep_redis_servers` `:947` + `_socket_dir_from_cmdline` `:969`) finds only servers with a **live** process, because the dir is derived from the process's argv / `redis.config`. Pass 2 is the only path to a **dead** server's residue — there is no process to pgrep, hence no cmdline to parse. Only the walk's **scope** is redundant: redislite always creates the socket dir with a bare `tempfile.mkdtemp()`.
...
**Falsifier:** the framing dies if any production redislite socket directory has a basename outside the ephemeral namespace. Redislite creates every socket dir via `tempfile.mkdtemp()` with no prefix argument (`redislite/client.py`), and no production caller passes `redis_dir`/`socket_file` (only `getattr(inner, "redis_dir", None)` reads exist — `embedded_lifecycle.py:151-152`). If it ever did, the `--full-scan` hatch (below) restores detection.
...
- **Cycle 1 — P1:** "the lossless claim is asserted, not demonstrated." Verified independently that `_classify` does **not** consult the dirname when a registry carrying `dbfilename == "redis.db"` is present (falls straight to `_cooldown_check`, `:1238-1360`), so an arbitrarily-named dir **is** reap-classifiable in principle. → Fixed: claim downgraded, codebase namespace used, explicit hatch added, naming scope recorded as an intended deviation.
- **Cycle 2 — P1:** "the test-impact analysis is asserted, not demonstrated." (`tests/test_reaper.py:269` `my-custom-name`.) → Fixed: full enumeration of every `discover()` call site and fabricated fixture (including `tests/test_embedded_concurrency.py`) — exactly one test changes.
- **Cycle 2 — P2 (accepted):** "the escape hatch is discovery-only; every destruction path independently gates on `_is_ephemeral_dir`, so an out-of-contract dir can never be removed." → Fixed by replacing the convergence claim with a **positive proof**, below.
...
1. `_is_ephemeral_dir(dir, tmpdir)` = `any(part.startswith(EPHEMERAL_PREFIXES) for part in Path(dir).relative_to(tmpdir).parts)` (`:168-190`).
...
3. Every destruction path requires exactly that predicate: kill-path cleanup (`:1619`), `_remove_stale_socket_dir` Guard 1 (`:1682`), `_sweep_quarantine_dirs` (`:1983`).
4. ⟹ a pass-2 candidate outside the namespace is **never actionable**; narrowing discovery to the namespace loses **zero actionable records** and loses only *reporting*, which the detect-only hatch restores.
5. **Lemma (for live candidates):** `classification == "candidate"` requires a live pid, and live servers are independently discovered by pass 1 regardless of dir name.
...
| D | Single unified tri-state census; quarantines pre-censused | rejected on a functional bug: `_run_sweep` calls `discover()` **before** `reap()` creates quarantines by rename-aside (`:2058` vs `:2078`), so a census inside `discover()` cannot see same-sweep quarantines; also changes `discover()`'s return type |
...
|---|---|---|
| 1 | A depth-1 **symlink** with an ephemeral-prefixed name pointing outside the tempdir, holding a decoy `redis.socket`/`redis.pid` for a dead pid | **Eliminated by construction:** `_iter_candidate_dirs` skips symlinked entries, exactly matching `find` without `-L` (which does not descend a symlinked dir) — so the new walk introduces **no** new discovery surface. Belt-and-braces: if such a dir reaches `reap()` as a crafted record, the destruction chain realpaths and re-verifies containment and performs zero actions |
...
| 4 | `full_scan=True` used to attempt destruction of a non-ephemeral dir | `full_scan` broadens **enumeration only**; classification yields `protected`; zero actions |
...
> **OVERRIDES:** the unconditional full-tempdir socket-dir walk recorded by **#1642 FIX 2** (`embedded_reaper.py:157-165`, "pollution cannot disable the ONLY path that cleans killed-suite residue") — pass-2 **discovery** is now scoped to the ephemeral namespace. Reason: for depth-1 candidates that namespace is *exactly* the set every destruction path already requires (`_is_ephemeral_dir`), so scoping loses no actionable record while removing the 224k-entry metadata storm; the recorded intent ("never gated on the tempdir's entry count") is preserved — the new gate is a **name** scope, not an entry-count gate, and an explicit detect-only `--full-scan` restores unscoped reporting.
