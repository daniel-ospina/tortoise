...
2. **Hydrate phase (parallel, in background):** mint + completeLogin's loads run concurrently and hydrate the cards as they arrive:
   ...
   - The team switcher + stale-closure guards (`orgIdRef`, `_teamAtCall`) must not regress: hydrate functions already re-check `orgIdRef.current` before landing data.
...
- Team switcher while hydrating: existing `_teamAtCall`/`orgIdRef` staleness guards are preserved (they already prevent landing stale data).
...
- **P1 (team-switch clobber):** the mount continuation's own writes (`localStorage.setItem(KEY_STORAGE, key)`, `setApiKey`, `apiKeyRef`, `setAuthMode`, `completeLogin`'s `setTeam`) can clobber a switch made while chrome is visible (multi-membership: mint-400 fallback populates teams early). Fix: after the mint capture `mintedOrgId`; before each state write and before `completeLogin`, bail if `orgIdRef.current` is set AND differs from `mintedOrgId` (the round-N staleness pattern).
- **P2 (null-capture race):** on a fresh session `orgIdRef.current` is null when the hydrate fires; `loadAll`'s `_teamAtCall` capture can race `loadTeams`' Round-8. The mint-time `mintedOrgId` from the P1 fix makes the bootstrap hydrate deterministic.
