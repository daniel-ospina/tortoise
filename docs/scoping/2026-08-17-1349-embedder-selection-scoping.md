...
- **One atomic deliverable, no MECE child-issue decomposition.** The issue-workflow escalation clause ("wiring or E2E → project-workflow") is not triggered: the 16-row wiring table is the integration-surface ACCOUNTING every task produces, not a decomposition; the E2E-8 check is a latency verification of one code path, not an E2E test-suite deliverable; there are no child issues — the two-PR structure is commit granularity, not issue decomposition.
