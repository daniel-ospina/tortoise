...
- **Multi-winner tie-break:** if >1 candidate clears (a)+(b), land argmax aggregate turn_recall@10; ties broken by lower E2E-8 latency, then smaller image size, then family-preserving (arctic-xs vs MiniLM encoder-space proximity — hypothesis-level, final tiebreak only).
...
- **T8 evidence production** — real-MiniLM Docker baseline (supersedes synthetic `baseline-embedded-2026-08-17.json`), LongMemEval-S **6 model-config runs** (MiniLM, bge-small, arctic-xs no-prefix, arctic-xs vendor, arctic-s no-prefix, arctic-s vendor — arctic both-config UNCONDITIONAL), hard tier × 6 configs, mini-BEIR × 6 configs, per-encode + E2E-8 (via T3 `--model` — **pre-swap E2E-8-with-candidate is a HARD PR2 pre-condition**, not a post-sunk discovery), labeled-pair calibration × 4 models. **Test unit = MODEL (3 families), not config** — the 3 pairwise BH tests are per model-family; within-model config choice (max of the 2 arctic configs) is a pre-registered selection rule, and its mild selection effect is acknowledged. **Wall-clock pre-registered:** 500Q × ~115k-token haystacks × 4 models ≈ 8-30h on the 2GB VM; cross-question encode caching (overlapping haystack content is re-ingested per question — cache encodes); slow-model subset fallback (`--limit`) with gate power stated for that n; artifacts + `gate_1349` verdict committed.
