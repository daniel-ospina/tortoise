...
- **Source:** LongMemEval-S split (HuggingFace `xiaowu0161/longmemeval-cleaned` — see note), via `tools/longmem_eval` (the ONLY harness with the temporal / multi-session / single-session / knowledge-update categories; dataset.py docstring: `single-session-user|single-session-assistant|single-session-preference|temporal-reasoning|knowledge-update|multi-session`). **Category set for the primary metric: {temporal-reasoning, multi-session, single-session, knowledge-update}** (all S categories except the synthetic-format ones; per-category breakdowns reported secondarily). The #1144 oracle/authored query sets have NO such categories (oracle = tiers easy/medium/hard; authored = domains) — "sampled per the eval's existing oracle/authored query structure" is superseded by this pin. (Verify exact HF repo id at execution — `xiaowu0162/longmemeval-cleaned` per dataset.py:36.)
- **Retrieval mode:** **vector-only arm** — NEW `vector_search()` in `tools/longmem_eval/retrieve.py` that encodes the query via `EmbeddingModel` and calls `run_vector_query(proj.g, vec, ...)` directly (via `sdk._get_proj()`, already used at retrieve.py:101) + the existing annotation path. **No `tortoise_fts_query` involvement** (it is hybrid RRF-only with no vector-only mode; a fusion flag does NOT exist). Matches the MemDelta "pure embedding swap" premise. Note: in embedded CI mode the vector arm is a no-op (no model) — only meaningful in Docker mode.
- **Recall variant:** **turn_recall@10** as the primary metric (per-query gold-evidence turn in top-10 retrieved turns); session_recall@10 and nDCG@10/P@5 reported as secondary. Per-question export exists (run.py ~:261) — required for paired deltas.
...
- **Win criterion:** candidate wins iff (a) aggregate mean turn_recall@10 beats real-MiniLM control by **≥ +5% relative** (delta relative to the control mean, aggregate over the full S-split category set — NOT per-query relative, which is undefined at baseline R@10=0) AND (b) BH-FDR at q=0.10 rejects the pairwise test (p ≤ threshold). Paired 90% CI on the delta excluding 0 is **reported as evidence but is NOT an additional gate** — the win gate is (a) AND (b) only.
...
**Encode protocol (pre-committed, cycle-3 corrected):** uniform NO-prefix protocol across all four candidates, queries and documents encoded through the same plain pipeline (matching production's single-model symmetric cross-lens + asymmetric search use). **Documented arctic penalty:** Snowflake arctic-embed is trained with a query-side instruction (`prompt_name="query"` — vendor-recommended for retrieval); the uniform no-prefix protocol measures arctic BELOW its vendor config. This is accepted for benchmark fairness (identical conditions for all candidates + production parity). **Re-validation step:** if arctic-xs/arctic-s wins the no-prefix benchmark, re-validate the winner in vendor config (query-side prefix via `prompt_name="query"`, documents plain) before landing the swap — the swap lands in the config that measured best; the cross-lens path stays plain (same single model, prefix applied query-side in the search path only, mirroring mem0's query/doc prefix tagging).

**Outcomes:**
- **Insufficient power:** if the real-model baseline leaves no headroom (near-ceiling R@10 on the LongMemEval subset) or CIs straddle +5% → **keep MiniLM, verdict: "needs real data"** — closes cleanly, no swap. **Degenerate baseline:** if control mean turn_recall@10 ≈ 0, the relative-delta win criterion is ill-defined (division by ~0) → treat as insufficient-power / needs-real-data, and fall back to an absolute-delta judgment only if a candidate clears a meaningful absolute R@10.
...
2. Candidate benchmark: MiniLM (control) + arctic-xs + arctic-s + bge-small-en-v1.5 on mini-BEIR (**pinned: MS MARCO dev + NFCorpus + SciFact + FiQA** — small, stdlib-fetchable, low-contamination) + in-repo hard tier + LongMemEval-relevant turn_recall@10 (vector-only arm) — via the LongMemEval-S runner (tools/longmem_eval) + the #1144 eval metrics. LongMemEval gate = retrieval recall (gold evidence in top-k), NOT the expensive end-to-end judged runner (that's post-swap for the winner only). mini-BEIR is a research surface, NOT a gate — the decision rule gates only on turn_recall@10 + E2E-8.
...
5. Conditional swap implementation (only if gates clear): embeddings.py model swap (384-dim), Dockerfile.hosted re-bake + entrypoint.sh cache path + hosted_api.py pre-warm, force-re-embed flag on backfill_embeddings.py (currently NULL-only), single Point.embedding index drop+recreate, threshold recalibration (**labeled-pair set = net-new named work**: commit a #399-style labeled near-dup/paraphrase/related/unrelated pair set — the repo has no committed real labeled set today, only inline test fixtures — re-score under the winner, sweep protocol per research), per-encode + E2E-8 latency measurement, decision record (model choice + location answer + threshold values).
