...
## 8. Slice A status — MERGED (#2687) + review round (2026-09-09)

Slice A landed on main as #2687 (commit 2935e17a7 → squash b3ed420d9):
`package_evidence_pool` in tortoise/retrieval.py (two-pass collapse of a
point + its own source chunks/turns into ONE slot, cross-item near-dupe
dedup, value/verbatim-first ordering), tri-state eval arm
`TORTOISE_LME_EVIDENCE_ASSEMBLY` (fail-safe OFF) + ask-lane
`TORTOISE_ASK_EVIDENCE_ASSEMBLY`. 23 hermetic tests + adversarial
...
- **P1 (value-safety violation):** the RATIO-based collapse merged DIFFERENT
  ...
  tests passed while realistic quotes crossed the 0.9/0.75 thresholds.
  **Fix:** a fact-critical differing-token guard (`_pkg_differ_value_critical`)
  — collapse refuses when the differing tokens contain a NUMBER / currency
  amount / unit-quantity word / negation, regardless of shared content. The
  ratio legs now only merge synonym-level restatement. The same-turn quote leg
  keeps its own guard and is no longer blocked by the content guard.
- **P2a:** currency symbols are content (`£300` ≠ `$300` ≠ `300`) — the
  punctuation strip preserves `$ £ € ¥`.
- **P2b:** negation is content — `not`/`no` removed from the package stopwords.
- **P2c:** eval outcome keys gated under the arm (off-path dict keeps the
...
mechanism that surfaces any verbatim source ref (≤1 per collapsed point), so
Run C is the decisive test of whether the number-failures are extractor-side
(value lost in distillation) or reader-side (value present, mis-handled).
A forensics script (`tools/longmem_eval/sliceb_forensics.py`) classifies
exactly where each gold value lives (point content / point quote / chunk /
...
- **Run C (arms ON + assembly ON)** has a DIFFERENT fp → needs its OWN full
  ingest pass (cache-B, arms-ON fp), a second multi-hour pass.

Status 2026-09-10: Run A in flight on `falkordb-eval:6380` at sha b3ed420d9
(`--cache-ingest`), 37/50 question graphs ingested + cache-markers verified
arming. The one-time pass ran far over the ~4.5 h estimate because the shared
box was at load 90–229 (47 concurrent pi sessions; a runaway `find`, `npm ci`).
Runs B/C remain, pending Run A completion.
