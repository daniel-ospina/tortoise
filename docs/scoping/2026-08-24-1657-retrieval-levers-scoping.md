...
**Control discipline (R6 precedent):** every lever OFF by default = byte-identical baseline path; per-question `legs` trace + `match_source_counts` already record leg provenance (M7 #1527) — the L4 leg-mix bucket answers "which leg found what" per lever run. `--mock --skip-preflight` CI smoke stays runnable; **every run that does not waive the dense leg** refuses to start without the embedder (R3 pre-flight — #4718 replaced the original "real runs" scoping: `--mock` alone is no longer a waiver). mini-BEIR runs are advisory, not gates (per its own README).
