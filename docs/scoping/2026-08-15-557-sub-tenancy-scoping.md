...
- **S3 — Metering into embedder's bill:** extend `metering.py` to record per end-user team (existing `(org_id, period)` key works unchanged) and roll totals up to the embedder's account for invoicing/overage (open Q5 granularity). Embedder sees aggregated usage; end-users see nothing.
...
- **Isolation boundary:** the tenant graph namespace (`org_{org_id}`) is structural (reuses #318's LANDED boundary; no tenant selector on any surface) — an embedder's end-user cannot read another end-user's graph, and cross-embedder isolation is the same boundary (graphs live under each embedder's namespace tree).
