...
`tortoise.premiselabs.co` (landing / legal, Cloudflare Pages),
...
(the session-bearing origin, the `tortoise-dashboard` Pages project — #4054 moved
the BFF there, so `tortoise.premiselabs.co/auth/start` answers **404 by design**).
...
| `PROBE_HOST_LABEL` | `app.premiselabs.co` | The incident **title is the dedupe key**. Two production targets must not share one label or they would fight over a single issue |
