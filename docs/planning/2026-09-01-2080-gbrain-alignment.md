...
> **2026-09-01 (Daniel): post-ship conversion gate REMOVED — considered pointless.** The internal falsification indicators above remain the honest measure. The epic's value is internally testable (baseline publishes, CI can fail, fix-wave improves, why-suite conflict-surfacing); no 6-month external threshold needed.
...
**Scope-cut recommendation (superseded 2026-09-01 — Daniel: multi-graph per team is already in the works, ready ≤48h):** the original recommendation to conditionally cut the W4 phase-1 `/v1/context` endpoint if #2083's registry schema slipped >2 weeks is **REMOVED**. #2083 is in motion with a 48h-class delivery horizon, so there is no standing slip risk — build phase-1 `/v1/context` against the per-graph-key tenancy contract directly (joint contract co-design with #2083's schema work stands). The sequence note remains: in-place enrichment of search/analyze first, then the endpoint.
...
**Verdict after fix round:** all P1/P2 applied — decision substance unchanged (APPROVE + W2/W3-first gate + W4 user-exposure gate). The conditional endpoint cut and the conversion checkpoint were later REMOVED by Daniel (2026-09-01): multi-graph is in the works ≤48h (no slip risk), and the 6-month conversion gate was pointless (internal falsification indicators retained).
