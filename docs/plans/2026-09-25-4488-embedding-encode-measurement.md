...
**Architecture:** A leaf module `tortoise/embed_metering.py` owns a mutable `EmbedTally` in a `ContextVar`. `embeddings.compute_embeddings` (the store-vector encode funnel) mutates it in place (O(1), no I/O). Work-owning boundaries take-and-reset it into the existing ledger via `metering.record_embedding_usage`: a pure-ASGI `EmbedMeteringMiddleware` (arms non-GET requests; resolves the org from `scope["state"]["org_id"]`, which the auth dependency already populates; flushes in `finally`), the hosted `_record_write_op(org)` (early, idempotent), MCP `_quota_gated` (org from `_current_org_id`), and — for work born in a detached task — a `meted(org_id)` CM that installs a FRESH tally and flushes on exit (background index jobs, the dream worker, the hosted capture body). Supabase gets additive `embed_*` columns + one atomic RPC; the registry lane gets the matching Cypher MERGE. `/v1/team` renders the reader.
...
1. **Step:** an org writes Points (store encode) → **Acceptance:** the ledger row carries texts/chars/wall_ms/model/revision → **Test:** `test_create_point_records_embedding_work`
2. **Step:** `GET /v1/team` → **Acceptance:** the `embed_*` fields render the figure (zeros for a fresh org) → **Test:** `test_team_response_renders_embedding_fields`

### Failure Modes
- Metering window unresolvable → **Expected:** dropped + alerted via `metering.report_unmetered_increment(lane="embed", …)`; the write is served → **Test:** `test_unresolvable_window_does_not_raise_out_of_flush`
...
- Model unavailable → **Expected:** no texts/chars/time; `embed_skipped` increments; the stored identity is NOT erased and the mixed flag is NOT flipped → **Test:** `test_skipped_only_flush_preserves_identity`
- TF-IDF fallback → **Expected:** not counted (the hook is inside the model branch) → **Test:** `test_tfidf_fallback_is_not_counted`
- Two flush boundaries in one request → **Expected:** exactly one increment → **Test:** `test_second_flush_is_a_no_op`
- Off-loop worker encode → **Expected:** lands in the tally the owning boundary flushes → **Test:** `test_note_lands_from_submit_off_loop`
- Mid-period model OR revision change → **Expected:** `embed_identity_mixed` latches TRUE and STAYS TRUE → **Test:** `test_identity_mixed_is_sticky_and_paired`
- **Cancellation residual (declared, not claimed):** a capture request cancelled while its pool worker still runs flushes at the request boundary; the worker's later `note_encode` lands in a retired tally and is **not** attributed. Same class as the #4451 on-loop residual; bounded, documented in the PR body → **Test:** `test_cancelled_capture_residual_is_documented` (asserts the awaited path records once; the cancelled path is a declared loss, not a silent claim).
...
- One `threading.Lock` guards **only** the tally snapshot/reset; `record_embedding_usage` is called **after** the lock is released (a blocking ledger RPC must not serialize concurrent encodes).
- `arm()` is idempotent. `meted(org_id)` installs a **FRESH** tally (never inherits) and **flushes on exit** (`__exit__` for sync callers — MCP `_quota_gated`, the pool worker; `__aexit__` offloading the blocking flush for async callers — the capture body, `_run_indexing`, `_run_docs_indexing`, `_dream_worker`); the prior `_ACTIVE` is restored on exit.
...
- Org sources: the middleware reads `scope["state"]["org_id"]`, which the **key-auth** dependencies set (`hosted_api.py:3809`, `:3927`). **The session lane (`_session_user_org`, `:4056-4185`) and the internal lane (`_check_internal`, `:2748`) set NOTHING**, so `/v1/onboarding/seed` (`:21912`, session-authed) and `/internal/starter-seed` (`:22077`, org from the request body) are **NOT** covered by scope state and must be bound explicitly — done by decorating the RUNNERS (`_run_onboarding_seed`, `_run_starter_seed`) with `_embed_metered`, which also covers the MCP caller that reaches them directly. Without that, every tenant provisioning fires a spurious UNMETERED-INCREMENT incident on a resolvable org. MCP `_quota_gated` resolves `_current_org_id`.
