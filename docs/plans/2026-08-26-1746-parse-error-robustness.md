...
**Acceptance:** `DeepSeekDirectModel.complete` sends `response_format={"type":"json_object"}` when `TORTOISE_JSON_MODE=1` (default) and omits it when `=0`; `probe_json_mode.py` CLI produces a verdict JSON applying the D6 rules (both-zero → inconclusive pinned in `test_probe_verdict_logic`); the live probe selects the adapter per the D6 rule and the @slow live test skips when the direct-path key/provider env is absent; a dry-run mode is unit-testable.
**Files:**
