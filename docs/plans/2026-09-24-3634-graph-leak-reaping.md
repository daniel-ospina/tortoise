...
Unjournalled legacy `org_*` (104) and the graph named `t` — reachable only via the existing `_sweep_team_strays` opt-in (note: *journalled* `org_*`/`team_*` **are** owned and dropped by the default journal path; only the journal-blind residue needs the gate). LC5 (the `from_uri`/`host=` redirect gap). The atomicity residual #3214. The product storage architecture #4333. LC4/LC7 — filed as a follow-up in Task 6.
