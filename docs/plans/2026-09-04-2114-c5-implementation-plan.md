...
4. Session-auth teams (graph_id NULL) → `ns = graph_namespace or org_{id}` → the default graph — **byte-identical to today** (E2E-5 exit gate).

Replacement discipline: every data-plane endpoint that opens the team graph (`namespace=team["org_id"]` / `namespace=org_id` where `org_id` is the authed team) switches to `_data_sdk(team)`. Control-plane `namespace="registry"` sites and provisioning internals are untouched. Grep-based inventory + per-surface audit in Task 2.
...
dream/backup/retention/consistency/fallback_snapshot loops that call `_make_sdk(namespace=org_id)` per TEAM must iterate the team's graphs: the default (team-wide, today's behavior) + every custom Graph (registry: Graph nodes by org_id; supabase: graphs rows). Per-graph SDKs open via the D-C5-1 seam. Scope: backup_sweep's manifest/restore + the dream/consistency worker loops; quota metering stays team-pooled (D-C5-6).

### D-C5-6 — Quota unchanged surface

metering.py per-team `write_ops` pool: ops from ANY of the team's graphs count once against the team pool (no double-count, no bypass — the metering call site passes org_id, which the spine already resolves). Graph-count caps (`max_graphs`) enforced at provisioning (C2) — untouched. Only test coverage added: a per-graph op counts exactly once.
