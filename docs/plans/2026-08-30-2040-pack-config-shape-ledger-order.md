...
| API endpoint | `POST /v1/organizations/{org_id}/import` shape rejection | In | 422 + `quarantined_import` audit + `last_import_quarantined_sha256` stamped + live graph untouched (`ids == []`) + `last_import_sha256` unstamped | Integration (`test_import_endpoint.py`, un-skipped no-`_seed_live_graph` pattern) |
...
| DB write (ledger props) | `last_import_sha256` / `last_import_quarantined_sha256` / `last_import_pack_failed_sha256` on Team node/row | Both | stamp-after-apply; clear-on-failure; allowlist `_IMPORT_LEDGER_PROPS` extended with the marker (review round 2) | Integration (FakeControlPlane PATCH observable via `fake.tables["teams"][0]`) |
...
**Step 3: Implement** — move `_stamp_import_prop(cp_source, org_id, "last_import_sha256", sha)` from before to AFTER the `_apply_import_pack_config` try/except (before the `team_import` audit; `cp_source` in scope), and immediately after it also clear the quarantine prop best-effort (`await asyncio.to_thread(_stamp_import_prop, cp_source, org_id, "last_import_quarantined_sha256", "")` wrapped in try/except → warning). **The success-path quarantine clear is REQUIRED**: `last_import_quarantined_sha256` is sticky (only `_quarantine_import` writes it, never cleared) — without the clear, a sha that was quarantined then succeeded would have BOTH props == sha, so the in-lock short-circuit's consultation would block `already` forever (every re-import re-swaps). In the `except ValueError` pack-failure block: quarantine (existing) + clear the success ledger (`await asyncio.to_thread(_stamp_import_prop, cp_source, org_id, "last_import_sha256", "")` wrapped in try/except → warning log) + raise 422. **Extend the in-lock idempotency short-circuit (~8173-8182)**: the `already` fast-path must ALSO require the POST-SWAP PACK-FAILURE MARKER (`last_import_pack_failed_sha256`) to be UNSET — a post-swap pack failure stamps the marker (in addition to the ledger clear), so the fast-path refuses `already` when the most recent attempt at this sha failed after the swap (the live graph holds this dump but the vocabulary may not be live). Code-review round-2 refinement (PR #2054): the plan's original `last_import_quarantined_sha256 != sha` consultation could NOT distinguish a pre-restore rejection of a DIFFERENT sha (graph untouched — a prior applied sha legitimately short-circuits) from a post-swap pack failure of THIS sha (already would be a lie); the dedicated marker (set ONLY by the post-swap 422/503 branches, cleared on success) restores the documented idempotent re-import contract for the pre-restore class while keeping the same-artifact retry + rollback paths convergent. The quarantine prop stays an AUDIT record (not consulted). (Success path unaffected given the quarantine + marker clears above.) Replace the `#2040` comment with the new ordering rationale; add the post-swap semantic carve-out line to `import_team`'s docstring.
...
- **Ledger consumers ("" sentinel contract):** `last_import_sha256` now means "fully applied incl. vocabulary"; `""` is the deliberate cleared sentinel. Consumers: the in-lock idempotency re-read (~8174 — `""` falsy-safe; the post-swap pack-failure marker (`last_import_pack_failed_sha256`, added by code-review round 2) is consulted instead of the quarantine prop, which is AUDIT-only), the `/v1/team` additive-tier read (`supabase_control.py:97-109` — fail-soft, renders empty cell), dashboard columns (`test_dashboard_login.py:451-467` — degrade-tested). No consumer treats `""` as a valid sha.
