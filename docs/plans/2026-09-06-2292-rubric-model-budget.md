...
class _FlipJudge(JudgeClient):
    """Judge that flips yes/no on prompt-order swap (fails AB+BA)."""
    def judge(self, rubric_id, item_id, prompt, temperature=0.0):
...
def test_declarative_position_bias_blocks_on_flip():
    spec = load_rubric_spec(CONFIG, "r2-coverage")
    pairs = [(f"evidence-{i}-a", f"evidence-{i}-b") for i in range(6)]
    ...
    assert not rec.passed and "position-bias" in rec.blocked_reason
...
def test_irt_leg_judges_anchored_renders_not_bare_probes():
    # RED for plan-review P1: the pairwise IRT loop hardcodes contentless
    # `probe {i}` prompts (gate.py) — judging garbage with no anchored item
    # render. Declarative mode must judge the REAL anchored renders passed
    # via irt_renders; assert the irt-* prompts carry anchored content, never
    # the bare probe string.
    spec = load_rubric_spec(CONFIG, "r2-coverage")
    renders = [f"evidence render {i}: the agent weighed a counter-argument "
               f"against its position and revised it." for i in range(6)]
    seen: list[str] = []
    ...
    validate_rubric("r2-coverage", render_prompt_text(spec), _Recording(),
                    pairs, ["yes"] * len(spec.items), ["yes"] * len(spec.items),
                    n_items=len(spec.items), irt_renders=renders,
                    vocabulary=DECLARATIVE_VOCAB)
    assert seen, "IRT leg produced no judge prompts"
    assert all("counter-argument" in p for p in seen), \
        "IRT prompts are contentless bare probes, not anchored item renders"
```
**Step 2.2** — Run → FAIL (vocabulary hardcoded; IRT at n=4 over contentless `probe {i}` prompts — `irt_renders` param missing; all_identical/label_flip stress texts pairwise-tie-shaped; normal-approx binomial).
**Step 2.3** — Implement: `DECLARATIVE_VOCAB` protocol (labels `("yes","no")`, irt yes label, stress spec, declarative prompt builders); `validate_rubric(..., vocabulary=..., n_items=None, irt_renders=None)` — pairwise mode keeps current behavior byte-identical (default vocabulary `better/worse/tie`, `irt_renders=None` ⇒ the legacy `probe {i}` loop — existing #1410 tests must not move); declarative mode runs the same four legs with vocabulary-parameterized prompts/stress + the gold-anchor agreement leg, and its IRT loop judges the anchored item renders from `irt_renders` (evidence-bundle renders injected by the CLI — `irt_renders[i]` replaces the `probe {i}` string; length ≥ n_items); declarative stress probes reworded for yes/no (all_identical = same-verdict-twice consistency on identical renders; label_flip = negated-label instruction still yields a non-degenerate verdict); `exact_binomial_p` (math.comb) for n ≤ 32 with the normal-approx fallback above it; IRT over anchored items (`_rasch_infit(verdicts, n_items)` with yes=1). JudgeClient `_mock_judge` yields yes/no deterministically in declarative mode (so hermetic gate tests stay reproducible); `_real_call` unchanged in this task (usage capture = Task 3).
...
def test_real_preflight_refuses_unpinned_or_fixed_sentinel(tmp_path, monkeypatch):
    ...
    # real-executor request over an arms.yaml whose pin is the placeholder
    # sentinel must ConfigError BEFORE attempt-dir creation.
    ...
    import shutil, yaml
    for f in ("arms.yaml", "budget.yaml", "thresholds.yaml"):
        shutil.copy(CONFIG / f, cfg_dir / f)
    cfg = run_mod.RunConfig(config_dir=cfg_dir, arms=["a4"], executor="real",
                            out_dir=tmp_path / "out")
    with pytest.raises(ConfigError):                 # placeholder pin refused
        run_mod.run_battery(cfg, stdout=lambda s: None)
    # arms.yaml re-locked to the concrete pin -> the pre-flight clears the
    # pin gate (and the existing Task-9 seam refusal is out of scope here)
    ...
```
...
def test_flip_flop_and_fp_rows_present():
    cfg = load_thresholds(CONFIG / "thresholds.yaml")
    metrics = {m for m, _ in _rows(cfg)}
    assert {"flip-flop-rate", "false-positive-rate"} <= metrics

def test_surfaced_rate_row_post_i1_semantics():
    cfg = load_thresholds(CONFIG / "thresholds.yaml")
    rows = _rows(cfg)
    assert rows[("surfaced-rate", "a0")] == 0.0    # no-store arm comparator intact
