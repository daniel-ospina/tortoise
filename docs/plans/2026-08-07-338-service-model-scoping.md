...
| **D4** | **README + docs service-first rewrite** — "Install → Connect → Query" (MongoDB Atlas pattern, onboarding epic #235 research). Quickstart: hosted signup (free tier) OR `docker run`/`docker compose` self-host; connect via `claude mcp add tortoise https://api.premiselabs.co/mcp` / `codex mcp add` / `.mcp.json` snippet; query = MCP tools (58) + REST (as #525 lands). `pip install` demoted to "SDK for local dev/scripting". | **Acceptance (behavioral):** no README/docs path instructs importing tortoise into a consumer distribution. **root `index.md`** (canonical index — also a license surface) updated to service-first ordering (MCP server → connectors → SDK); `docs/00_index.md` referenced if present (AGENTS.md routing index). |
...
| README/docs quickstart paths | Docs | T5.1 (self-host primary; hosted gated #518/#235) + T5.3b grep gate | ✅ |
