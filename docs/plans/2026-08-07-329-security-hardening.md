...
- `tortoise/quota.py` (stdlib-only module-level imports; function-level sdk import):
  ...
  - `resolve_org_limits(org_id) -> dict` reads the full Team node from the registry (`MATCH (t:Team {id:$id}) RETURN t.tier, t.max_points, t.max_api_keys, t.max_sessions`); **missing Team node → raise QuotaCheckError** (fail-closed, no silent defaults); defaults only for missing attributes: points 1000, api_keys 20, sessions 1000.
  - `enforce_org_limit(team_limits, resource, sdk=None)` — takes the limits dict (resolved ONCE by the caller: `get_current_org`/TeamResolutionMiddleware → `resolve_org_limits`, NEVER re-fetched per write); count via `MATCH (n) RETURN count(n)` on the team graph (or registry graph for api_keys); ANY counting exception → QuotaCheckError (fail-closed); count >= limit → QuotaExceededError. No org_id/limits → return cleanly (stdio/operator skip).
...
- **Relief path (P1):** `sdk.team_update` allowed-fields extended with `max_points`, `max_api_keys`, `max_sessions` (no control-plane HTTP surface exists today — `team_update` is SDK/registry-level; a REST control-plane surface is a follow-up). TTL test: limit change propagates within the documented 60s cache TTL (middleware gains an injectable `cache_ttl: float = 60.0` constructor param for tests). Relief test mechanism: unit-level via `quota.resolve_org_limits` (primary) + real-auth-path assertion that a raised `max_points` is honored by `get_current_org`.
- REST `hosted_api._check_team_limit` delegates to quota helper (same semantics, now fail-closed); `get_current_org` fetch extended to include max_points/max_api_keys/max_sessions so REST and MCP see IDENTICAL limits (route through `quota.resolve_org_limits` — one fetch).
- MCP: `TeamResolutionMiddleware` (mcp_auth.py) resolves the full team limits dict once (registry lookup via the same helper, cached 60s like the auth cache) and sets a `_current_org_limits` ContextVar alongside `_current_org_id`.
- **No team context (stdio/MCP without tt_ key):** `enforce_org_limit` returns cleanly (skip) — mirrors REST `if not org_id: return`. Batch caps remain unconditional.
...
**Acceptance:**
- `tortoise_list_graphs` over HTTP returns ONLY graphs owned by the calling team (`org_{org_id}` prefix + the team's own graph); stdio returns the full list (operator context).
- Decision: team-scope (not remove) — preserves the tool for stdio/operators and keeps HTTP functional with zero enumeration.

**Files:**
- Modify: `tortoise/mcp_server.py` (`tortoise_list_graphs` L645-648), `tortoise/sdk.py` (`list_graphs` L1206 — accept optional team scope filter; **exact `org_{id}` equality, not prefix matching**)
