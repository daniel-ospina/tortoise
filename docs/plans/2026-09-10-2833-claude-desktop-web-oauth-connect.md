...
**Step 1: Write the failing tests.**
   ...
   (c) **`parse_resource` table**: bare, trailing-slash, uppercase host, explicit `:443`, embedded fragment, **and team-scoped** (`…/mcp/organizations/<id>`, with the case/port variants) → each returns the documented `(canonical, org_id)`.
**Step 2: Run** → **FAIL**.
...
**Step 5: Apply the literal changes** on the connector surfaces listed above. **Never blanket-replace `premiselabs.co/mcp/`** — the team-scoped form `…/mcp/organizations/{org_id}` exists in `docs/oauth-mcp.md:64` and must not be flattened.
