...
| 12 | `tools/embedder_probe.py` | State | Both | Unit | monkeypatch injection; 384-dim assert; HARD FAIL on degrade; warm-process swap; HF revision pin | Warm-process no-op → wrong-model vectors under right cache key |
...
| 14 | `tests/fixtures/labeled_pairs.jsonl` | State | Read | Unit | ≤200 pairs, ≥30/band, #399 anchors | Band coverage gap → ungrounded dedup/checkpoint recalibration |
| 15 | `benchmarks/run_report.py` (:639 provenance literal) + `synthetic_corpus.py` (:29) | State | Read | Unit + Integration | provenance reads EMBEDDING_MODEL constant (**unit test: run_report provenance output equals EMBEDDING_MODEL, not the :639 literal — tests/bench/ additions**); E2E-8 ≤300ms p95 | Post-swap reports lie about embedder; E2E-8 measured post-sunk (must be pre-swap pre-condition) |
| 16 | `tests/test_embeddings.py:269` | State | Read | Unit | asserts the constant, not the literal | Breaks on rename |
| 17 | Docs (ADR-009, research dir, 00_index, embedding-retrieval.md) | External | Out | — | ADR-009 Pending→Accepted; per-label blast radius; tenant-visible changes | Decision record lost on negative gate (must ship in PR1) |
| 18 | #265 encrypted tier (pending-merge, NOT in code) | Future | — | — | 384-dim = chosen scope cut + escape clause | #265 lands non-384 before PR2 → pool reopens |
| 19 | Self-hosted (Dockerfile.selfhost) | External (deploy) | Out | — | No bake; lazy first-use download; TF-IDF degrade documented | Blocked-network operators get silent degrade (pin-old-image escape) |
...
- **Unit:** probe injection (test_embedder_probe.py), bootstrap p + BH-FDR + gate branches + manifest/precondition validation (test_gate_1349.py), nDCG@10 binary-gain + empty-graph/breaker/cache/checkpoint (test_vector_arm.py), backfill --force WHERE predicate + composition parity + meeting purge (test_backfill_embeddings_force.py), labeled-pair schema (test_labeled_pairs_schema.py), calibration bands incl. empty inputs (test_calibrate_thresholds.py), pair-label runner kappa + single-judge abort (test_pair_label_runner.py), mini-BEIR smoke on 10 queries + empty/corrupt inputs (test_mini_beir.py), ci_selection rglob/integrity (test_ci_selection.py additions), provenance-writer key redaction (tests/eval/retrieval/test_integration.py).
...
**Acceptance:** `inject_model(name, query_prompt=None)` monkeypatches the `SentenceTransformer` symbol in `sentence_transformers` BEFORE the first `EmbeddingModel.get()`, calls `_reset()`, asserts 384-dim + non-None (HARD FAIL, no silent TF-IDF degrade), records model_id; `reset()` restores; `PROBE_MODELS` registry maps minilm/arctic-xs/arctic-s/bge-small → HF ids **revision-pinned (`model_id@<commit>`); resolved revision recorded in provenance/manifest; same-revision asserted across the 6 config runs** (plan-review cycle-2 fix — HF tags are mutable). **Warm-process injection (plan-review P1 fix):** inject_model must genuinely swap even if `EmbeddingModel.get()` was already called in-process (verified via a discriminating-embedding check — cosine distance to a reference vector must differ) or HARD-FAIL; no pre-injection cache entries served under the new model key. **CI registration (plan-review P1 fix, cycle-3/4 corrected — coherent mechanics, complete halves fate):** this is a PR1-blocking concern, executed as ONE change: (a) **manifest keys become `tests/`-relative subdir forms** — `config/ci-surfaces.yml` entries use `longmem_eval/test_vector_arm.py`, `eval/retrieval/test_gate_1349.py`, `mini_beir/test_mini_beir.py` etc. (today the manifest is basename-keyed with zero "/" entries — `classify_test_file()` compares `f.name`); update `tools/ci_selection.py` `classify_test_file()`/`select()` to match on the `tests/`-relative path (select() already strips `tests/`) and emit the same relative form so `tests/<rel>` path construction works for the halves full-run path AND the PR-selection leg; (b) **reconcile ALL pre-existing subdir test files: register bench ×4 + eval/retrieval ×5 (10 files) in `config/ci-surfaces.yml`; `tests/e2e/` (17 files: 4 direct + 13 hosted) are deliberately NOT registered and `integrity()` rglob SKIPS the `tests/e2e/` prefix** (rationale: welcome-e2e-monitor + legal-e2e jobs + ENV_BROKEN_FILES — this is singular, not register-and-exempt); (c) **HALVES FATE — the critical piece (cycle-4 P1 fix): full runs execute ONLY `tests/$f.py` for `f` in the matrix halves lists; manifest registration feeds only the tier-2 PR leg, so manifest-registered-but-not-in-halves tests NEVER execute.** Enumerate the FULL halves extension set explicitly: `eval/retrieval/test_bootstrap.py`, `eval/retrieval/test_gate_1349.py`, `eval/retrieval/test_integration.py` (etc.), `longmem_eval/test_vector_arm.py`, `mini_beir/test_mini_beir.py`, `test_embedder_probe.py`, `test_labeled_pairs_schema.py`, `test_calibrate_thresholds.py`, `test_pair_label_runner.py` (PR1) and `test_backfill_embeddings_force.py` (PR2/T12) — with slow-gated ones in SLOW_FILES; (d) **SOURCE_PATTERNS entries for the new tool dirs** (`tools/longmem_eval/`, `tools/mini_beir/`, `tools/embedder_probe.py`, `tools/calibrate_thresholds.py`, `tools/pair_label_runner.py`) so tools-only PRs select the right tests (tools/ is in NON_PYTHON_PREFIXES today — a tools-only PR runs tier-1 smoke only); (e) **unit tests for the rglob behavior in `tests/test_ci_selection.py`** (subdir-unregistered → flagged, subdir-registered → clean, e2e-exempt → not flagged, tools-SOURCE_PATTERNS selection) — the existing `test_integrity_covers_all_test_files` asserts `integrity(load_manifest()) == []` and will fail without this; the integrity drift gate is unconditional on every event (#1262). Candidate-model tests skip-if-not-cached (HF_HUB_OFFLINE=1 in the test job env) with a **per-job zero-skip sentinel that FAILS only on cache-restore-succeeded-but-corrupt and WARNs on cache-miss-plus-download-failed (matching the pre-cache step's #1211 continue-on-error convention)**.
**Files:**
