...
**Acceptance:** `inject_model(name, query_prompt=None)` monkeypatches the `SentenceTransformer` symbol in `sentence_transformers` BEFORE the first `EmbeddingModel.get()`, calls `_reset()`, asserts 384-dim + non-None (HARD FAIL, no silent TF-IDF degrade), records model_id; `reset()` restores; `PROBE_MODELS` registry maps minilm/arctic-xs/arctic-s/bge-small → HF ids **revision-pinned (`model_id@<commit>`); resolved revision recorded in provenance/manifest; same-revision asserted across the 6 config runs** (plan-review cycle-2 fix — HF tags are mutable). **Warm-process injection (plan-review P1 fix):** inject_model must genuinely swap even if `EmbeddingModel.get()` was already called in-process (verified via a discriminating-embedding check — cosine distance to a reference vector must differ) or HARD-FAIL; no pre-injection cache entries served under the new model key. **CI registration (plan-review P1 fix, cycle-3 corrected — coherent mechanics):** this is a PR1-blocking concern, executed as ONE change that flips rglob on: (a) **manifest keys become `tests/`-relative subdir forms** — `config/ci-surfaces.yml` entries use `longmem_eval/test_vector_arm.py`, `eval/retrieval/test_gate_1349.py`, `mini_beir/test_mini_beir.py` etc. (today the manifest is basename-keyed with zero "/" entries — `classify_test_file()` compares `f.name`; subdir-prefixed registration alone would classify-never and flag everything); update `tools/ci_selection.py` `classify_test_file()`/`select()` to match on the `tests/`-relative path and emit the same relative form so the `tests/<rel>` path construction works for BOTH the halves full-run path (`tests/$f.py`) and the PR-selection leg; (b) **reconcile ALL pre-existing subdir test files in the same change**: enumerate + register the 25 currently-unregistered files (tests/bench ×4, tests/e2e ×3, tests/e2e/hosted ×13, tests/eval/retrieval ×5) in `config/ci-surfaces.yml` — with an explicit deliberate-exclusion exemption for `tests/e2e/` in `integrity()` (python-ci.yml header: browser tests covered by welcome-e2e-monitor + legal-e2e jobs) and a decision on their halves/SLOW_FILES fate (add bench + eval/retrieval to halves; keep e2e exempt); (c) **unit tests for the new rglob behavior in `tests/test_ci_selection.py`** (subdir-unregistered → flagged, subdir-registered → clean, e2e-exemption → not flagged) — the existing `test_integrity_covers_all_test_files` asserts `integrity(load_manifest()) == []` and will fail without this; the integrity drift gate is unconditional on every event (#1262). Candidate-model tests skip-if-not-cached (HF_HUB_OFFLINE=1 in the test job env) with a **per-job zero-skip sentinel that FAILS only on cache-restore-succeeded-but-corrupt and WARNs on cache-miss-plus-download-failed (matching the pre-cache step's #1211 continue-on-error convention)**.
**Files:**
