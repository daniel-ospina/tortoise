...
**Intent:** The driver's core action — enumerate teams (seam), size-guard each, dump each team graph via the SHIPPED `create_backup` (per-team `backups/{org_id}/` stream), empty-content transition guard, prune. NO registry machinery (superseded by #669).
