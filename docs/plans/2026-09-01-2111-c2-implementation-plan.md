...
4. `POST /v1/organizations/{org_id}/graphs` (key auth): resolve key → **key.org_id must == path org_id else 404** (P1 #6) → 403 if no graphs:create scope OR delegation_depth == 0 (minted key can't provision — E2E-4) → suspension 403 → `_provision_graph` (caller_key_id = key.id).
...
- **Review-caught fixes during implementation (self-review before the PR gate):**
  ...
  4. **Session delete was dead code (real bug):** DELETE /v1/graphs/{id} used `Depends(get_current_org)` (key-only) but the plan (§6.3/W3) requires "key with graphs:delete OR owner/admin session" — a session JWT (eyJ…) 401s at get_current_org's format gate, and its return dict never carries `user_id`. Fixed: the endpoint now uses `Depends(get_current_org_session)` (the dual-auth dependency: key → get_current_org; session → `_session_user_org`), reading `session_user_id` for the session-face role check. The #1148 dashboard-key-login gate only rejects legacy `tt_` keys on flag-off teams — tk_ graphs:delete keys and sessions always pass (matches the provisioning face). New tests: `test_delete_graph_session_owner_204`, `test_delete_graph_session_member_403`, `test_delete_graph_403_suspended`.
...
- **Code-review-gate findings (sub-agent review, 2026-09-02) — ALL FIXED:**
  ...
  - **Security P1 (management surfaces, code-review #2):** deleg=0 keys could reach account-management endpoints beyond mint/session-login (key revoke, billing checkout/portal, backups restore, agent-token revoke, onboarding team-create) — the #1148 dashboard-key-login gate only rejects tt_ and the DB CHECK constrains deleg=0 scopes, not capability surfaces. Fixed CENTRALLY: `get_current_org_session` (the management-set dependency) rejects deleg=0 key callers (403 KEY_NOT_USER_MINTED) — one gate covers every gated endpoint. create_api_key's own deleg gate retained as defense-in-depth.
  - **Security P1 (data-plane dormancy, code-review #2b — SUPERSEDES the earlier "P2 not-fixed" reading):** the earlier entry said scope/graph enforcement is C5's by design so C2 leaves deleg keys data-plane-open. #2b showed that is incoherent AND unsafe: the C2 deleg gate (placed before the gate_key_login branch) blocked deleg=0 on gated + ungated WRITES, but 19 raw-`Depends(get_current_org)` endpoints (points/search/packs/sessions/context/demo/backups reads AND writes) stayed OPEN — a handed-out "read-only per-graph" key could read/write the ENTIRE team pre-C5 (the C1 fail-closed invariant "a graph-bound key must never widen" is unenforceable until the spine consumes graph_id/scopes). FIXED: full dormancy — deleg=0 keys are mintable / revocable / listable (the provisioning lifecycle) but authenticate to NO capability surface until C5 routes them per-graph:
    1. `get_current_org_gated` (new): raw-data-endpoint dependency = pure `get_current_org` resolution + `_reject_minted_delegated_key` — all 19 raw data endpoints swapped to it (resolution seam stays OPEN: C1's tenancy-field contract test + C5's spine read graph_id/scopes off the resolved dict — #2114 consumes it).
    2. `_reject_minted_delegated_key` (new shared helper): key_id set AND delegation_depth == 0 → 403 KEY_NOT_USER_MINTED; used by get_current_org_gated + get_current_org_session (management set).
    3. MCP mirrors the gate in TeamResolutionMiddleware (403, not 401): `apikey_verify` (registry) now returns delegation_depth/scopes (was {org_id,key_id}); resolve_api_key (Supabase) already carried them. New negative test `test_minted_deleg0_key_403_over_mcp`.
    4. Posture pin: `test_minted_key_dormant_on_data_plane` (GET /v1/points with deleg=0 → 403 KEY_NOT_USER_MINTED).
    C5 #2114 flips dormancy off deliberately per-graph (it replaces the raw data deps with graph-scoped routing); C3 #2112 mints deleg=NULL scoped lifecycle keys (pass the gate) or deleg=0 graph keys that stay dormant pre-C5 — handoff recorded.
  - **Security P2 (delete cascade partial-failure, code-review #2b):** delete_graph's cascade revoked keys in a bare loop — one revoke raise → HTTP 500 AFTER the tombstone committed, remaining keys stayed active, _acl_user_drop_hook never ran. Fixed: per-key try/except + ERROR log, always 204 (best-effort eventual revocation, mirrors _revoke_graph_keys; retry converges idempotently).
  - **Security P2 (delete docstring overclaim, code-review #2b):** docstring claimed "a graphs:delete tk_ key always passes" — false: _mint_graph_key never stamps graphs:delete (child policy ∩ _MINTABLE_SCOPES = read/write) and the deleg gate rejects deleg=0 first. Fixed: docstring states the real contract — graph-lifecycle tk_ keys must be owner-minted deleg=NULL scoped keys (C3 #2112 consumes the C2 shared mint with that contract).
  - **Security P2 (quota registry tombstone drift, code-review #2b):** _count_resource(resource="graphs") registry branch counted tombstones (no status filter) while sdk.graph_count + Supabase exclude deleted — latent drift for any C3/C7 path reusing enforce_org_limit("graphs"). Fixed: same `status IS NULL OR status <> 'deleted'` predicate (one line).
  - **Review #2b/#3/#4 fixes (convergence round):** (a) MCP deleg gate hardened: registry `apikey_verify` now returns delegation_depth/scopes (was {org_id,key_id}); fresh-path gate + warm-cache defense-in-depth pop; jsonrpc data payload `{code: KEY_NOT_USER_MINTED}` (403-class signal, mirrors ERR_SUSPENDED precedent); new `test_minted_deleg0_key_403_over_mcp`. (b) registry `apikey_create` mirrors the Supabase DB CHECK (deleg=0 + escalation scopes → ControlPlaneError) — selfhost parity when C5 flips dormancy off. (c) `_provision_preflight` factored (suspension→tier-402 shared by both endpoints — #1853 ordering pin can't be re-copied wrong); dup-name/quotas stay per-endpoint under the shared lock (D1 residual drift recorded, C3/C7 handoff). (d) Supabase cross-worker dup-name race (unique index uq_graphs_org_name_active) → 409 "Graph name already exists" instead of generic-500 (registry lane double-201 window documented in the multi-worker caveat). (e) delete_graph cascade: per-key try/except + ERROR log, always 204 (one revoke failure never 500s a committed tombstone). (f) claim/claim_email: `_reject_minted_delegated_key` added post-resolve (they resolve outside every deleg gate; safe by the anon-team invariant today — symmetric hardening + comment). (g) Supabase-lane quota-count drift-swallow (C1 pre-C1-schema compat): documented at _graph_quota_gate as a known mode divergence (registry 500-reachable, Supabase dormant during degraded reads) + C5/C7 handoff to narrow to the 404-class. (h) comment/docstring truth pass (Agent #4): delete_graph docstring (real contract: owner-minted deleg=NULL lifecycle keys), _provision_graph docstring (created_by = session user UUID or "api", NEVER key id), _provision_preflight banner (tier AFTER suspension per #1853), create_team_graph docstring (deleg gate fires at the gated dep first), delete-cascade comment (tombstoned graph invisible to GET /v1/graphs — no list reconciliation exists), "redundant" comment corrected (session-login exchange gate is LOAD-BEARING — /v1/session/login is SKIP_AUTH, bypasses the DI dep). D2 anon-block recorded in the banner note. Posture pin `test_minted_key_dormant_on_data_plane` (GET /v1/points deleg=0 → 403).
