...
- **Code-review-gate findings (sub-agent review, 2026-09-02) — ALL FIXED:**
  ...
  - **Security P1 (data-plane dormancy, code-review #2b — SUPERSEDES the earlier "P2 not-fixed" reading):** the earlier entry said scope/graph enforcement is C5's by design so C2 leaves deleg keys data-plane-open. #2b showed that is incoherent AND unsafe: the C2 deleg gate (placed before the gate_key_login branch) blocked deleg=0 on gated + ungated WRITES, but 19 raw-`Depends(get_current_team)` endpoints (points/search/packs/sessions/context/demo/backups reads AND writes) stayed OPEN — a handed-out "read-only per-graph" key could read/write the ENTIRE team pre-C5 (the C1 fail-closed invariant "a graph-bound key must never widen" is unenforceable until the spine consumes graph_id/scopes). FIXED: full dormancy — deleg=0 keys are mintable / revocable / listable (the provisioning lifecycle) but authenticate to NO capability surface until C5 routes them per-graph:
    ...
    3. MCP mirrors the gate in TeamResolutionMiddleware (403, not 401): `apikey_verify` (registry) now returns delegation_depth/scopes (was {team_id,key_id}); resolve_api_key (Supabase) already carried them. New negative test `test_minted_deleg0_key_403_over_mcp`.
