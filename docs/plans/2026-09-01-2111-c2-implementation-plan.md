...
- **Code-review-gate findings (sub-agent review, 2026-09-02) — ALL FIXED:**
  ...
  - **Security P2 (quota registry tombstone drift, code-review #2b):** _count_resource(resource="graphs") registry branch counted tombstones (no status filter) while sdk.graph_count + Supabase exclude deleted — latent drift for any C3/C7 path reusing enforce_team_limit("graphs"). Fixed: same `status IS NULL OR status <> 'deleted'` predicate (one line).
