...
- Base columns missing (table broken / corrupt, distinct from a full outage) → **Expected:** base-only retry also raises → RuntimeError propagates (fail-closed, outcome unchanged), but the failure path now costs up to 3 teams queries + 3 WARNING records per request (2 queries + 2 WARNINGs on the additive-degrade path) for the entire degradation (acknowledged trade-off for the single-round-trip healthy path; bounded only by the deferred seam error-discrimination closure, item ①) → **Test:** `test_team_by_id_base_read_fails_closed` + existing resolve fail-closed tests
...
(Trade-off note: a non-drift teams failure now costs up to 3 queries + 3 WARNING records per request (2 queries + 2 WARNINGs on the additive-degrade path) — the price of keeping the healthy path at one round-trip; gating the retry to genuine column absence is the deferred seam error-discrimination item ①.)
