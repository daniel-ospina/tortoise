...
```python
def _teams_row_fail_soft(cp, org_id: str, *, select: list[str],
...
**Step 1:** Rewrite `team_by_id` (additive set = 0015 `suspended_at`/`flagged_at` + 20260813000005 `dashboard_key_login` — seam-uniformity; `deleted_at`/`grace_hours` stay in the select so the base retry keeps them — their absence fails the retry closed):

```python
def team_by_id(cp, org_id: str) -> dict | None:
