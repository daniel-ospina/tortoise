...
- **D2 — `recover_team_key` RPC returns org_id; caller pre-reads the teams row** for org_name/tier/suspension. Deleted team → uniform 422 (caller pre-check); the RPC's own deleted/zero-row RAISE is the fail-closed backstop mapped to 422 on known codes, unknown → 500.
...
- Table: `token_hash text PRIMARY KEY`, `org_id text NOT NULL`, `created_at timestamptz DEFAULT now()`, `last_used_at timestamptz`, `revoked_at timestamptz`; partial unique `uq_agent_signup_tokens_team ON (org_id) WHERE revoked_at IS NULL`; `REVOKE ALL ... FROM public, anon, authenticated`; `GRANT SELECT, INSERT, UPDATE ... TO service_role`.
...
- `resolve_signup_token(p_token_hash) RETURNS text` — `SELECT org_id ... WHERE token_hash=... AND revoked_at IS NULL` + `UPDATE ... SET last_used_at=now()` (one tx).
- `recover_team_key(p_token_hash, p_org_id, p_lookup_hash, p_key_prefix, p_max_api_keys DEFAULT 2) RETURNS text` — `SELECT ... FOR UPDATE` (serializes same-token recovery; the ONLY FOR UPDATE in the trio — `resolve_signup_token` is not); zero rows → RAISE; reject `teams.deleted_at`; INSERT api_keys `created_via='recovery', created_by='st_'||left(p_token_hash,12)` (ON CONFLICT (lookup_hash) DO NOTHING) FIRST, then IF a row was actually inserted, count `(created_via IS NULL OR created_via <> 'bootstrap')` active keys and at `> max_api_keys` revoke OLDEST non-bootstrap (a no-op retry never revokes); RETURN p_org_id. p_api_key/p_key_hash were DROPPED in review (the RPC binds auth on lookup_hash alone — plaintext is caller-held).
...
**Acceptance:** wrappers call `cp.rpc` with the exact param names; `resolve_signup_token` returns `str | None`; `recover_team_key` returns org_id or raises RuntimeError with the RPC code embedded.
...
- `recover_team_key(cp, *, token_hash, org_id, api_key, key_hash, lookup_hash, key_prefix) -> str` — RPC; map nothing here (hosted_api maps codes).
...
**Acceptance:** `provision_team_with_token` writes the token row in the same call as the provision; `resolve_signup_token` returns org_id or None; `recover_team_key` does cap-check + revoke-oldest + insert atomically (under a `threading.Lock` — mirrors the FOR UPDATE serialization).
...
**Acceptance:** `RecoveryVelocityTracker` (threshold default 5/24h, env `TORTOISE_ABUSE_RECOVER_THRESHOLD`/`TORTOISE_RECOVER_IP_LIMIT` fallback), `record_recovery(ip, org_id)`, module-level `RECOVERY_TRACKER`, notify kind `abuse_recovery_velocity`, dashboard event `recovery_velocity`.
