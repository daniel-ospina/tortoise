...
### Task 3: Currency read (tri-state)  *(O3 resolved — see §8; only the enforcement form is still open)*
...
- **O3 — the currency read's surface (gates Task 3).** ✅ **RESOLVED for placement, REOPENED for enforcement.** The already-recorded **Policy B** ruling on #5038 (owner, 2026-09-24, comment `5814346672`) decides the placement: *"A currency check reads the version recorded on the extraction link and compares it to the source's current `contentHash`. That is a **read** … **no stored `status` field**"* — i.e. on the **existing** read surfaces, adding **no new tool and no new SDK method**, so #4282's tool/method rule is satisfied and neither the in-repo-tool nor the SDK/MCP option is needed. Policy B *also* requires the stale fact to stay **"still there, still readable, and flagged"** (*"Nothing is silently withdrawn"*), so **withholding it from ordinary searches is a REOPEN of Policy B** — filed in the owner queue (labelled `owner-decision`), options being **ranked lower** or **dropped from ordinary searches**. Research report: `docs/research/2026-09-25-5038-source-currency-read-path.md`. *Original options:* in-repo tool / SDK method / MCP tool — the SDK/MCP arms are **superseded** by that ruling rather than chosen.
