...
### Task 2: Transfer semantics  *(gated on O1)*
**Intent:** decide and, if O1 ≠ D, implement the transfer's treatment of the version.
**Acceptance:** for a chain A→B→C, live and post-rebuild agree on the successor's `sourceVersion`; the anchor exists at the successor and NOT at old; if a version is carried, the identical guard is applied at **both** `sdk.py:6364-6367` and `projection/__init__.py:5602-5605`, and `structural_seen` includes the captured value.
**Files:** Modify `tortoise/sdk.py` (emission SELECT/descriptor/live transfer), `tortoise/projection/__init__.py` (consumer + dedupe key), the `DirectEdgeRepoint` schema. Test: extend `tests/test_pointsuperseded_rebuild.py`.

### Task 3: Currency read (tri-state)  *(gated on O3 for the surface)*
**Intent:** deliver issue deliverable 3 without a surface change.
**Acceptance:** per Point/per link `current`/`stale`/`unknown`, **`unknown` when either side is `NULL`/`''`**; never nested under `tortoise_stale`; in-repo `tools/` script only.
**Files:** Create `tools/source_currency.py`. Test: unit + integration.
...
- **O1 — supersede semantics (gates Task 2).** *Options:* **D** record nothing (the ontology-default for a non-re-read successor) / **A** inherit (a §4.6 **reopen**, not an `OVERRIDES`) / **B** re-derive (matches §4.6, needs a write-decision descriptor) / **C** a dedicated journaled record. *Analysis:* D needs no transfer edit and is the honest default; A records a version the successor was never read from; B matches §4.6 at more cost; C adds a record type. *Recommendation:* **D**, escalating to B only if the owner requires a transferred belief to keep a staleness signal.
- **O2 — 2489 step-4 departure.** `docs/plans/2026-09-07-2489-structural-edge-parity.md:71` is a plan **step** (rationale scoped to direction/confidence/weight). *Recommendation:* surface on #5038; only relevant if O1 = A/B.
- **O3 — the currency read's surface (gates Task 3).** *Options:* in-repo tool / SDK method / MCP tool. *Analysis:* the latter two need owner approval first; **#4282** (the canonical MCP/SDK surface consolidation, 98→26 tools / 150→40 methods) governs any such addition. *Recommendation:* in-repo tool now; decide the SDK/MCP question under #4282.
