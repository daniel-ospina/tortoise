...
- Test: `tests/test_reaper.py` (CLI subprocess tests) + **cron/launchd config verification (R5 fix — concrete: `plutil -lint` the .plist for macOS launchd; validate crontab line format for cron; document both checks in the install doc)**
