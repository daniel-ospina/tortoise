...
3. JSONL `rebuild_all()` if tortoise.db absent + embedded.db exists; **tortoise.db present (any state) + no marker = resolve via the (e3) discriminator (R7 fix — 'partially-present' was broader than the test discriminator): `rebuild_all()` is documented as ALL-OR-NOTHING per event-batch (each batch applies atomically; interruption leaves either 0 applied batches or all) → 0-node/corrupt tortoise.db = interrupted → delete + rebuild cleanly (never treat as no-op); valid-with-nodes = genuine conflict → error/--force (see test (e3)/(e5)).**
