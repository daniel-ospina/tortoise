...
13. `test_capture_journal_line_order_registration_before_fold` — a capture-style sequence (create Objects then `apply_supersessions`) → assert via `read_all()` that the `ObjectRegistered` lines PRECEDE the `ObjectSuperseded` line. Pins the synchronous post-apply emission point — load-bearing for the single-log `rebuild()` path (projection/__init__.py:1148), which folds chronologically with NO deferred sweep and would resurrect a superseded Object from an [OS, OR] journal.
