...
- **Stub-adoption title + status asymmetry** (connector stub adopted by a capture mention): live keeps the stub's title (ON MATCH `coalesce($title, o.title)`) and has NO `status` property (ON MATCH never writes status — #1350 clobber guard; `status ∈ _OBJECT_HANDLED` blocks extras); rebuild ON CREATE writes `title=''` + `status='live'` (EventAPI-parity, pre-existing) — accepted + documented (benign: status-absent and 'live' are both outside the recall-exclusion tuple); createdAt is NOT asymmetric (ON MATCH `coalesce(o.createdAt, $ca)` adopts the synthesized value).
...
7. `test_no_log_sdk_no_journal_no_prop_change` (**green-pin guard** — journal-less SDK: no `event_log_path`): create_object **passing `point`/`payload` props** → no log file exists / no ObjectRegistered anywhere; the reserved props are NOT persisted live (the unconditional pop applies on journal-less SDKs too — pinning the journal-less half of the reserved-name narrowing so a regression to a journal-gated pop, which would reintroduce divergent journaled/journal-less live persistence, fails loudly); node props otherwise the pre-change set (createdAt from the projection path; no synthesis artifacts; no envelope keys). Cannot fail pre-fix for real caller shapes — it only guards against an over-broad implementation.
...
10. `test_probe_failure_fails_open_to_journal` — monkeypatch `proj.g.query` to raise on the probe MATCH → create an Object → assert (a) the create still succeeds (no probe exception escapes), (b) a warning is logged, (c) the ObjectRegistered line IS journaled (durable bias), (d) a subsequent `rebuild_all` restores the node. Pins the try/except — a regression to fail-closed (silent skip) would re-open the node-loss bug with no test catching it.
...
**Step 2.3** — GREEN: rerun `tests/test_object_registered_journal.py` → all 12 pass.
...
**Step 3.6** — Green: `TORTOISE_DB_URI='docker://:falkordb@localhost:6379/tortoise_test_matrix' uv run pytest tests/test_status_projection.py tests/test_object_registered_journal.py -v` → all pass (19 migrated + 12 in the new file).
