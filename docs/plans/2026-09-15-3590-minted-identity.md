...
| `plan-review` gate (`workflow/05`) | **CLEAN at the cycle-4 safety cap.** Adversarial review cycles r1–r4 have run (2026-09-15); findings per cycle were **5 / 5 / 5 / 1 + 6 P2**. Cycle 4 was the cap, and its verdict was: *"S0, S1, S3 and S4 are executable as written. S2 is blocked by 1× P1."* That single P1 is **now resolved by a product decision**, not by another review cycle — **Product Decision #4** (terminal-only holders do not block a create; a create on a superseded/deprecated/archived/outdated name mints a fresh live entity and leaves the terminal node untouched) — and the six P2s are **execution-time edits** applied in place (the reviewer: *"I would not spend a fifth cycle on the P2s"*). The two positives were recorded verbatim: **no shape still produces a guess**, and **no vacuous assertions** (the plan's strongest feature). Because nothing blocking remains, the document is now `doc_status: ready` and carries the `<!-- plan-review: … -->` signature at the foot of this file. See the `Review r1`–`r4` rows below. |
...
| Review r4 (2026-09-15, the cycle-4 safety cap) | **1 P1 + 6 P2s applied in place; all accepted. This closes the loop: `doc_status: ready`.** The cap verdict was *"S0, S1, S3 and S4 are executable as written; S2 is blocked by 1× P1"*, with two positives recorded — **no shape still produces a guess** and **no vacuous assertions** (the plan's strongest feature) — and an explicit *"I would not spend a fifth cycle on the P2s."* **The P1 (resolved by Product Decision #4 + plan edits):** D2's live-holder predicate makes `create_object("X")` whose only holder is `status='superseded'` see zero live holders and **mint a fresh id** → a second live node with that name; but S2's test row for `tests/test_object_registered_journal.py:199-228` asserted the opposite (a superseded name re-mentioned stays superseded and journals nothing) while **citing that same test** to justify R5's deletion-specific predicate. The **name**-path scenario and the **id**-path scenario were conflated, and the two name-keyed `_object_row` assertions (`MATCH (o:Object {name:$n})`, **no status filter**) became row-order coin flips. Fixes: (a) the semantics are stated in **D2** and added as **Product Decision #4** (marked product-visible); (b) `:199-228` is **re-derived onto the explicit-id path** (`_server_id=` — the superseded-**id** no-op, the real #1350 guard) and the name-path scenario gets its own test; (c) both R5 justifications (**D5-R5**, **S2 step 6a**) re-point at the explicit-id test. 6 P2s: D8's predicate gains the `issue_`/`pr_` shapes and drops its completeness claim; S0's `delete_then_replay_vs_never_created` is marked `xfail`/owner #3573 (it cannot hold today — `_delete_entity` is a bare `DETACH DELETE`); S1's two stub-adoption tests get an executable `xfail(strict=True)… #3590/P1-D` disposition and a row in **S1's** table; D9/S2 step 5 forbids a bare mint in producer lanes; `_record_non_fold` gains an explicit `journal_pos` source; and D3/the S2 table stop naming `TERMINAL_EXCLUDED_STATUSES` as the predicate. The pre-existing `apply()` catch-all gap is cited by number — **#3597** (OPEN). |
...
- zero **live** holders → **mint** a fresh ULID (D1) and register. **Terminal-only holders do not block a create (Product Decision #4 — DECIDED, not open):** if the name's only holder(s) are terminal (`superseded`/`deprecated`/`archived`/`outdated`/`retracted`, i.e. whatever `_LIVE_HOLDER` excludes), the create sees zero live holders and **mints a fresh live entity**; the terminal holder is **left untouched**. This follows directly from the model — the name is a mutable **label**, not the identity, so a terminal holder is not "occupying" the name. **Only a deleted *id* is permanently barred (R5)** — the bar is on the id, not on the name. This is a **product-visible behaviour change**: today a `create_entity("object", "X")` whose only holder is superseded returns the superseded node and journals nothing (the `_upsert_*` name-dedup / the canonical-id re-fetch); after S2 it journals a second `ObjectRegistered` and a **second live node bearing that name** exists alongside the untouched terminal one;
- **≥ 2 live holders → the name is ambiguous; by-name resolution refuses** (records a non-folded entry; the run fails — R8/R9). It never picks one. (A terminal holder does **not** count toward the ambiguity count — the same predicate, stated once.)
...
**Consequence — same-name coexistence is now ALLOWED**, which is the behaviour change the issue asks for. It is reachable by **two** routes — a rename is **not** one of them (D3 refuses that): **(i)** an **explicit-id** writer — `_connect_issue_objects` (`sdk.py:19427`, ids like `github-issue-{repo}-{number}` and `issue_<sha8>`), `tortoise/github_map.py:147-155` (`ObjectRegistered` with a connector id), `EventAPI.add_object(..., id=…)` (`api.py:255-269`, used by `mining.py:592`), and any other direct `{id:$id}` `MERGE`; and **(ii)** a **plain create on a name whose only holder(s) are terminal** (Product Decision #4) — it mints a fresh live entity **beside** the untouched terminal holder. Two explicit-id writers that happen to pick different ids for the same name, or a terminal-then-create sequence, are the ways two live same-name carriers arise; once they do, the resolver refuses to pick one (R8).
...
1. **No production graph holds a serviceable Object or Subject — verified by sweeping EVERY graph on EVERY instance, not a 4-graph sample, and by the enumerated legacy id shapes (P2; the disjunct list now includes `issue_`/`pr_`).** An earlier draft checked four graphs on one instance, reported "0 Objects, 0 Subjects" and "1,595 graphs", keyed its guard to node counts that had already drifted, and — the P1-A defect — swept **only** `id STARTS WITH 'obj-'`. Legacy Subjects are minted `sub-<26hex>` (`sdk.py:1358`, `label[:3].lower()`), so the Object-only predicate provably could not see one and the evidence table reported only `obj-` totals. Corrected, re-swept 2026-09-15 (snapshot — see the drift note) across all **three** live FalkorDB instances, with both prefixes run per graph:
   ...
   **P2 — two MORE shapes were invisible to the two-prefix predicate, and one of them was found live.** The r3 re-sweep (2026-09-15, same session, all three instances, **widened** predicate) returned **8 hits**: `graphops_measure_tmp` (4 `obj-` Objects) and `probe2517props` (1 `obj-` Object) as before; **plus three `id == name` Subjects** — `agent-pi` in `test_proj125_9a328d44`, `test_proj125_feae921e`, `test_proj125_bccddb9d` — which the `obj-`/`sub-` predicate **provably could not see**. (`_mint_subject_stub`, `projection/edges.py:79`, does `ON CREATE SET s.id=$name` — the fourth shape.) `mining._object_id`'s `obj_<hash16>` (underscore) is the third shape: not one hit live, but the code path exists, so the predicate must see it. Graph counts had also drifted again (`falkordb` 1452 → **88**, `falkordb-eval` 6 → **7**), which is exactly why the counts are a snapshot and the predicate is the guard.
   ...
   ⛔ **The guard is a PREDICATE, never a count — and it is PER-LABEL.** The S4 precondition is: *"any legacy-shaped `Object`/`Subject` id exists in any **service** graph on any live instance ⇒ **S4 must not merge**."* The predicate is the **enumerated disjunct list below**, not a claim of completeness — the word "complete" is exactly how a shape gets missed (cycle 2's P1-A swept `obj-` only and could not see a legacy `sub-` Subject). An earlier revision called this "complete over … all four shapes" while omitting a **fifth** shape the plan itself names as a name-derived producer in D9: `_connect_issue_objects`'s no-id fallback emits `f"{key.rstrip('s')}_{sha256(name)[:8]}"` for `key in ("issues", "prs")` (`sdk.py:19457-19462`) → **`issue_<8hex>` / `pr_<8hex>`**. That is the same defect class, so the predicate is widened to it and the completeness wording is withdrawn:
   ...
   MATCH (n) WHERE (n:Object  AND n.id STARTS WITH 'obj-')
              ...
              OR (n:Object  AND (n.id STARTS WITH 'issue_' OR n.id STARTS WITH 'pr_'))
   RETURN labels(n), n.id, n.name
   ```

   The disjuncts are, in order: `_entity_name_id` Object; `_entity_name_id` Subject; `mining._object_id` (`obj_` + 16 hex — **underscore**, invisible to `STARTS WITH 'obj-'`); the same for a Subject; the `id == name` stub shape (`_mint_subject_stub`, `projection/edges.py:79`); its Object twin; and the `_connect_issue_objects` fallback (`issue_<8hex>`/`pr_<8hex>` — **Object-only**, because that writer only ever targets `:Object`). **Zero matches in every service graph** is required before S4, and a run that reports only an `obj-` total is not a pass (it cannot see a legacy Subject — nor the `obj_`/`id == name`/`issue_`/`pr_` shapes). **The r3 table above is a four-shape sweep** (8 hits, all in throwaway graphs); it **predates** the `issue_`/`pr_` disjuncts, so those two totals were **not measured** in it. **Re-run the widened (seven-disjunct) predicate immediately before S4 merges** and record the totals — the *recorded counts must match the predicate that gates*, and an `issue_`/`pr_`-omitting run is not a pass. The check enumerates **every graph on every instance** (sweep `GRAPH.LIST` per instance, then the predicate per graph) and never uses `=~` — **FalkorDB does not support `=~` and fails silently (empty result, not an error)**, which is exactly how the original verification produced a false negative; use `STARTS WITH`/`ENDS WITH`/`CONTAINS`. (The `=~` hazard is filed as its own issue.)
...
**Chosen:** in Slice 2, every producer that emits an `EventRecorded` whose `subject`/`object`/`uses` is a bare name **must also emit the entity's journaled registration** (`ObjectRegistered`/`SubjectAdded`) carrying its id, before the event — the id **resolve-or-minted through the SDK resolver** (`_resolve_or_mint`, S2 step 1), **never a bare `_mint_entity_id()`/`ulid()`** (P2): these producer lanes fire **repeatedly for the same name** (`slack.py:267`, `linear.py:190/217`, `mining.py:825`, `sdk.py:11508`), so a bare mint per event would register a **second** id for a name that already has a live holder → a second live carrier → a permanently create-poisoned name. Only `_resolve_or_mint` is sanctioned as a minter. Enumerated producers needing this: `tortoise/connectors/slack.py:267`, `tortoise/connectors/linear.py:190` and `:217`, `tortoise/sdk.py:11508` (`subject=agent_name`, no registration), `tortoise/mining.py:825`. `connectors/github.py:261/284` and `tortoise/github_map.py:155/256` already do this. Lanes where the entity is registered first need no change: the projection's name→id lookup finds the replayed registration and wires the edge with **no mint**.
...
- Modify: `tortoise/projection/__init__.py` — the `non_folded` recorder (`_record_non_fold(journal_pos: int | None, ev, *, shape: str)`) and `materialize()`; the `apply()` malformed-event arm and a new `else` unhandled-type arm record, with `apply()` gaining a `journal_pos: int | None = None` parameter so the recorded entry can name the position (P1-E, P2) — **records only, never raises**
...
1. **Write the shape list into the shared fixture.** Follow the mechanism #3589 prescribes (`tests/fixtures/org_naming_vectors.json` is the in-repo precedent — a single vector file consumed by the Python and dashboard suites). Each vector is `{id, description, steps, expect_live, expect_replay, verdict, owner}`. Seed it with:
   ...
   - `delete_then_replay_vs_never_created` — the P1-1 distinction: a delete materialises a `status='retracted'` node on **both** sides, a never-created name materialises **nothing**; the two must not compare equal — **verdict: `xfail`, owner: #3573 at S0.** It does **not** hold today: `_delete_entity` (`sdk.py:17075-17088`) is a bare `DETACH DELETE`, so live-after-delete materialises **nothing** and `live(deleted) == live(never-created) == {}` — the vector's own assertion **fails**. Un-`xfail`ed in S2 alongside the `#3573` pair (the tombstone half of D5), which is when the shape becomes true
...
| `::test_delete_then_replay_vs_never_created` | **`xfail(strict=True)`, owner #3573 at S0 (un-`xfail`ed in S2)** — today `_delete_entity` is a bare `DETACH DELETE` (`sdk.py:17075-17088`), so live-after-delete materialises **nothing** and live(deleted) == live(never-created) = `{}`: the distinction holds **after S2**, not today. The S0 table's earlier "holds today and after S2" wording was wrong |
...
| `tests/test_object_registered_journal.py:238` / `tests/test_subjectadded_journal.py:182` (`create_entity` stub adoption) — **`xfail(strict=True, reason="adoption lands in S2 — owner #3590/P1-D")` in S1 (P2)** | S1's `create_entity` still computes `_entity_name_id`, so a raw stub (different id) is **not** adopted by the id-keyed `MERGE` — these two tests **fail at S1**. They are `xfail`-pinned here (so S1 lands green and cannot silently pass) and **re-derived in S2** where `_resolve_or_mint` exists; see S2's test table |
...
5. **Journal the stub mint in the write layer, not the projection (D9).** The projection cannot append to the JSONL log — `_emit_event` lives on the SDK (`sdk.py:2275`) — so it cannot mint: a projection-side mint would be replay-nondeterministic (step 4).
   ...
   - In the producers enumerated in D9, **resolve-or-mint through the SDK resolver** (`_resolve_or_mint`, step 1 — **never a bare `_mint_entity_id()`/`ulid()`**, D9) and emit that id's registration (`ObjectRegistered`/`SubjectAdded`) **before** the `EventRecorded` line so the replay looks the name up and finds it. A bare mint per event would register a second id for a name that already has a live holder → a second live carrier → a permanently create-poisoned name.
...
8. **Point the `xfail`-pinned delete shapes at the fix, and re-derive the "gone" assertions to tombstone semantics (P1-1).** Remove `xfail` from `delete_object`, `delete_then_recreate` **and `delete_then_replay_vs_never_created`** (the third was `xfail`-pinned at S0 for the same reason — P2) in `tests/fixtures/entity_identity_vectors.json`; they must now pass. Rewrite the two green-pins that assert the opposite:
...
| `::test_superseded_id_reregistration_is_a_noop_not_a_raise` | **P1-B** — re-register (via an explicit-id channel) a **superseded** id: live **does not raise** (the #1350 no-op) and replay folds without a non-fold; the #1350 clobber guard is preserved (the **explicit-id** re-derivation of `tests/test_object_registered_journal.py:199-228` — one test, cross-referenced). Guards against R5 reverting to `TERMINAL_EXCLUDED_STATUSES` |
...
| `tests/test_object_registered_journal.py:238` `test_stub_adoption_journals_canonicalization` — **re-derived in S2 (P1-D); the S1 `xfail` is REMOVED here** | a raw name-stub (id `X`) resolved by `_resolve_or_mint` is **adopted**: the registration carries `X`, the merge key is `id`, and no second same-name node appears; live == replay |
| `tests/test_subjectadded_journal.py:182` (Subject twin) — **re-derived in S2 (P1-D); the S1 `xfail` is REMOVED here** | same, with the Subject lane's live `status='live'` (`_persist_extra_props`) asymmetry preserved |
| `::test_ambiguous_name_resolution_fails_closed` | ≥2 live same-name → `AmbiguousEntityName`, never a pick |
| `::test_one_live_one_terminal_holder_is_not_ambiguous` / `::test_lone_terminal_holder_reference_refuses` | **P2** — the D2 `_LIVE_HOLDER` predicate (`projection/entities.py::_LIVE_HOLDER`, delegating to `live._terminal_excluded`), both directions. **Not** a re-typed `TERMINAL_EXCLUDED_STATUSES` literal (which would exclude `status='live', outdated=NULL` via three-valued logic) |
...
| `tests/test_rebuild_live_invariant.py::test_delete_then_replay_vs_never_created` | **P1-1** — the two must not compare equal; the S0 `xfail`/owner #3573 is **REMOVED here** |
...
| `tests/test_object_registered_journal.py:199-228` (`test_remenion_after_fold_does_not_journal_or_resurrect`) — **re-derived onto the EXPLICIT-ID path in S2 (P1; this is the real #1350 guard test)** | a **superseded id** re-registered explicitly (`_server_id=`, mirroring `::test_superseded_id_reregistration_is_a_noop_not_a_raise` — one assertion, two rows) stays superseded, journals nothing, and does not resurrect: the **id**-keyed no-op R5 must not break by treating supersession as deletion. **The earlier row asserted the NAME path** (`sdk.create_entity("object", "strategy-A")`, `:207-218`) — under D2 + Product Decision #4 the name path now **mints a fresh live entity** and journals a **second** `ObjectRegistered`, so the name-keyed `_object_row` assertions (`MATCH (o:Object {name:$n})`, **no status filter**) would have been row-order coin flips. The name path gets its own test, next row |
| `::test_create_on_superseded_name_mints_fresh_and_leaves_holder_untouched` — **NEW (P1 / Product Decision #4)** | `create_object(name)` whose **only** holder is `status='superseded'`: a **fresh** live id is minted, the superseded holder is **untouched** (`id`/`status`/`supersededBy` unchanged), exactly **one** new live node for that name appears, a second `ObjectRegistered` is journaled, and `rebuild` reproduces **both** nodes (superseded + live) exactly |
...
| # | Item | Status |
|---|---|---|
| U1 | **What a replay of a pre-change event means once the resolver only understands new ids** (the decision doc's explicit gap) | **Moot under a fresh start — and falsifiably so.** D8 records the three checks. The first is now a **per-label predicate over every graph on every instance over the enumerated legacy id shapes** (`obj-`, `sub-`, `obj_`, `id == name`, `issue_`, `pr_` — an `obj-`-only or two-prefix run is not a pass), **not** a node count — because the count-based guard drifted and `=~` fails silently. **Re-run those checks before S4 merges**; if the predicate matches any service graph, S4 must not merge and a dual-read resolver is required first. |
...
1. Same-name coexistence is allowed (D2), reachable via **explicit-id writers** or **a plain create on a terminal-held name** (see #4) — a rename onto a live holder's name is **refused** (D3). Plain `create_*(name)` stays idempotent **while a live holder exists**. This is the issue's stated behaviour change, narrowed to remove the create-poisoning failure mode.
...
4. **Terminal-only holders do not block a create (DECIDED 2026-09-15 — resolves cycle 4's P1).** A `create_object(name)` / `create_subject(name)` whose only holder(s) carry a terminal status (`superseded`/`deprecated`/`archived`/`outdated`/`retracted` — whatever `_LIVE_HOLDER` excludes) sees zero live holders and therefore **mints a fresh live entity**; the terminal node is **left untouched**. A **name** is a mutable label, not the identity, so a terminal holder does not "occupy" it; **only a deleted *id* is permanently barred** (R5 — the bar is on the id). **This is a product-visible behaviour change:** today the same call returns the existing superseded node and journals nothing; after S2 it journals a second `ObjectRegistered` and the name resolves to the new live node, with the superseded node still present and still superseded. It is what makes D2's `single_terminal_holder_reference` vector behave as written ("never returns a terminal id"), and what makes D3's `test_rename_onto_terminal_holder_is_allowed` and S2's `::test_create_on_superseded_name_mints_fresh_and_leaves_holder_untouched` true.

---

## Rollback (epic level)

Each slice is a separate commit and reverts independently (per-slice rollback notes above). No slice carries a data migration, so **there is no data rollback in any slice** — the only durable artifacts are JSONL journal lines, and every line type this plan introduces (`Deleted`, `Renamed`) is inert under a reverted binary (no branch), which restores the pre-slice behaviour rather than a worse one. `doc_status` for this plan is **`ready`**: the plan-review gate closed **clean at the cycle-4 safety cap** (verdict: S0/S1/S3/S4 executable, S2 blocked by 1× P1) with that P1 resolved by **Product Decision #4** and the six P2s applied as execution-time edits. (An earlier draft of this sentence said `live`; the gate's own signature at the foot of this file is authoritative.)

<!-- plan-review: cycles=4, status=clean, version=2.3.0 (cycle 4 = the safety cap; verdict "S0, S1, S3 and S4 are executable as written; S2 blocked by 1× P1"; that P1 resolved by Product Decision #4 — terminal-only holders do not block a create — and the 6 P2s applied as plan edits; findings 5/5/5/1+6P2; reviewer: "I would not spend a fifth cycle on the P2s", so no cycle-5 re-review was run) -->
