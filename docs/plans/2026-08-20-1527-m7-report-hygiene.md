...
- Extractor identity (M7 #1739): `extractor_model` discriminates **both** wire id and tuning. Bare adapters fingerprint as `wire-id` plus a tuning suffix (`|max_tokens=…|temperature=…|thinking_budget=…|disable_reasoning=…`) when non-default (`max_tokens` whenever not None — an explicit 0 cap is a real value; the rest when truthy), so registry entries sharing a wire id but differing in tuning (`deepseek-v4-pro-xhigh` vs `deepseek-v4-pro-noreason`) differ and a cross-tuning resume is refused — note `deepseek-v4-pro` ≡ `deepseek-v4-pro-xhigh` are BYTE-IDENTICAL configs (both max_tokens=500, temperature=0.0 → both fingerprint `deepseek/deepseek-v4-pro|max_tokens=500` — correct, nothing differs); only `-noreason` is the discriminating variant of the trio. The default CLI wrapper path (`RoutingModel`/`RotatingModel` — neither exposes `.model_id`/`.id`) is composed structurally as `provider:wire-id` per member adapter joined by `+` — deterministic across processes, never an address-bearing repr. A single-lane wrapper emits the bare member fingerprint (no provider/shape prefix — one lane has no routing, so the default path compares against the equivalent bare `MODELS` entry); multi-lane wrappers are shape-prefixed (`routing:`/`rotating:`) so failover vs rotation over the same members never collide. `RoutingModel` members join in (primary, fallback) order (order is effective config — the primary serves); `RotatingModel` members are sorted by provider (pool order is a routing detail — weights are per-provider — so a reorder keeps checkpoints valid while a membership change invalidates them). RotatingModel **weights are excluded**: on the production path they are a pure function of the member set (fixed per-provider values), which IS fingerprinted; programmatic weight changes are out of scope (git_sha-protected). Provider is a routing detail for single adapters (wire-id-only, the #1732 cross-provider contract) but part of the wrapper identity (a pool change is an effective-config change). Env-level extraction toggles (`TORTOISE_JSON_MODE`, `TORTOISE_EXTRACTOR_PROMPT`, …) are **out of fingerprint scope** (pre-existing; #1739 covers constructor tuning) — a toggle across a resume silently reuses results; operator responsibility.
- SESSION_WORKERS (M7 #1739 / review #1742): `--session-workers > 1` runs session-parallel extraction (v2 ingest ONLY — the flag is rejected loudly on other modes via `parser.error`, where it was previously a silent no-op). Each worker builds a FRESH model via the ingest_v2 factory (note: the live ingest path currently shadows the parallel worker factory with a sequential copy — pre-existing, tracked separately — so workers fall back to the shared `extractor_model`; the fingerprint still records the served config). `_build_cli_extractor_model` builds the fingerprinted model, and run_main threads the resolved spec + tuning into the factory, so the workers serve EXACTLY what the fingerprint records: a spec'd run fingerprints identically across a session-workers toggle only when the sw>1 resolved lane matches the sw=1 registry adapter (e.g. `deepseek-v4-pro` in a single-lane env — same wire id + tuning — resume accepted); `deepseek-flash-direct` differs across the toggle in openrouter/venice single-lane envs (family-prefixed `deepseek/deepseek-chat` at sw>1 vs bare `deepseek-chat` at sw=1 — different lane serves a different wire id) and in all multi-lane envs, so the toggle is REFUSED — safe direction, the fingerprint records what each path serves. The unset run stays UNCAPPED (the #1350 owner decision applies regardless of session_workers). Registry entries the factory cannot express (`thinking_budget`/`disable_reasoning` — `_build_single` forwards only max_tokens/temperature) are REFUSED loudly at `session_workers > 1` (never a silent reasoning flip); the M5 unknown-spec gate applies on both paths (a typo fails fast, never a garbage wire id in the checkpoint); spec wire ids are resolved through `_REGISTRY_KEY_TO_ID` so every lane gets a valid id (`deepseek-flash-direct` → `deepseek/deepseek-chat` — bare ids 404 on OpenRouter). `run_evaluation` additionally REFUSES (ValueError, pre-loop) any session_workers>1 call whose worker-factory config does not fingerprint identically to the passed extractor_model (the CLI threads the resolved trio; programmatic callers must too).
- SELF-INVALIDATION (documented, safe direction): #1739 changes the fingerprint literal for tuned registry entries vs the #1732-era binary (`deepseek/deepseek-v4-pro` → `deepseek/deepseek-v4-pro|max_tokens=500`) and for ALL wrapper-path runs (address repr → composed literal). Any checkpoint written by the #1732 binary with a tuned extractor entry (or via the default wrapper path) REFUSES resume post-#1739 — same-command same-config resume now raises `CheckpointStaleError`: delete the checkpoint and re-run (no migration path; the refusal is loud, never silent). The #1732-era bare wire-id literals (`deepseek-chat`, `deepseek/deepseek-v4-flash`, default tuning) are unchanged.
