...
- SESSION_WORKERS caveat (pre-existing factory divergence, documented): with `--session-workers > 1`, ingest_v2 builds per-worker wrappers via `build_extractor_model(args.extractor_model or None)` (function-default max_tokens=4000; `_build_single` forwards only max_tokens/temperature — registry tuning like `disable_reasoning`/`thinking_budget` is not expressible). The fingerprint records the RUN-LEVEL model — the extracting model on the `session_workers=1` path — so a session-workers toggle is not discriminated by the gate. Tracking the factory's effective tuning in the fingerprint is out of #1739 scope (pre-existing factory limitation; the fingerprint now makes the divergence deterministic, never silent-0x).
