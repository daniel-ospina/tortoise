...
- SESSION_WORKERS (M7 #1739 / review #1742): `--session-workers > 1` runs session-parallel extraction — each worker builds a FRESH model via the ingest_v2 factory (`build_extractor_model(spec or None)` at the factory-default max_tokens=4000; `_build_single` forwards only max_tokens/temperature, so registry tuning like `disable_reasoning`/`thinking_budget` is not expressible there — pre-existing factory limitation). `_build_cli_extractor_model` builds the fingerprinted model the SAME way the workers serve (registry model at session_workers=1; factory config at >1), so the fingerprint records the extracting config on every path and a session-workers toggle (an effective-cap change) refuses resume. (The flag was previously parsed but never threaded to `run_evaluation` — a silent no-op and a latent NameError in the factory closure; now wired.)
