...
**Step 8:** Commit: `test(fixtures): #2165 synthetic multi-session Object graph + ingest_v2 Event-aboutObject wiring + env flag + census + ci-surfaces`.
...
**Acceptance:** slice builder returns typed rows with tier tags + admission metadata `{rows_requested, rows_admitted, truncated}`; v2-lane-faithful fixture (zero point-eventId) assembles a dated spine from tier-2 createdAt; hosted variant (Event-aboutObject present) superset; as-of window excludes post-D rows; undated → last, no sentinel masquerade; hub fan-out cap binds AT THE QUERY (plan-review P2: hub fixture with rows > per-slice pre-fetch cap → `rows_requested == cap, truncated=True, rows_admitted < total`); state slice reads status+supersededBy+supersededAt in ONE Cypher statement (no torn supersession header under a concurrent #2242 fold — plan-review P2: mock-port test pins single-statement read); as-of boundary pinned: equality-day inclusive/exclusive normalized to UTC date (superseded on-or-after D excluded / before D retained) + test at supersededAt == question_date (plan-review P2); undated row in an as-of query → admitted undated-last (pinned, no validity check — plan-review P2); malformed `when`/`startedAt` (garbage/wrong-format/empty) → parse-fail → undated tier, never a raise (plan-review P1 — sentinel-strip generalizes to parse-fail→undated).
**Files:**
- Modify: `tortoise/assembly.py`
...
**Acceptance:** flag-OFF golden evidence equality for fired-shape Qs (captured pre-branch at Task-6 Step 1 against clean HEAD, committed with the branch; invalidation policy WRITTEN into the test module docstring — the #2070 knob series (A1-A7 pool/boost/rerank knobs) + any render_context/_render_block text change invalidates the goldens and forces re-capture + re-review, never reflexive regeneration — plan-review P2); flag-ON + fired → assembled evidence under caps + alignment invariant + exactly ONE model call; both-flags-on → why survives on real evidence points, synthesized rows contribute none; enrich/decorate FAILURE degrade pinned: W4-flag-on fired path with enrich_items raising (or a real point row missing) → typed degrade (assembled block without why keys / AskRetrievalUnavailable), never a raw raise or partial block (plan-review P2); fired-but-unresolved → legacy fallback no error; STAGE-LEVEL degrade: malformed-date row seeded on the fixture → typed AskRetrievalUnavailable or undated-tier fallback, never a raw raise (plan-review P1); qtype on the wire unchanged (incl. caller override case pinned — R12 bullet, not R17/P3-1 — plan-review P2); hosted Event-aboutObject-present graph test (R6); R16(b) OUT-OF-SUBGRAPH gold: one fired-shape question whose gold evidence lives OUTSIDE the resolved subjects' subgraphs, asserting/reporting the A-vs-B admission delta it produces (pool-replacement starvation check — plan-review P1); regression: test_ask_sdk.py / test_ask_api.py / test_reader.py / test_retrieval.py / test_ask_retrieval_levers.py green; ASK_ASSEMBLED CONTRACT pinned (plan-review P2): signature (question, *, question_date=None, question_type=None, caps override?, _reader_factory seam for the eval arm's pinned/stub reader), return shape (recall_*-list precedent or ask-dict — CHOOSE + pin with a shape-asserting failing test), performs the ONE reader call ONLY when a reader is supplied (pure assembly otherwise → returns slices + post-cap lines + admission counts for the eval arm), honors the same fired-gate (fired=False → returns assembly=False + no block), and in hosted-delegated client mode (TORTOISE_API_URL set, no local graph) raises AskRetrievalUnavailable with a clear message (documented in answer-surface.md) — with a TORTOISE_API_URL-set test.
**Files:**
...
**Step 1:** Failing test: gold turns across ≥2 sessions both in the post-cap line list on the fired shape (v2-lane graph); legacy control admits <2 AT THE PINNED WIDENED KNOBS; R9 substrate (≥20 same-subject sessions, gold marked deep-rank) built from the Task-1 builder. **Step 2:** RED. **Step 3:** Wire the arm + register the file in ci-surfaces.yml (same commit). **Step 4:** Green incl. A-default vs A-widened admission inequality assertion + matched-control deltas + oracle-tier-or-pre-registered. **Step 5:** Docs (ONTOLOGY supersededAt; answer-surface note incl. real-lane caveat). **Step 6:** Full acceptance: docker-lane targeted suites + `uv lock --check` + ruff + `ci_selection.py --integrity`. **Step 7:** File the census-driven follow-up issues; commit + plan-status note.
