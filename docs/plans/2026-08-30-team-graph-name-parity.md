---
title: "Plan — #1903 dashboard-created teams provision graph_name=org_{org_id}"
type: engineering
domain: capability
doc_status: live
created: 2026-08-30
...
# Issue #1903 — Dashboard-created teams: mint graph_name = org_{org_id}

> **For Pi:** Use `executing-plans` to implement this plan task-by-task.

**Goal:** Dashboard-created teams (POST /v1/organizations, POST /v1/onboarding/team) mint `graph_name = org_{org_id}` so export/backup resolve the real data graph and delete drops it instead of orphaning it.

**Team:** epistemic-team
**Role:** product-implementer

**Architecture:** Naming-parity fix at the two Supabase-lane provision mint sites. `_create_team_supabase_lane` (hosted_api.py:6170) and `_create_onboarding_team_lane` (hosted_api.py:10632) currently mint `f"org_{name}"`, but every data-plane op resolves `_make_sdk(namespace=org_id)` → graph `org_{org_id}`. Export (`_team_namespace` :7394), backup (`teams.graph_name`), and delete (`_drop_team_graph` with stored name) all consume the STORED name — minting `org_{org_id}` makes the stored name equal the actual data location, fixing all three surfaces at the root with a 2-line production change. Registry lane (`sdk.team_create` → `org_{name}`) and the pre-fix fleet are tracked separately in #2023 (out of scope by issue + scope decisions).
...
> Gate skipped: plan touches zero third-party deps. Naming-parity change with in-repo precedent — `register_user` (hosted_api.py:3307), `agent_signup` (:8569), `provision_tenant` (:917) already mint `org_{org_id}`. No external knowledge needed.

### Integration Surface Map

| Surface | Layer | Test assignment |
|---|---|---|
| POST /v1/organizations → `_create_team_supabase_lane` mint | integration (fake control plane + temp embedded DB) | test_writer_inventory.py `test_create_team_user_path` — assertion update; new export/delete round-trip in test_export_delete.py |
...
| provision_team RPC `p_graph_name` | integration | same tests assert `p_graph_name == f"org_{org_id}"` |
| GET /v1/organizations/{id}/export → `_team_namespace` | integration | test_export_delete.py new dashboard-created-team export round-trip |
| DELETE /v1/organizations/{id} + `_purge_deleted_teams` → `_drop_team_graph` | integration | test_export_delete.py new dashboard-created-team delete round-trip (spy on `_drop_team_graph_strict` arg) |
...
## Task 1: Mint org_{org_id} in both dashboard-lane provision endpoints
...
> Side-effect note (positive): the eager TeamMeta create (:6114) and `_journal_append_product` (:6118) follow the minted name — they now land in the REAL `org_{org_id}` graph instead of a phantom `org_{name}` graph; the mode-aware `graph_list` / `_team_namespace` default-graph derivation (from stored name) also now resolves the real graph. No test asserts the old behavior; no consumer depends on `org_{name}`.

## Task 2: Update assertions that pin the old org_{name} values

**Intent:** Tests must reflect the corrected contract (dashboard-lane teams mint org_{org_id}), not the buggy behavior.
...
**Step 2:** `test_subteam_provisions_via_rpc` (:799): `assert body["graph_name"] == "team_subteam"` → `assert body["graph_name"] == f"org_{body['org_id']}"`; add `assert p["p_graph_name"] == f"org_{body['org_id']}"`.
...
def test_dashboard_created_team_export_returns_points(self, sb_client, as_user):
    ...
    assert r.json()["graph_name"] == f"org_{org_id}"  # Indicator 1
...
**Intent:** Prove Indicator (3) — delete targets the org_{org_id} graph (not the old org_{name} → orphan).
...
def test_dashboard_created_team_delete_drops_org_id_graph(self, sb_client, as_user, monkeypatch, capture_audit):
    ...
    assert (org_id, f"org_{org_id}") in dropped  # Indicator 3
...
**Files:** per commit-workflow skill (commit message `fix(data): #1903 dashboard-created teams provision graph_name=org_{org_id}`).

## Rejected Alternatives

- **Option B — make the data plane resolve org_{org_id} everywhere** (`_team_namespace` + drop callers + backup sweep): breaks documented registry stored-name semantics (test_export_uses_stored_graph_name, PR #873) and touches more surfaces. Rejected in scope-verify (P1 analysis).
- **Option C — change sdk.team_create to mint org_{org_id}**: broad blast radius (~6 test files + CLI/MCP/embedded callers); selfhost-only; tracked in #2023. Rejected for scope.
