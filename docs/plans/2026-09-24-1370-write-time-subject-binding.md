...
1. **The binder is the ONLY confidence-gated `aboutSubject` producer.** The legacy
   ...
   (local point loop, hosted point loop, hosted event loop). This closes the review P0.
...
3. **Journaled carrier = existing `EntityLinked` + optional `confidence`.** No new
   ...
   live-only, as today). Refusals use a JSONL-only, non-folding `EntityBindingRefused`
   audit record, registered in `_NO_PROJECTION_FOLD`.
4. **Tri-state gate, fail-closed.** `bound ≥ τ_hi` / `suspected ∈ [τ_lo, τ_hi)` /
   `unbound < τ_lo`. Defaults `τ_hi=0.7`, `τ_lo=0.4`. Non-numeric/None/NaN ⇒ unbound.
   Unknown/non-subject kind ⇒ unbound (never invented). Unresolvable name ⇒ unbound,
   no stub.
...
| 2 | `subject_binding.is_subject_kind` | Pure logic | Internal | Unit | bare-name ∈ `SUBJECTS`; `other` excluded | namespaced vs bare; unknown pack kind; `other` |
...
**Acceptance:** `is_subject_kind` true for the 5 declared subject kinds (bare/formatted), false for `other`/objects/unknown; `decide_binding` returns bound/suspected/unbound at the exact boundaries and unbound for non-numeric/NaN.
...
**Acceptance:** a point with a ≥τ_hi subject slot gets `(Point)-[:aboutSubject]->(:Subject)`; <τ_lo writes none; kind routing makes the entity a `:Subject` and no `:Object` duplicate; subject-kind names in `about_entities` write no edge and mint no stub.
**Files:** Modify `tortoise/sdk.py`; Test same file.

## Task 4 — hosted seam parity

**Intent:** the hosted commit path binds identically.
**Acceptance:** the same slot payload driven through `_execute_commit_writes` yields the same binding records/edges as the local seam.
...
**Acceptance:** `audit_subject_binding(graph)` reports the fractions; `tools/subject_binding_audit.py --gold` prints the misattribution + unbound rates and a `--calibrate` sweep; a test pins the default-threshold misattribution rate on the authored fixture at ≤10%.
