...
6. Id-reuse drop-pre-recreation via the RAW hard-delete lane (raw producer delete — originally chosen because SDK `delete_point` emitted a tombstoning `PointRetracted`. Superseded 2026-09-22 (#3300): `delete_point` now journals a hard delete, so the choice here is about exercising the raw producer, not about the SDK door.)
