...
- `GET /v1/events?after=<cursor>&types=a,b&limit=100` (hosted_api) — auth `Depends(get_current_team)`; team SDK `_make_sdk(namespace=team["org_id"])`; responds `{"events": [...], "next_cursor": "..."}`.
...
class TestEventsPoll:
    ...
    def test_tenant_isolation(self, client, internal_client):
        ...
        app.dependency_overrides[get_current_team] = lambda: dict(TEST_TEAM, org_id="iso-evt-a")
        try:
            r = client.get("/v1/events")
...
- HTTP-mode team scoping: `_get_team_sdk()` (mcp_auth.py:58) already returns `TortoiseSDK(namespace=org_id)` from the request-scoped ContextVar — poll/retract inherit isolation; stdio mode uses namespace `None` → `"default"` team.
