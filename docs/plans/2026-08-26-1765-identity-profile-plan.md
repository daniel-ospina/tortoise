...
| 7 | `org_memberships.identity` reg- UNIQUE partial index | DB | Write (constraint) | SQL suite | `WHERE user_id IS NULL AND role='owner' AND status='active'` | Pre-existing duplicates → pre-scan/abort; 23505 → 409 |
