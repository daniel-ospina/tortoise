...
| 11e | Authz (team-member until W10) | Auth | Guard | Integration (docker lane) | DELETE + detail GET dual-auth (session JWT OR tt_ key); session user without membership in ?org_id= team → 403 (`_session_user_org`); key auth team-scoped by resolution |
...
- `@app.delete("/v1/sessions/{session_id}")` with `Depends(get_current_org_session_ungated)` (dual-auth team-member; session membership validated in `_session_user_org`).
