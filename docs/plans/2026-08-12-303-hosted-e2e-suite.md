...
**Tech Stack:** Python 3.12, pytest, pytest-playwright (APIRequestContext), uvicorn subprocess, embedded FalkorDBLite, `cryptography` (ES256 mint, transitive dep — verified in uv.lock).
...
**JWKS mock:** in-process `http.server` thread serving EC JWKS at `/auth/v1/.well-known/jwks.json` (URL pinned at server import, first fetch lazy on the first session-auth request — the mock must be up before the first JWT request, not before server boot); conftest mints ES256 JWTs (`cryptography`; claims: `iss={SUPABASE_URL}/auth/v1`, `aud=authenticated`, `sub=<user_id>`, `exp=+1h`, `kid` matching; PyJWT ES256/RS256 verify, exp leeway 30s). The pytest process NEVER opens the server's DB file (redislite single-writer hazard, tests/conftest.py:96-101) — all verification is HTTP.
