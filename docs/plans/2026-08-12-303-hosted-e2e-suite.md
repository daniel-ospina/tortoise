...
10. **Step:** Agent session captured → **Acceptance:** turns extracted as Points (regex mode) → **Test:** E2E-10-D
...
| E2E-10-D session capture | POST /v1/sessions (regex mode) → session stored; GET /v1/sessions(+/{id}) shows turns→Points extraction | turn cap exceeded → 400; oversized turn content → 422; unauthenticated → 401 |
