...
**What the graph returned** (122 component Objects ingested into a dedicated namespace `r1_capability_registry`, never the `tortoise` graph; the lever-(i) probe then registered its two endpoints `ux-consistency` and `ux-coverage` as Objects *in addition*, so a later `count(o)` read **123** — **the 122/123 ambiguity is between "components ingested" and "Objects present after the probe", not between enumerators**) — `tortoise_fts_query(entity_type="object")`:
...
**Reading it honestly — the comparison is unfavourable to the graph, but it is NOT the verdict.** The table below is retained because it is a real measurement of the graph's lexical leg, but **both columns are computed on the mislabelled fixture, the two recall columns are not the same function, and the graph column cannot be recomputed.** It is therefore **not** the lever-(ii) test, and the "Winner" column has been removed. What it supports is a single qualitative statement: **on this corpus the graph's FTS leg showed no lexical advantage over a two-line Jaccard baseline.**
...
2. **There is no semantic leg for Objects at all** — see below. This is the finding that survives everything.
...
- **2026-09-11T00:33Z** — *[R1]* **lever (ii) — NOT ESTABLISHED.** `MATCH (o:Object) WHERE o.objectKind='skill' RETURN count(o)` → 123 (122 components + one probe node). This is a **filter**, not composed pack/kind retrieval, and it was never compared to the ranked-queue baseline — constraint 1's stated bar. **The pre-registered test for lever (ii) was not run** (cycle-2 P0).
...
### Cycle 3 — 1 P0 + 3 P1 + 6 P2 (after cycle-2 fixes)
...
**Correction of record (intra-cycle).** Three cycles, three P0s, and each was introduced or missed by the *previous* fix cycle. The pattern is worth naming: every one of them moved the brief toward **a cleaner, more decisive story than the evidence supports** — a manufactured verdict, a manufactured root cause. The gate's value here was not catching arithmetic; it was repeatedly refusing a tidier answer. The brief now reports an unresolved kill-switch, an unestablished lever (i) as well as (ii), a contested ground truth, and a non-reproducible half — which is what actually happened.

### Cycle 4 — 1 P0 + 5 P1 + 6 P2 — ⚠️ **capped at 4 cycles; 5 P2s remain unfixed**
...
**⚠️ Capped at 4 cycles (AGENTS.md review-loop hard cap). 3 P2s remain unfixed.** Items 9–11 are all cosmetic or precision-only; **no open finding is P0 or P1**, and none of the three changes a conclusion. They are recorded here rather than fixed so that a later reader can see exactly what the gate left on the table and does not mistake the brief for gate-clean. **This brief did NOT exit the review gate with `NO ISSUES FOUND`.**

**What the four cycles actually did.** They produced **four P0s, and every one was an over-claim in the same direction — the brief telling a cleaner, more decisive story than the evidence supported:**

1. *Cycle 1* — published a hand-written tokeniser's output as Align's baseline method.
