...
**Low ⚠️ single-publisher:** the **generation-counter mechanism** — its entire evidential base is the Linux `open_by_handle_at(2)` man page + `include/linux/exportfs.h`, and it is the claim that arrived through the **aborted third review gate**. Also Low single-source: Aura and Skir as the only candidate rename-continuity mechanisms (Aura is productized — but is a competing product with authoring interest disclosed; Skir is design-stage and not shipped).
