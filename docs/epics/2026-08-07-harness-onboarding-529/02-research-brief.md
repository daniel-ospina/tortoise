...
| Analytics | `_track_analytics_event` + `_ALLOWED_ANALYTICS_PROPS` (already contains `harness`, `section`) | ✅ Pipeline exists; **copy beacon is dead** — `copyMcpConfig()` fires `fetch(…, {method:"PATCH"})` with no body AND no Authorization header; the PATCH handler requires Bearer auth (`get_current_org`) → 401, nothing ever recorded |
