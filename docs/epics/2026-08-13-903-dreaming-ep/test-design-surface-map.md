...
| 2 | `tortoise/mcp_server.py` dream/health tools | API | In | Integration | `dream` tool gains optional mode param; `dream_health_check` tool (alarm verdict + metrics). ~~staleness_report tool~~ — **cut/deferred at human gate 2**: the `lastDreamedAt` read property remains in scope; the internal staleness-ranked query helper is owned by C3. Child issue #1249 derives its checklist from THIS row — the cut tool is NOT in scope. | Tool param mismatch with SDK; tool registry collision (epic #888 consolidated the surface — additive change) |
| 3 | `tortoise/ep.py` `TortoiseEP.run(warm_start=True)` | State mutation / Concurrent | Both | Unit + Integration | **Warm-start reuses the ALREADY graph-persisted edge messages** (`msg_alpha`/`msg_beta`/`back_msg_alpha`/`back_msg_beta` — `_load_cache`/`_flush_cache` today; NO new cache store per plan W3/Architecture): seed from graph, skip message updates with delta ≤ fixed threshold γ, flush back; invalidation on topology change, edge TRANSFER (supersede/invalidate/approve_merge), and evidence/baseline writes; keying = `(op_id, claim_id, rel_type)` + forward/back slots. | Race: fast-path `compute_confidence` must not engage γ-skip state (warm_start flag default False — only Dreamer-locked runs enable it); stale seeds after transfer/baseline (invalidation must fire — DE2E-6a negatives b/c); threshold too loose → beliefs drift (parity gate DE2E-6a); non-converged-run flush seeds (DE2E-6a negative d) |
...
- **Atomic writes:** `confidence` + `lastDreamedAt` must be set in the same Cypher write (existing dream.py L112 pattern extended) — never two writes that can interleave.
