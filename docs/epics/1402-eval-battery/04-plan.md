...
| CLI | `battery run --tier 1|2|3 --arms ...`; `battery parity`; `battery calibrate --print`; `battery validate-judge --rubric <id>` | exit codes: 0 ok, 2 gate-blocked, 3 inconclusive, 4 arm-failed, **5 empty-corpus (refuse to start)** | Subcommand flags versioned in help |
