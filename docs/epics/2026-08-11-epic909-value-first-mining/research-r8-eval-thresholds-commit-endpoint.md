...
**Metering:** keep `write_ops` (+1 per NON-duplicate commit call — the published billed unit, unchanged; L1 replays bill zero; overflow-to-hold commits bill zero and their re-submission bills the single +1 — one logical payload billed exactly once, plan resolution R4); add `nodes_written` (+net-new non-episodic, post-dedup — the cost-driver unit) on the existing MeteringRecord. Do NOT switch the billed unit to node-delta in v1. Prevents the 25× per-node arbitrage vs create_point.
