...
- 2026-08-20 — web (data-model hook): Hindsight (arXiv 2512.12818) typed-fact classification (world/experience/opinion/observation) at extraction + type-aware retrieval + background observation consolidation ≈ our Tier A/B + E7; dual temporal axes (occurrence + mention) validate `when` vs `createdAt`; EverMemOS 7-type taxonomy over-engineers (4 backends, no transactions); redhat memory-hub survey counterpoint: type earns its keep at extraction (selection), value must live in the text. Conclusion: Tier-A marker = classification hint (extraction selection + optional retrieval bias), NOT per-type pipelines.
