...
> Embedded remains the default; `TORTOISE_DB_URI` is not set anywhere in P1, so every new mechanism is **dormant**. Acceptance: full 6,837-test embedded suite green (identical to baseline — **cycle-6 P2-11/P2-17: 6,837 is the pre-P1 baseline; the P1 tasks ADD ~44 unit/conformance tests (redirect_seam 16, wipe_server 10, skip_guard +9, round_trip_parity 2, divergence_conformance ~6, bare-test control 1), so the P1 Gate's item 1 states the post-addition measured total; the epic issue's 5,969 is recorded STALE (research-brief §5: "the epic's 5,969 does not reproduce" — verifier-reproduced 6,837 across 3 venvs)**), redirect/wipe/manifest unit-tested, no orphan delta.
...
# breaks the sweep's contract). The tests-side in-memory _JOURNAL/_WIPED_UP_TO
# delta stays tests-side only (P2-13).
```
...
Verify (cycle-3 P2-13 + cycle-4 P2-3/P1-8 + cycle-5 P2-4 + cycle-6 P1-4 — unit, not just manual two-process): **`test_concurrent_suite_end_sweep_leaves_other_suite_graphs`** with a FAKE `list_graphs`/marker dir: suite A's end-sweep (journal A) must leave suite B's live graphs (marker B present) untouched; with B's marker removed (B crashed), A's stale sweep drops B's journaled graphs. **`test_one_session_two_markers_counts_as_one_suite` (P2-4/P2-16):** a session holding BOTH its docker-format marker AND its embedded-format marker (the real docker-session shape) counts as ONE suite — its end-sweep runs its own journaled-graph sweep despite its OWN second marker being present (the "other suite" predicate is PID-grouped — same pid, different tokens = one suite). **Cycle-6 P1-4 (FM-5) — the test is EXTENDED to pin `_redislite_hygiene`'s own end-sweep against the docker marker:** `_redislite_hygiene` (conftest L286-300, unmodified) removes its OWN embedded marker then computes `others = [t for t in active_suite_tokens() if t != token]` — the session's own docker-format marker (different `{pid}-{uuid8}` token, same pid) counts as "other" → the embedded end-sweep degrades to `only_safe` → the 6 P2 carve-out stems in half b spawn REAL embedded servers that survive → E2E-7's orphan assert reds. FIX (chosen Good): the extended test runs `_redislite_hygiene`'s end-sweep with the session's own docker marker present and asserts `others == []` (pid-grouped predicate: a same-pid marker is own regardless of token) — the EXECUTABLE pin; AND the conftest fixture ordering is pinned so the docker session-end sweep (which removes the docker marker) tears down BEFORE `_redislite_hygiene`'s end-sweep (declare the docker session-start/end fixtures AFTER `_redislite_hygiene` in conftest.py — pytest tears session fixtures down in reverse setup order, so docker teardown precedes `_redislite_hygiene` teardown; the ordering alone cannot be asserted by a test, which is why the test-side predicate extension is the primary pin). **`test_journal_tolerant_reader_truncated_line` (P2-3):** a journal whose last line is a truncated half-name (simulated torn write) parses to the complete prefix lines only, and the stale sweep drops exactly those; a journal whose FIRST line is unparseable is treated as empty and deleted (poison guard). **`test_allow_remote_session_teardown_green` (P1-8):** the ALLOW_REMOTE session above. The manual two-process check stays as the integration confirmation.
...
1. Full embedded suite green: `uv run pytest tests/ -q` → **post-addition total ≈ 6,881** (cycle-6 P2-11: the 6,837 pre-P1 baseline + the ~44 new P1 tests — test_redirect_seam 16, test_wipe_server 10 (incl. the cycle-6 command-vector test), test_skip_guard +9 (incl. the 6399-exemption test), test_round_trip_parity 2, test_divergence_conformance ~6, test_wipe_server's bare-test control 1 — count the ACTUAL collect-only number at the gate; cycle-6 P2-17: the epic issue's 5,969 is recorded STALE, 6,837 is the verified baseline, research-brief §5).
...
`EXPECT_URI` is set **IFF** `URI` is set (both gated on `full == true && half == b`, computed from the SAME `$URI` variable) — a tier-2 PR (`full=false`) creates half-b jobs with `EXPECT_URI=0` and no URI, so the tripwire never fires on the tier-2 shape. **Pinned by a YAML-parsing test added to `tests/test_ci_selection.py` after this edit:** parse `python-ci.yml` (the existing `workflow_matrix_issues` pattern), locate the run step's env block, and assert `EXPECT_URI` is set iff `URI` is set (same gate expression, same branch).
...
| P2-11 | P2 | Stale gate counts (6,837 baseline) don't include ~45 new tests (redirect_seam 16, wipe_server 10, skip_guard +9, round_trip_parity 2, divergence_conformance ~6, control 1) | P1 Gate item 1 states the post-addition measured total (≈6,881, collect-only at the gate); Task 1 Step 6's +2 comment noted as commit-local | Phase 1 Gate item 1, Task 1 Step 6 |
