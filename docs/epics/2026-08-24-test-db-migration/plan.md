...
- **E2E-7** — orphan-assert CI step re-targeted per surface; **cycle-4 P1-9 adds the server-side GRAPH.LIST growth bound** (docker sessions end with `GRAPH.LIST` count < journal size + constant — DETACH-only wipes empty but never remove graphs, invisible to the redislite-process count; **cycle-5 P2-3: capture the journal size BEFORE the session-end sweep — the sweep deletes the journal, so the bound's "journal size" must be read pre-sweep and compared to the post-sweep `GRAPH.LIST` count**). **Gates:** P3 (docker ~0) → P4 (reaper demotion; dev-machine <20 without reaper).
...
def test_redirect_connection_boundedness(uri_env):
    ...
    _other_pids = {m for m in active_suite_markers()}
    _my_pid_prefix = f"{os.getpid()}-"
    _info_leg_runs = not any(not m.startswith(_my_pid_prefix) for m in _other_pids)
    pools = []
    for i in range(20):
...
def test_per_test_wipe_or_touches_only_session_set(server_proj, monkeypatch, tmp_path):
    ...
    _wipe_or(server_proj)  # first wipe: cursor advances past mine
...
# (ACTIVE_SUITES_DIR/{TORTOISE_TEST_SESSION}.graphs.jsonl); `_journal_append`
# writes it, `_read_journal` parses it — the wiring test patches it to a tmp
# path so the REAL file-journal path is exercised.
```
...
Add `namespace="test_<file>_<uuid>"` to no-namespace `TortoiseSDK` bulk-wipe/exact-set constructions in migrated files (or route through the URI-aware `sdk_factory`); `namespace="registry"` → `test_registry_<uuid>` (test_invites_http — per-test uuid, so the file's 36 tests share within a test but never across tests); `namespace="team-a"` → `test_<file>_<uuid>` (test_pack_state); `graph_name="team_team-k"`/`"t"` fall under the Step 4 derivation rule. Verify the SDK's `_get_proj` emits `test_*_tortoise` graphs, and add a wipe_server skip assertion for the swept names (a seeded `registry_tortoise` survives `wipe_server` untouched — extend `test_wipe_server_clears_only_test_prefixed`). **Registry-guard test (cycle-5 P1-5):** `test_namespace_literals_guard_passing_or_routed` in `tests/test_markers.py` — grep every `namespace="..."` literal in NON-allowlist (migrated) test files; assert each is either guard-passing (`test_`/`tortoise_test`/`test-` hyphenated — SDK-normalized per the acceptance bullet) or listed in the per-file routing table (`ROUTED_NAMESPACES: dict[file, dict[literal, replacement]]` — the Task 7 seam route; an intentional team-graph semantic must be DECLARED in the table, never silent). A new `test-*`/non-test literal in a migrated file reds the guard — this is the census's executable form (the 4th-cycle gap closed: registry's 19 files, e2e-900's 5 files, and the `team_test-*` hyphenated family all trip it until routed).
...
7. **Artifact upload + step-wall emission (cycle-5 P1-7/P2-5):** the upload step currently ships ONLY `/tmp/pytest.log` (VERIFIED python-ci.yml L452-455) — it gains `/tmp/junit.xml` (the canary classifier's half-b input, Task 9 Step 6). The pytest run step gains an `id:` (e.g. `id: pytest-run`) and emits `echo "step_wall=$SECONDS" >> $GITHUB_OUTPUT` at its end — cycle-5 P2-5: python-ci.yml has NO `$SECONDS`/`$GITHUB_OUTPUT` in the run step today (VERIFIED by grep), so the cycle-3/4 claim of "the existing in-step SECONDS accounting the watchdog already uses" is a false premise — the capture must be ADDED, not exposed; `steps.pytest-run.outputs.step_wall` feeds E2E-5's gate and the canary classifier.
...
4. Orphan assert on docker halves ≈ 0 (E2E-7); **server GRAPH.LIST count < journal size + constant after session-end/stale sweeps (cycle-4 P1-9 — the DETACH-only accumulation bound; cycle-5 P2-3 — journal size read BEFORE the sweep, compared to the post-sweep count)**.
