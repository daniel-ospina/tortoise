...
| CI-8 | — | #3634 Task 3: the residue opt-in is a NEW narrow `== "1"` env read, which GROWS `_KNOWN_NARROW_READS` — a ledger whose guard documents it as able to *only shrink* | Ledger entry added with an `OVERRIDES (#3634)` reason; the ledger docstring amended to "shrink by default; a new entry requires its own recorded decision"; the opt-in is pinned to appear nowhere in `python-ci.yml` | None |
