...
def test_team_registry_isolation_across_sequential_tests(server_proj, monkeypatch):
    ...
    for i in range(2):
        ...
        assert bs.team_graph_name(None, "team_x") == f"test_org_{i}_tortoise"
...
1. **P1-5** — the consumption seam's coverage is 2 of ~22 registry-mode tests (verified: 27 `run_backup_sweep(` call sites; `team_graph_name` registry branch is deterministic `org_{id}`), so half b reds at P2 without a file-wide fixture.
