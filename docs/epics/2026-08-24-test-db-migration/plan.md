...
```bash
uv run python tools/ci_selection.py --register --surface core  # cycle-2 P1-3: new test files must be manifest-listed
...
def test_team_registry_isolation_across_sequential_tests(server_proj, monkeypatch):
    ...
    # from org_graph_name() (backup_sweep.py:190, call at :514) — on docker
    ...
    # consumption: monkeypatch tortoise.backup_sweep.org_graph_name to the
    ...
    for i in range(2):
        ...
        team = server_proj.db.select_graph(f"test_org_{i}_tortoise")
        ...
        assert bs.org_graph_name(None, "team_x") == f"test_org_{i}_tortoise"
        assert f"test_org_{i}_tortoise".startswith(("test_", "tortoise_test")), \
...
```bash
uv run python tools/ci_selection.py --register --surface core  # cycle-2 P1-3: test_wipe_server.py
...
```bash
uv run python tools/ci_selection.py --register --surface core  # cycle-2 P1-3: test_divergence_conformance.py
uv run pytest tests/test_ci_selection.py -k integrity -q
...
```bash
uv run python tools/ci_selection.py --register --surface core  # cycle-2 P1-3: test_backend_identity_tripwire.py
uv run pytest tests/test_ci_selection.py -k integrity -q
...
```bash
uv run python tools/ci_selection.py --register --surface core  # cycle-2 P1-3: test_markers.py
uv run pytest tests/test_ci_selection.py -k integrity -q
...
5. Skip-guard step: `RC=$(cat ${RUNNER_TEMP:-/tmp}/pytest-rc)`; skip when `$RC != 0` or the files file is empty; else `uv run python tools/skip-guard.py /tmp/pytest.log --junitxml=/tmp/junit.xml --manifest <expected-nodeids>`.
...
| P1-3 | P1 | New test files (test_redirect_seam, test_wipe_server, test_markers, test_backend_identity_tripwire, test_divergence_conformance, test_round_trip_parity) never registered in `config/ci-surfaces.yml` → the `--integrity` drift gate reds | `uv run python tools/ci_selection.py --register --surface core` added to every creating task's commit step (verified: `--register` exists at tools/ci_selection.py L456) | Task 1 Step 6, Task 2 Step 8, Task 4 Step 5, Task 5 Step 4, Task 8 Step 3
...
| P1-1 | P1 | P0-1a seam routes only the SEED; consumption (the sweep's per-team graph name from `org_graph_name()` backup_sweep.py:190/:514 → `team_team_x`, then `_backup_team` select_graph :264) still dumps the EMPTY derived graph on docker while data sits on `test_team_<uuid>_tortoise` → content assertions fail at P2 (test_backup_sweep rides half b) | Seam must ALSO route consumption: docker-lane tests monkeypatch `tortoise.backup_sweep.org_graph_name` (or `enumerate_orgs`) to return the seam names per test (`test_team_registry_isolation_across_sequential_tests` + new `test_sweep_consumes_seam_team_graph`); P0 guard in `_backup_team` asserted (consumed name stays `test_`-prefixed) | Task 2 Step 6b, Step 1 tests
| P1-2 | P1 | backup_sweep census incomplete (20 of 29 sites): missed L452/475/510/533/538/603/606 (select_graph), L744 (select_graph), L745 (assert org_graph_name) | Route by regex sweep (`select_graph("registry_control_plane")` / `select_graph("team_team_x")` / `org_graph_name` grep + dynamic `f"org_{org_id}"` L57 / `team_team_e` L233 / `team_myapp` L770/860 / `team_beta` L816), not an enumerated list | Task 2 Step 6b
...
| P1-5 | P1 | backup_sweep consumption seam covers only 2 of ~22 registry-mode sweep tests: `org_graph_name` (backup_sweep.py:190) registry branch returns deterministic `org_{id}` (L246) and `run_backup_sweep` consumes it at :514 — the two cycle-3 docker-lane tests monkeypatch it individually; the other ~20 ride half b and dump the EMPTY derived graph (seam-seeded data sits on `test_team_<uuid>_tortoise`) → red | Consumption routed FILE-WIDE: a module-scoped autouse fixture in test_backup_sweep monkeypatches `tortoise.backup_sweep.org_graph_name` to the seam constants for every sweep test (the patch can't live in `_make_env` — its monkeypatch arg is None at L162, verified); the L745 contradiction resolved explicitly: `test_team_graph_name_reads_from_teams` (tests the REAL function) is exempted from the autouse patch by name, so the real-function semantics stay covered and the patch isn't vacuous | Task 2 Step 6b
...
1. **P1-5** — the consumption seam's coverage is 2 of ~22 registry-mode tests (verified: 27 `run_backup_sweep(` call sites; `org_graph_name` registry branch is deterministic `org_{id}`), so half b reds at P2 without a file-wide fixture.
...
| CI-7 | — | #3634 Task 3: 723 `registry_test_*` + ~31 eval-residue graphs have NO journal left, so neither the journal sweep nor `wipe_server`'s test-prefix filter can attribute them — an unreachable leak the epic's own CI-3 mint produced | Opt-in `_sweep_legacy_strays` (`TORTOISE_TEST_SWEEP_LEGACY == "1"`), deleting only `_LEGACY_RESIDUE_PREFIXES` (proved disjoint from every owned family); NO default call site, AST-pinned; live leg asserts a shared-registry-shaped name survives | **OVERRIDES #7795** — name SHAPE authorises an irreversible journal-blind delete, permitted only because the gate is explicit and the set is disjoint; the gate must be set manually (never in CI — pinned) |
| CI-8 | — | #3634 Task 3: the residue opt-in is a NEW narrow `== "1"` env read, which GROWS `_KNOWN_NARROW_READS` — a ledger whose guard documents it as able to *only shrink* | Ledger entry added with an `OVERRIDES (#3634)` reason; the ledger's GOVERNING sentence now states the rule it follows — shrink by default, and adding an entry requires its own recorded decision (`OVERRIDES:`) and is itself a change to the contract (the two deliberate exceptions are the tenant-namespace opt-in and this one); the opt-in is pinned to appear in NO workflow's job/step `env` map or `run` script (all of `.github/workflows/`, structured YAML scan — not just `python-ci.yml`) | **OVERRIDES #3634** — `_KNOWN_NARROW_READS`'s "the ledger can only shrink" invariant (#4097's truthy-set contract): `TORTOISE_TEST_SWEEP_LEGACY` ADDS one narrow `== "1"` entry, because an irreversible journal-blind delete must not ride the wider truthy set; the growth is deliberate and is pinned by `tests/test_wipe_server.py::test_legacy_sweep_gate_is_narrow_by_design` |
| CI-9 | — | #3634 Task 5: the E2E-7 gate was the plan's whole-server `GRAPH.LIST count < journal_size + 20` assert — a server-wide count cannot distinguish this session's owned residue from pre-existing/foreign graphs, so it false-positives a clean session and is blind to a leak a smaller foreign count masks | `_owned_survivors(journal_names, live_names, default)` = owned ∧ journalled ∧ ¬default ∧ still live, mirroring `_sweep_drop`'s skips; the whole-server count stays a `print(... WARNING ...)` and `server-hygiene-end.json` is still written; `journal_size = len(_read_journal())` is untouched for the bound | **OVERRIDES #3634** — the plan's whole-server count assert as the gate; a count is not the property claimed (owned names gone), and an owned-survivor set test is exact on it. Live-fixture demo + unit tests in `tests/test_server_hygiene_gate.py` [CORRECTED — Task 5 review: there is NO live-fixture demo committed, and the original AST pin was weak. What IS committed: unit tests for `_owned_survivors`; TWO AST pins — (i) the gate's OWN `journal_names = set(_read_journal())` capture must precede the sweep (the earlier pin matched ANY `_read_journal` call, so the whole-server bound's `journal_size = len(_read_journal())` satisfied it and moving the gate's capture after the sweep still passed), and (ii) the gate's `Raise` has no `Try` and no `full_sweep` ancestor; and hermetic FIXTURE-DRIVEN tests that call the real `_server_graph_hygiene` generator with every server call faked (raises on an owned survivor; silent when `failed`/`error`/`skipped`; silent on preserved/default/foreign names; probe failure is an infra skip)] |
| CI-10 | — | #3634 Task 5: an unconditional E2E-7 assert over every post-sweep journalled name would break the log-and-continue policy (epic cycle-8 P2-3/P2-4) — a transient delete error must not red the suite, and a RAISED sweep yields `own={"error": ...}` with no `failed` key | Gate short-circuits on `not own.get("skipped") and not own.get("failed") and not own.get("error")`; sits outside the broad `except Exception` (or `AssertionError` is swallowed and the gate is vacuous) and is a SIBLING of the bound-check `if` — a direct child of `if not others` (last-suite-standing), gated ONLY on the three own flags and NEVER on the bound's `full_sweep` condition [CORRECTED — Task 5 review: the gate was originally nested INSIDE the bound-check `if`, so it inherited `full and full.get("full_sweep", False)` and was DISABLED exactly when the leftover sweep failed or reported `full_sweep=False` — i.e. precisely when cleanup was incomplete and survivors are most likely; it is now unnested]. The gate's `_live_graph_names` probe is wrapped in its own `try`/`except` (print-and-continue, the bound check's idiom) so a connection/auth/maxmemory/stall failure reads as an infra skip rather than reding the suite indistinguishably from a real leak. | **OVERRIDES #3634** — an unconditional assert over post-sweep names; the residual is the UNJOURNALLED mint — a graph materialized with no ownership record at all (LC4/LC7, #5048) is outside the owned ∧ journalled set by construction — plus the concurrent-suite window (`if not others`: the gate does not run while another suite is active). NOT a residual: a *silent-success* drop (`_drop_one_graph` returns `True` but the graph survives) IS gated — the capture is pre-sweep and the post-sweep live probe never consults `own["dropped"]`, so the surviving owned name raises. AC4 is AST-pinned, not session-observed (`_run_session` pops `TORTOISE_TEST_JOURNAL_FILE`) — the pin targets the gate's OWN `journal_names` capture, so the bound's `journal_size` read cannot satisfy it |
