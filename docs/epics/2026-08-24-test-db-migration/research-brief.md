...
**Verification baseline:** 6,837 tests collected, 3 docker FalkorDB containers running locally, 4 live redislite orphans, CI job walls captured from post-#1645 runs.
...
|---|---|---|
| Collected tests | **6,837** (40.60s collect, verifier-reproduced) | 6,837 ✓ |
...
| Carve-out tests (17 files) | **339** + 3 busy-error per-test = **342** (5.0% of 6,837) | ~16–18 files ✓ |
...
|---|---|---|---|---|---|
| D1 | `_is_embedded` seam (L390) | `self._is_embedded = (path is not None)` — the mode flag itself | — | — | No change; the flag remains the mode discriminator |
| D2 | `_auto_health_recover` probe-fail path (L499) | Probe fail → **auto-rebuild from adjacent JSONL** (`recover_from_log`) or raise w/ actionable message | Probe fail → **raise** RuntimeError ("DB health check failed on open… rebuild --dir") — production-fail-loud | `test_ops_safety` (11 tests: `test_auto_rebuild_empty_graph_from_adjacent_log` :77, `test_server_mode_probe_failure_fails_loud` :204, `test_probe_failure_fails_loud_in_production` :175), `test_embedded_concurrency` `test_jsonl_recovery_after_total_graph_loss` :780 | Server behavior IS the target; recovery tests stay in carve-out (embedded-only semantics). No production code change. |
| D3 | Lost-graph check (L519) | Probe ok + 0 nodes + non-empty adjacent log → `recover_from_log` auto-recovery | Skipped entirely — a remote graph is never rebuilt from a local log | `test_ops_safety` (`test_no_rebuild_*` family :64–141) | Same as D2 — carve-out |
| D4 | `_GuardedGraph` bulk-wipe guard (L88) + `_assert_test_graph` (L980 def / L990 prefix check) | **Guard disabled** (returns early when embedded) — per-instance temp DB is inherently isolated | **Guard active**: bulk `DETACH DELETE` refused unless graph name starts `test_`/`tortoise_test_`. **VERIFIED: `"test"` and `"tortoise"` both FAIL the guard.** | Every bulk-wipe test on the default `test` graph: `test_projection` :1461, `test_search_engine_gaps` :1409–1464, `test_a9_direct_edge_traversal` :56, `test_index_surfacing` :239, `test_recall_gaps_subgraph` :141, `test_about_event_untangle` :47, `test_pre_migration_safety` :78/84, `test_embedded_concurrency` (live reset uses `test_live_mw_tortoise` ✓ passes) | **P0 hermeticity constraint**: shared graph can never be named `test`. All docker-mode wipe graphs must be `test_*`/`tortoise_test_*`. SDK's namespace mapping (sdk.py L1115–1123 (verifier-corrected)) already emits `{test_*}_tortoise` — the containment seam. |
| D5 | Range index creation (L1214–1224; point_props L1223) | `point_props = ("id","pointKind","content_hash")` — **identical in both modes** (verifier-checked); a single is_operator range index on server mode would break the composite (D6 comment warning) | Same as embedded — no mode split at THIS site | `test_indexes` `EXPECTED_RANGE_EMBEDDED` (:21) **pins the embedded shape** | Verifier P2-2: the observable docker-mode difference comes from the COMPOSITE at D6, not D5. Do NOT add is_operator to the D5 range set (would re-create #522). |
| D6 | Composite freshness index (L1244–1257) | Plain `(lastDreamedAt)` only | Composite `(is_operator, lastDreamedAt)` — the REAL docker divergence (D5 is identical) | `test_epic903_freshness` composite test (currently docker-skipped), `test_session_index_health`, `test_embedded_concurrency` :532 | Docker now RUNS the composite test; embedded composite expectation stays in carve-out. Mode-split the is_operator range assertions HERE (verifier P2-2) |
| D7 | Embedded repair sweep (L1308) | Drops every Point index containing `is_operator` (permutation sweep, the #522 crash-reopen fix) | **Never runs** — the composite is correct on server engines | crash-recovery reopen paths (`test_crash_recovery`), `test_embedded_concurrency` chaos/reopen | Code path becomes embedded-only (dead on docker); keep for carve-out, no test churn |
| D8 | HNSW vector index (L1497) | **No vector index** — brute-force `vec.euclideanDistance` in `search_engine.run_vector_query` (L393, 528–557) | `CREATE VECTOR INDEX … HNSW` with procedure→Cypher fallback, records `_vector_index_api` | `test_hnsw_vector_index` (3 tests, docker-only probe), `test_search_engine` (docker parts), `test_integration_search`, cross-lens (D9) | Docker path already exercised by live tests; brute-force ordering pinned by `bench/test_smoke_embedded` (explicitly "degraded arms EXPECTED on embedded") — carve-out |
...
|---|---|---|---|---|---|
| D9 | Cross-lens `is_embedded` (sdk.py L6766/6784 (verifier-corrected) → `run_vector_query`) | brute-force over ENTIRE Point index (documented degradation, cost O(pool × total)) | HNSW-accelerated (cost O(pool × top_k)) — **recall ordering can differ** | `test_cross_lens` (in CI half a), `test_search_engine_gaps` | No code change; ordering tolerance belongs in docker-run tests; cross-lens numeric assertions must be docker-calibrated (same class as D12) |
| D10 | Retrieval pool-floor flag (sdk.py L9584/9617 (verifier-corrected) → `is_embedded`) | same API, `is_embedded` forwarded to `run_vector_query` | same | `test_topic_summarization`, retrieval tests | No change — flag carries the D9 semantics |
| D11 | `_probe_embedded_busy` / `EmbeddedStoreBusyError` (sdk.py L1057/call-site L1006 (verifier-corrected)) | **Embedded-only fail-fast**: cross-process same-path open raises (reads `<db>.settings` pidfile, liveness-probes holder) | **No such concept** — server is multi-tenant; concurrent writers on one graph are legal (last-writer-wins per op, `test_concurrent_writers_live_falkor_no_lost_writes` proves no lost writes) | **CARVE-OUT GAP (not in reviewer's list)**: `test_audit` (d) case (:478), `test_pack_state` `TestBackfillScript` dry-run (:668, "embedded FalkorDBLite is single-writer — a subprocess would hit EmbeddedStoreBusyError"), `test_index_directory` `test_e2e9_cross_process_embedded_overlap` (:1855) | Per-test carve-out (3 tests) or rework to server semantics. These 3 tests must NOT be wholesale-migrated. |
...
- SDK namespace mapping (sdk.py L1115–1123 (verifier-corrected)) already auto-generates guard-passing graphs: namespace `test_<x>` → graph `test_<x>_tortoise`. **This is the containment seam to build on.**
...
**E. The no-graph-name raw-construction hole (verifier P1 — CLOSED):**
~80 `FalkorProjection(` sites in tests pass NO `graph_name` and would land on the
server default `"tortoise"` — which FAILS `_assert_test_graph` (bare `tortoise`
...
- **How raw path-constructions reach the server:** when `TORTOISE_DB_URI` is
  set, `FalkorProjection(db_path=...)` must redirect to the server (the seam
  becomes URI-aware at the CLASS level, not just the fixture level) OR the
  ~150 non-seam files must each gain a URI branch. The class-level redirect is
  the lower-touch option (one place), and it makes every raw construction
  docker-safe automatically. Verifier-confirmed: this is the P2-flip blocker;
  the plan must specify it before the matrix flip.
...
**Carve-out total: 339 (17 files) + 3 busy-error per-test = 342 tests (5.0% of 6,837).** With the ~72 already-live tests, **≈95% of the suite migrates** — beats the ≥90% target.
...
**Net allowlist shrink:** 34 → ~21 (16 carve-out files + fixtures + repro) after migration. The `test_no_new_raw_embedded_constructions` enforcement test (L186) keeps working — it just gets a smaller list.
...
| Measurement | Value |
|---|---|
| `pytest tests/ --collect-only` | **6,837 tests in 40.60s** (verifier-reproduced across 3 venvs; the epic's 5,969 does not reproduce) |
...
| Fast-matrix tests (fnmatch over CI file lists × collect-only counts) | half a = **2,606** (151 files), half b = **2,654** (150 files) → **~5,260 fast**; non-fast remainder = **~1,577** (38 files: test-slow + track_b + e2e/) = test-slow file set + track_b-marked + e2e/ + fixtures. The CI comment's "~600 embedded tests" is **stale** (predates the matrix expansion) | ~600/half in CI comment (wrong) |
...
**Constraint:** on the inverted default (docker primary), "no docker" must FAIL CLOSED or skip VISIBLY — never green-skip. The vacuous-pass trap (early-return `skip_if_no_falkor`) is the #942 failure class this epic must not re-create at 6,837-test scale.
...
1. **Scale confirmed (verifier-corrected):** 6,837 tests / 337 files; fast-matrix halves = 2,606 + 2,654 (151/150 files); carve-out = 342 (5.0%) → **≈95% migrates** (target ≥90%). The epic's 5,969 does not reproduce — re-baselined.
2. **NEW P0 constraint (not in the epic):** the graph guard rejects bare `"test"`/`"tortoise"` graph names. The current `shared_proj(graph_name="test")` + `wipe()` pattern cannot port to docker as-is. Hermeticity = guard-passing graph names (`test_*`/`tortoise_test_*`) + a server-mode wipe filtered to test-prefixed graphs. The SDK's namespace→`{test_*}_tortoise` mapping (sdk.py L1115–1123 (verifier-corrected)) is the containment seam; the docker tests already prove the per-test-graph pattern.
...
5. **Allowlist audit:** 34 → ~21 after migration; the 7 drift-registered files (export_cli, import_endpoint, projection, indexes, ingest, supplementary, semantic_extractor) confirmed migratable; 6 of 8 non-carve-out allowlist entries confirmed migratable (only `e2e/hosted/test_12_selfhost_migration` needs plan-time review).
...
grep -rn "_is_embedded" tortoise/ --include="*.py" | grep -v projection/__init__.py  # sdk.py L6766/6784 (verifier-corrected), L8972
