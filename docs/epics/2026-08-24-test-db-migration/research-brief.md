...
| D6 | Composite freshness index (L1225) | Plain `(lastDreamedAt)` only | Composite `(is_operator, lastDreamedAt)` — the REAL docker divergence (D5 is identical) | `test_epic903_freshness` composite test (currently docker-skipped), `test_session_index_health`, `test_embedded_concurrency` :532 | Docker now RUNS the composite test; embedded composite expectation stays in carve-out. Mode-split the is_operator range assertions HERE (verifier P2-2) |
...
~92 `FalkorProjection(` sites in tests pass NO `graph_name` and would land on the
...
**Carve-out total: 271 + 1 = 342 tests (5.0% of 6,837).** With the ~72 already-live tests, **≈95% of the suite migrates** — beats the ≥90% target.
