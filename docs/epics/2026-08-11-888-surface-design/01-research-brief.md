...
| 9 | `topic_summarize` | sdk.py:1891 (fully implemented — epistemic structure: significant/contested/disputed pairs), topic_summarization.py | 6 | **YES** — topic summarization | **WIRED in mcp_server.py (handler `tortoise_topic_summarize` at line 920, calls `_get_team_sdk().topic_summarize()`) but UNREGISTERED in TOOL_REGISTRY** — exposing it requires only a ToolDefinition entry, not new code. Expose-candidate for agent-facing topic summaries. | MEDIUM |
...
| 11 | `session_index_health` | sdk.py:4948 (fully implemented — scans conversation docs, matches files to Events by session_id + file_hash), __main__.py | 3 | **Rare** — admin diagnostic | **NOT WIRED in mcp_server.py** (no handler function) — needs handler + registration to expose. Expose-candidate as admin tool if session indexing health becomes agent-facing. | LOW |
...
- **1/43 is already wired in mcp_server but unregistered** — `topic_summarize` (handler exists at mcp_server.py:920, missing ToolDefinition only). Grep verification of mcp_server.py for the rest: `compute_reputation` (comment-only at line 1097, no function), `complete_source` (no reference), `create_derivation` (no reference), `session_index_health` (no reference).
- **5/43 are SDK methods with no MCP tool that are expose-candidates pending need** (`topic_summarize` — registration-only, `compute_reputation` for EP quality, `complete_source` for source enrichment, `create_derivation` for provenance, `session_index_health` for admin diagnostics)
