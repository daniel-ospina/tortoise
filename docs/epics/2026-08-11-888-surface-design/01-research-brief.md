...
| 9 | `topic_summarize` | sdk.py:1891 (fully implemented — epistemic structure: significant/contested/disputed pairs), topic_summarization.py | 6 | **YES** — topic summarization | **WIRED in mcp_server.py (handler `tortoise_topic_summarize` at line 920, calls `_get_org_sdk().topic_summarize()`) but UNREGISTERED in TOOL_REGISTRY** — exposing it requires only a ToolDefinition entry, not new code. Expose-candidate for agent-facing topic summaries. | MEDIUM |
