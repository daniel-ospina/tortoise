...
- **SDK infrastructure exists but is TWO-MODE, and the delta differs per mode:**
  - **Registry mode (selfhost):** `tortoise/sdk.py` has `_graph_create` (registry Graph node, team→graph 1:N), `graph_list`, `graph_count`; custom namespace shape `org_{org_id}_{graph_id}`. The `Graph` registry entity already exists.
...
- **Tenancy today is team-scoped:** `resolve_api_key` → org_id + limits; MCP carries `_current_org_id`; API key → team → default graph. **No per-graph key resolution exists** in either mode.
