...
| **D12** ⭐ | **Near-duplicate MERGE is authorised**, with **Jev as arbiter** over the claim plus narrative/raw data. ⛔ **Supersedes the `OVERRIDES:` on `#4899`** (*"never merging two claims into one"*) — **the owner reversing their own earlier ruling.** ⚠️ High bar; never across a differing number, name, negation or condition — **the list is now longer: see §16.4 for the authoritative never-across classes (adds language, date/scope and the pair-read load-bearing connective, `#5139`).** | **DECIDED 2026-09-24** |
