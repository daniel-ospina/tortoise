...
| `retractedAt` | ISO8601 | — | — | ✅ | The **fold timestamp written by the retraction fold** (`ObjectRetracted` → `_fold_object_retracted`, coalesced so a re-fold keeps the FIRST stamp). Absent on non-retracted Objects. The tombstone that makes a deleted Object non-current on every read surface while keeping it queryable — #2977. |
...
| updatedAt | ✅ | ❌ | ❌ | ✅ | — | ✅ |
| status | status | ⚠️ SDK extra-prop (projection-owned planned, #2295) | ✅ (fold cache — event-derived) | doc_status | — | — |
