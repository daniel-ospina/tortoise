...
| `CORRECTS` | Point → Point | unidirectional | 1→1 | — | New point **corrects/replaces** an outdated point (supersession). Marks target `outdated: true`; edge disposition is **restatement-scoped per #2421** (see supersession semantics — semantic edges are triaged carry/drop/pend; v1 still transfers, the triage is pending). Created by `supersede_point` (sdk.py:4224) / `invalidate_point` (sdk.py:4126). |
