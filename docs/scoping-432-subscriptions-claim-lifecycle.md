...
| Per-team JSONL event log (`data/events/org_{id}/`) | Data | Step 2 (`subscriptions.py`, flock reuse) | ✅ |
...
| Tenant isolation guard (server-derived org_id) | Cross-cutting | Step 2/3 (AC3) | ✅ |
...
| 3 | Event-store placement | **FalkorDB event nodes in the team's graph namespace** (per research recommendation — `:GraphEvent`, zero-relationship, unique constraint on event_id, index on (org_id, seq)). NOT container JSONL. See docs/research/2026-08-08-event-log-subscriptions.md (verified CLEAN after 2 review cycles). |
