...
| `steps[]` | Per step: name, URL, resolved `ui` state, `observed`, `ok`, detail, screenshot — plus the step-specific `extra`, which `before-observation` and `after-observation` use for `surface` (one of `card` / `wizard` / `none`: which DOM surface produced the read) |
...
* **One edge-only connection predicate — so the guard judges both surfaces the
  same way.** #4646: the Overview card and the wizard's step-3 heading both read
  `connectionObservation.js::harnessConnectionObserved` (the `harness-connected`
  agent step edge), consumed by `overview.js` and `main.jsx`. The instrument
  therefore judges BOTH surfaces with that one vocabulary; it no longer selects a
  derivation per surface (it did while the card additionally accepted the
  server's wire-complete forms). `server_observed(..., accept_wire_complete=True)`
  still accepts those WIRE-COMPLETE FORMS (on a real projection
  `onboarding_complete` IS `resolve_wire_completion`'s output, so probe and rule
  agree by construction — see `server_observed`) for the
  server-parity test — it is not a surface's vocabulary, and judging a screen
  with it would pass a hidden connection the card no longer shows. A new surface
  must read the same one predicate.
