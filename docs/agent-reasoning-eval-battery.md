...
- **What it measures:** reasoning *depth* of the DELIBERATION CONTENT — counter-arguments considered and weighed, specific risks/downsides named, support-vs-oppose weighing made explicit, position revision after counter-evidence. AMENDMENT (#2292): graded constructs are ARM-NEUTRAL (no tool names, edge counts, or arm identity in any judged item/evidence — the graph-only anchors “mitigations filed per support edge” were a verb-availability artifact and are RETIRED from the graded surface; evidence renders are tool-stripped + neutrality-linted).
- **Judged subscore:** an anchored YES/NO coverage rubric (`r2-coverage.json`) grades each deliberation on four constructs (counter-argument weighing, what-could-be-wrong specificity, support-vs-oppose weighing, revision after counter-evidence); the rubric is VALIDATED on real deliberation text before any scoring (2026-09-08 record: retest 1.00, two-model raw agreement 0.886, Gwet AC1 0.849 — under docs/epics/1402-eval-battery/benchmarks/); scoring is BLOCKED without a validated rubric (JudgeGateBlocked). Delta = graph ≥ plain.
...
- **Gate:** JUDGED coverage subscore ratio ≥ 1.5× vs the no-store control WITH the a0=0 floor (control == 0 ⇒ gate passes iff treatment > 0 — decision (e), one canonical form; the old 1.43× tolerated mismatch is retired); ≥ 80% of decisions reach 3+ Challenge/Deepen cycles when graph is used (Decide-skill fidelity, diagnostic only).
...
- **Matched-recall definition (ex ante):** equal top-K factual retrieval F1 (K=5) on a factual probe subset of the scenario corpus, measured before the reasoning battery runs. **Symmetric trigger:** if ANY arm (A0–A4) falls ≥0.10 F1 short of the corpus-best factual retrieval, rerun on a recall-matched balanced subset; if that subset is <50% of the corpus, the differential verdict is **INCONCLUSIVE** (reported, not re-interpreted). **Amendment (2026-09-12, #3327): the trigger's population is the retrieval-capable comparators `{a1, a2, a2b, a3, a4}` — `a0` is EXCLUDED from the trigger and RETAINED as the trigger's positive control and as a published profile row annotated *recall-confounded*. The original sentence above ("ANY arm (A0–A4)") is preserved and read by purpose. Basis: `a0`'s recall is 0.0 by construction (`battery/arms/a0_plain.py:26-27`), so an a0-inclusive trigger fires on every run and makes every differential run INCONCLUSIVE — which would make UNIQUE, and per review MECHANISM-NOT-UNIQUE and WEAK-UNMITIGATED, unreachable by construction. Decision: `docs/research/2026-09-12-matched-recall-a0-control.md`; contract rows §3.2.1/§7 of `docs/benchmarks/comparison-systems.md`. No `trigger_population` constant exists in `matcher.py` yet — wiring is tracked by #3327.** ⚠️ **Corrected 2026-09-13, #3346 — the sentence immediately above is stale/false on current `main` and is preserved verbatim as the pre-wiring record.** PR #3439 (#3327) landed the wiring: `battery/recall/matcher.py:49` defines `TRIGGER_POPULATION: tuple[str, ...] = ("a1", "a2", "a2b", "a3", "a4")`, and `match_recall` takes it as the default of its keyword-only `population` parameter (`matcher.py:206`, `population: Sequence[str] = TRIGGER_POPULATION`); `battery/recall/prepass.py` and `battery/runner/run.py` wire it into the run path. The amendment's **conclusion is unchanged** — the population is still caller-supplied, now explicitly via `population=`, so a test-local population remains expressible and the run-level default is the five comparators. (The same stale clause was corrected in `docs/benchmarks/comparison-systems.md` §3.2.1/§7.)
