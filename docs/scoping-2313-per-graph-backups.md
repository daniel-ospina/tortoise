...
| #669 / #2023 / #770 | `teams.graph_name` is the SOR for the default graph name (Supabase lane mints `org_{org_id}` since #1903; registry-lane SDK teams keep `org_{name}`) | The sweep must keep reading `org_graph_name` for the default and derive custom namespaces from the graphs seam — never assume a naming convention. |
