...
| **MCP rename table (Phase 0.3 of #4282)** — the caller's migration row for every current MCP tool: the name to call **today** (PR #4031's `RETIRED_USE_INSTEAD`, or an explicit `no replacement`), the **target-surface** destination, and whether the old name retires with a warning shim or is simply absent — plus the name-by-name agreement check against #4031 (6 disagreements are findings). **GENERATED** by `tools/mcp_rename_table.py`; never edit by hand | `docs/product/mcp-rename-table.md` |
