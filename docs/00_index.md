...
| **MCP tool surface + public SDK methods** — the curated list: every tool and public method, what it does, what uses it, a recommendation and its rationale; the approved baseline (`config/surface-manifest.yml`) and the gate that reds on an unrecorded drift (#3863). **Adding, removing or renaming a tool or method needs Daniel's approval FIRST** — the gate does not enforce approval (`CONTRIBUTING.md`) | `docs/product/mcp-sdk-surface.md` |
