...
| G9 | **Staleness-error gate (epic #903, DE2E-9 — Indicator 3 acceptance)** — on the F4 frozen-ground-truth fixture (tests/epic903_fixtures.f4_frozen_truth): mutate region R's evidence, measure mean |Δ| of stale beliefs vs the POST-mutation truth, then run stale-first passes with a bounded per-pass budget; assert mean |Δ| ≤ 0.01 once freshness coverage ≥ 80% (threshold, not monotonicity). Thresholds pinned BEFORE implementation and re-locked at calibration; the mutation is load-bearing (validated to move the convergence point's posterior by > 0.01). Implemented in tests/test_epic903_staleness.py |
