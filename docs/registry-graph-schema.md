...
- **Organization account** — the account-level unit of the product (owner direction 2026-09-06). An organization account owns **one or more graphs** and carries billing/tenure/membership. This is what the registry `Team` entity below (and the `teams` table, `team_id`, …) represents: those **legacy code/DB identifiers still read "team"** (phase-2 candidates, #3543). The **public API route is renamed**: the tenant-scoped surface is `/v1/organizations/…` and the OAuth MCP resource is `{origin}/mcp/organizations/{org_id}` (#3543, landed before first customers). User-facing copy and docs call this unit an *organization account* (or *organization* where UI shorthand is established).
