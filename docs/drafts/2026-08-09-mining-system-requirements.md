...
**Cue taxonomy (from the window-1 mitigation audit, 2026-08-11 — the loop's iteration-2
finding):** the relevance-tempering cue set is WIDER than "estimate/telemetry/tension".
Covered cues: "it's an estimate", "decide with real telemetry", "a positioning tension,
not structural", "the caveat is", "only if", "gated on", "the one swing variable is",
"only achievable because", "the leading indicator is", "preliminary", "watch-gate, not a
statistical test", "none would let it be built as-is", "still to run before the gate is
green", "real but not transformative". A deep-miss pattern: the target edge itself is
often not extracted (the whole support structure is missing) — mitigation recall depends
on IMPL-recall first.

**Coverage target (measured, like layer-correctness):** mitigation RECALL ≥0.75 on the
gold set (audit measured 4/14 = 29% on window #1 — the under-identification the owner
flagged). Precision stays high (audit verified zero over-extraction).
