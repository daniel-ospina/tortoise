...
| `ObjectSuperseded` | 1 | hosted commit endpoint (`hosted_api._execute_commit_writes` §6b) + capture (`sdk._extract_session_v2`) + eval (`tools/longmem_eval/ingest_v2`) — entity-level supersession records; ALL THREE emit via the shared `commit_ops.apply_supersessions` (id-style kwargs — #1350/#2164/#2193) | `id`, `name`, `supersedes_by`, `session_id`, `evidence` — id-style kwargs. The GraphEvent payload carries all five for every producer; the JSONL line carries them whenever the producing SDK is built with an `event_log_path` — the hosted producer never is (`_make_sdk` passes no `event_log_path`), nor is the eval harness (`tools/longmem_eval/run.py` builds none), so in-tree only event-log-configured capture (offline mining, `tortoise/mining.py`) journals JSONL | Hosted commit endpoint / SDK capture / eval ingest (all via `commit_ops.apply_supersessions`) |
