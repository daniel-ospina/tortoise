...
THE SURFACE IS OWNER-APPROVED, AND THIS CHECK DOES NOT ENFORCE THAT
    `config/surface-manifest.yml` is a frozen, OWNER-APPROVED baseline. `check` is a
    CONSISTENCY control: it re-derives the baseline from the code and reds on any difference. It
    does NOT verify that a human approved anything. An expansion that edits the declaration AND
    re-cuts the baseline (`cut`) passes BOTH this lint and the D2 gate — so a green gate is not
    approval, and a lane that trusts the machine to stop it will not seek the approval it needs.
    The approval itself is carried by a RULE, not by this check: adding or removing an MCP tool
    or a public SDK method requires Daniel's explicit approval BEFORE the change — raise it as a
    USER QUESTION / DECISION RELAY per `AGENTS.md`. See `CONTRIBUTING.md`
...
# The header comment `cut` writes above the YAML. It is a COMMENT, not data: `check` reads the
# file with `yaml.safe_load`, so it is invisible to the derivation comparison. It is kept in the
# generator rather than typed into the artifact because `cut` rewrites the file from scratch and
# would silently drop a hand-added comment — and the moment a lane runs `cut` is exactly the
# moment the approval rule matters most. `cut` records a NEW surface; it does not approve it.
MANIFEST_HEADER = """\
# ⛔ THE SURFACE THIS BASELINE APPROVES CANNOT CHANGE WITHOUT DANIEL'S APPROVAL.
# Re-running `tools/surface_manifest.py cut` records a NEW surface — it does not approve it.
# Adding, removing or renaming an MCP tool or a public SDK method is the owner's decision: the
# surface is what every agent and customer integration can see and call, so a change to it
# materially affects customer outcomes. Get Daniel's explicit approval FIRST — raise it as a
# USER QUESTION / DECISION RELAY per AGENTS.md; CONTRIBUTING.md has the procedure.
# `tools/surface-guard.py` and this file's `check` catch an UNRECORDED drift only, never an
# unapproved one: an expansion that re-cuts the baseline passes both. A green gate is NOT approval.
"""
...
#     SHAPE only — never that the approval happened. The consent carrier is the mandated RULE
#     (CONTRIBUTING.md §"The MCP tool surface…" + AGENTS.md), i.e. Daniel's own review. There is
#     deliberately no ruleset or CODEOWNERS doing it, and this check cannot: an expansion that
#     re-cuts the baseline passes it (see the module docstring).
...
def cmd_render(args: argparse.Namespace) -> int:
    ...
    add("")
    add("> ⛔ **This list is approved, and it cannot change without Daniel's approval.** Adding or")
    add("> removing an MCP tool or a public SDK method is a decision for the owner: the surface is what")
    add("> every agent and customer integration can see and call, so a change to it materially affects")
    add("> customer outcomes. Raise it as a USER QUESTION / DECISION RELAY per `AGENTS.md` and get")
    add("> Daniel's explicit approval BEFORE the change.")
    add(">")
    add("> **`tools/surface-guard.py` and `tools/surface_manifest.py check` do not enforce that.** They")
    add("> red on an UNRECORDED drift only — a change that re-cuts the baseline passes both, so a green")
    add("> gate is not approval. See `CONTRIBUTING.md` §\"The MCP tool surface and public SDK methods")
    add("> cannot grow without Daniel's approval\".")
    add("")
