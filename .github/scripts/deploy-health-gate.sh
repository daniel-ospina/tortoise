...
#   just no longer fails a deploy that was merely EARLY.
#
# BEHAVIOUR
...
#                       #1381). The field is READ from parsed JSON, never
#                       text-matched: /health's additive `backup_watcher` block
#                       added a SECOND `"ok": true` to the body, and a
...
#                       both planes, so it is the strongest signal and is
#                       reported LAST.
#
...
if [ "$db_ok" != "1" ]; then
  echo "::error::db.ok stayed false for $((DB_TRIES * DB_SLEEP))s — FalkorDB unreachable"
  exit 1
fi
echo "db.ok true"
